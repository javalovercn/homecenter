		...
		return (HCTimer) ContextManager.getThreadPool().runAndWait(new ReturnableRunnable() {
			...
			public Object run() throws Throwable {
				return new HCTimer("", HCTimer.ONE_SECOND * 30, true) {
					...
					public void doBiz() {
						LogManager.warn(
								"long time scheduler job, it maybe block shutdown process of project, please put long task in ProjectContext.run(Runnable) or ProjectContext.runInProjectLevel(Runnable). Job details : ["
										+ projID + "/" + schedulerName + "/" + jobName + "].");
					}
				};
			}
		}, threadToken);
