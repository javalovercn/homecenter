...
public class HCLimitSecurityManager extends WrapperSecurityManager implements HarHelper {
	...
	public static boolean isSecurityManagerOn() {
		return ResourceUtil.isJ2SELimitFunction();// System.setSecurityManager
													// in Android will rise
	...
	public static synchronized final HCLimitSecurityManager getHCSecurityManager() {
		...
		if (hcSecurityManager == null) {
...

			{
				...
				final String[] classNames = { SystemClass, "sun.security.util.SecurityConstants" };// Runtime.thread,
																									// "java.awt.EventQueue",
	...
	// private final HashSet<String> blockReadFullPathLists = new
	// HashSet<String>();
		...
		if ((currentThread == eventDispatchThread && ((csc = hcEventQueue.currentConfig) != null))
			...
			if (permClass == ReflectPermission.class) {
				if (perm.getName().equals("suppressAccessChecks")) {// JRuby使用反射
					...
					// throw new HCSecurityException("block
					// Field/Method/Constructor setAccessible in HAR Project ["
					// + csc.projID + "]."
					// +
				...
				if (csc != null) {
					...
					if (actions.indexOf("write") >= 0) {
						...
						if (p_key.equals("file.separator")) {
							...
							if (p_key.equals("socksProxyHost") || p_key.equals("socksProxyPort")) {// there
																									// is
																									// no
																									// dot
																									// ('.')
				...
				if (permName.equals("createRobot")) {
					...
					// throw new HCSecurityException("block
					// addAWTEventListener/removeAWTEventListener in HAR Project
					// [" + csc.projID + "]."
					// +
					...
					// throw new HCSecurityException("block accessClipboard in
					// HAR Project [" + csc.projID + "]."
					// +
				...
				if (permissionName.equals("setSecurityManager")) {
					...
					// throw new HCSecurityException("block RuntimePermission
					// [getFileSystemAttributes] in HAR Project [" + csc.projID
					// + "], you may need ProjectContext.getFreeSpace(File).");
				} else {
					// if(csc !=
					// null){//阻止其它RuntimePermission，比如setDefaultUncaughtExceptionHandler
					// throw new HCSecurityException("block RuntimePermission ["
					// + permissionName + "] in HAR Project [" + csc.projID +
					// "].");
					// }
				}
				...
				if (permissionName.equals("control")) {// JRuby正常反射需要
					...
					// throw new HCSecurityException("block LoggingPermission ["
					// + permissionName + "] in HAR Project [" + csc.projID +
					// "].");
				}
	...
	public final void checkMemberAccess(final Class<?> clazz, final int which) {
		if (which == Method.DECLARED) {
			...
			if ((currentThread == eventDispatchThread && ((csc = hcEventQueue.currentConfig) != null))
				...
				// at :
				// Users.homecenter.Documents.eclipse_workspace.homecenter.$_dot_.jruby_dot_jar.jruby.java.java_ext.__file__(file:/Users/homecenter/Documents/eclipse_workspace/homecenter/./jruby.jar!/jruby/java/java_ext.rb:3)
				// at :
				// Users.homecenter.Documents.eclipse_workspace.homecenter.$_dot_.jruby_dot_jar.jruby.java.java_ext.load(file:/Users/homecenter/Documents/eclipse_workspace/homecenter/./jruby.jar!/jruby/java/java_ext.rb)
				// 故关闭下行
				// ResourceUtil.checkHCStackTraceInclude(null,
				// jrubyClassLoad);//不能用系统ResourceUtil.class.Loader，因为JRuby引擎初始化时，会触发此处。
			}
			// 白名单方式
			// final String name = clazz.getName();
			// if(blockMemberAccessLists.contains(name)){
			// final Thread currentThread = Thread.currentThread();
			// if(currentThread == eventQueueThread ||
			// limitGroup.parentOf(currentThread.getThreadGroup())){
			// throw new HCSecurityException("fail to memberAccess on : " +
			// name);
			// }
			// }
		}
		// }
		// System.out.println("checkMemberAccess : " + clazz.getName() + ",
		// which :" + which);
			...
			if (csc != null && (fileCanonicalPath.startsWith(StoreDirManager.RUN_TEST_CANONICAL_PATH, 0)
				...
				if (csc != null) {
					if (fileCanonicalPath.startsWith(tempDirCanonicalPath, 0) == false) {// 非temp目录
						...
						// throw new HCSecurityException("block write file :" +
						// file + ", file type [" + forbidExt[i] + "] is
						// forbid.");
						// }
						// }
						// }
						// if(canonicalLowerPath.startsWith(hcRootPathLower, 0)
						// == false){
						// isTryWrite = true;
						//// throw new HCSecurityException("block write file :"
						// + file + ", outside dir:" + hcRootPath);
						// }
	...
	public final void checkExec(final String cmd) {
		...
		if (csc != null) {
			...
			// throw new HCSecurityException("block exec on file :" +
			// blockFile);
			// }
			// }
			...
			// Not in csc
			// if(fileCanonicalPath.startsWith(StoreDirManager.TEMP_CANONICAL_PATH,
			// 0)){
			// }else{
			checkHCStackTrace();
			// }
	...
	// try {
	// blockReadFullPathLists.add(new
	// File(file).getCanonicalPath().toLowerCase(locale));
	// } catch (IOException e) {
	// ExceptionReporter.printStackTrace(e);
