...
public class SafeDataMover {
	...
	static final void copyToTarget(final File srcParent, final File targetParent){
		...
		if(lists == null){
			L.V = L.WShop ? false : LogManager.log("there is no thing in src (cp) : " + srcParent.getAbsolutePath() + ", isDir : " + srcParent.isDirectory());
			return;
		}
		
		targetParent.mkdirs();
		
		L.V = L.WShop ? false : LogManager.log("copy dir src : " + srcParent.getAbsolutePath() + ", target : " + targetParent.getAbsolutePath());
