...
//	    		nioRelay = new NIOServer(null, ia.getHostAddress(), 0, 0, new RelayActionRead(ctx));
	    	}
	...
	//		publicShowIP = getStunIP(rootIP, rootPort);
			...
			if(networkInterfacename.equals(HttpUtil.AUTO_DETECT_NETWORK)){
				final String host = (PropertiesManager.isSimu() == false)?RootServerConnector.HOST_HOMECENTER_MOBI:RootServerConnector.IP_192_168_1_102;
		    	...
		    	if(UPnPUtil.isSupportUPnP(ia)){
					...
					if(StarterParameter.checkDirectPublic(ia, StarterParameter.publicShowIP, true)){
		        		...
		        		final String[] ups = UPnPUtil.startUPnP(ia, 0, 
		        				StarterParameter.getUPnPPortFromP(PropertiesManager.p_DirectUPnPExtPort), TokenManager.getToken());
...
//			LogManager.log("[multicast] multicast server is alive, skip start new multicast server.");
//			return;
//		}
		...
		final Thread t = new Thread(){
			...
			public void run(){
				...
				while (true){
		            ...
		            L.V = L.WShop ? false : LogManager.log("[multicast] receice query from : " + recvPack.getAddress().toString() + ", port : " + recvPack.getPort());
			...
			final MulticastSocket tryJoinGroup(final MulticastSocket server, final InetAddress addr, final NetInterAddresses ni){
				try{
					if(ni != null){
						...
						LogManager.log("[multicast] success join group address : " + addr.toString() + ", port : " + port + ", from networkInterface : " + ni.ni.toString());
						return server;
					}else{
			...
			final void sendDirectServerIP(final byte[] bs, final DatagramPacket recvPack) {
				...
				try{
					server.send(recvPack);
					L.V = L.WShop ? false : LogManager.log("[multicast] successful send direct server ip/port to " + recvPack.getAddress().toString() + ", port : " + recvPack.getPort());
				}catch (final Throwable ex) {
					ex.printStackTrace();
				}
			}
