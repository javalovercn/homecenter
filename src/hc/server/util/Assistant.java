...
/**
 * the <code>Assistant</code> of current project is implemented by project provider, which process voice command.<BR>
...
public class Assistant {
	 ...
	 * 2. the voice is to open <code>CtrlResponse</code>, and the key words of status of <code>CtrlResponse</code> are included in voice. (open once is required)<BR>
	 * 3. the voice is to open <code>HTMLMlet</code>, and the key words of JLable(s) or tool tip text of JComponent are included in voice. (open once is required)
	 * <BR><BR>
	 ...
	 * 3. server will try to process voice for you instead of writing codes here, which is based on analysing the <code>AnalysableRobotParameter</code> between <code>Robot</code> and UI, so it is NOT recommended to use complex object defined by yourself in {@link Robot#operate(long, Object)}.
	 * <BR><BR>
	 * How to more effectively help the server understand the relationship between UI and <code>Robot</code>, to enhance the automatic processing of voice instruction accuracy?<BR>
	 ...
	 * server can be sure it does right, but if there is no keywords in form, but icons or <code>ScriptPanel</code> in form, you should set keywords by {@link JComponent#setToolTipText(String)}.
	 ...
	 * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;business may be driven from UI (voice assistant, <code>HTMLMlet</code> and <code>Dialog</code>), so it is good practice to implement business in {@link Robot#operate(long, Object)}.<BR>
	 * 3. use the <code>AnalysableRobotParameter</code> instead of complex object defined by yourself.
	 * @param cmd
