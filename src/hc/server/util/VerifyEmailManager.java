            	...
            	if(verifiedStatus[0] == 0){
                	...
                	ContextManager.getThreadPool().run(new Runnable() {
						...
						public void run() {
							...
							while(verifiedStatus[0] == 1 && (sendEmailFrameSnap = sendEmailFrame) != null && sendEmailFrameSnap.isVisible()){
...
//		panel.setBorder(new TitledBorder(""));
		...
		lineEndPanel.add(jPanel3, new GridBagConstraints(0, 0, 1, 1, 1.0,
				0.0, GridBagConstraints.LINE_END, GridBagConstraints.NONE,
	...
	public static void showVerifyEmailWarning(final ThreadGroup threadPoolToken) {
		...
		ContextManager.getThreadPool().run(new Runnable() {
			@Override
			public void run() {
				LogManager.warning("===current email is verified on this server before, but there is an other server is verified by the same email later!===");
				LogManager.warning("===you can't use same email on multiple server!===");
				final String msg = "<html></html>";
				final JPanel panel = new JPanel(new BorderLayout());
				final String emailOnOther = (String)ResourceUtil.get(9196);//"[{email}] should be verified on this server again!";
				final JLabel label = new JLabel(StringUtil.replace(emailOnOther, "{email}", IConstant.getUUID()));
				final StringBuffer sb = new StringBuffer();
				sb.append("<html>");
				sb.append("<BR/>");
				final String descStr = (String)ResourceUtil.get(9197);//this Email was verified on this server,
