...
public class HSQLDBConnProvider implements ConnectionProvider {
	...
	public HSQLDBConnProvider(final String key, final ProjectContext projectContext, final String domainName) {
		this.key = key;
	...
	private final File buildCronDir() {
		return projectContext
				.getPrivateFile(StoreDirManager.HC_SYS_FOR_USER_PRIVATE_DIR + StoreDirManager.CRON_SUB_DIR_FOR_USER_PRIVATE_DIR);
	}
					...
					if (isNewDB || getLastCompactMS(projectContext, domainName) == 0) {
						try {
							...
							final SqlFile sqlFile = new SqlFile(
									new InputStreamReader(HSQLDBDelegate.class.getResourceAsStream("tables_hsqldb.sql"), IConstant.UTF_8),
	...
	private final Connection compackDB(Connection connection, final String domainName) throws SQLException {
		if (System.currentTimeMillis()
				- getLastCompactMS(projectContext, domainName) > ServerUIAPIAgent.getProjDesignConf(projectContext).compactDayMS) {// 低频率能增强数据库安全
			try {
