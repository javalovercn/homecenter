...
public class DiskManager {
		...
		monitor = new HCTimer("FreeSpaceDiskMonitor", HCTimer.ONE_MINUTE * 30, true) {
			...
			public void doBiz() {
				if(isLowDiskFreeSpace()) {
					try {
						TrayMenuUtil.displayMessage(ResourceUtil.getWarnI18N(), buildWarnMsg(null), IContext.WARN, null, 0);
	...
	private static String buildWarnMsg(final J2SESession coreSS) {
		final String msg = ResourceUtil.get(coreSS, 9274);//available storage space of server is not enough {freeMin} M.
