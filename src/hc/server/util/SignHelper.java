...
//		privateKeyPem = privateKeyPem.replaceAll("\\s", "");
//
//		final DerInputStream derReader = new DerInputStream(Base64.decode(privateKeyPem));
//
...
//	}
	...
	public static void savePfx(final File pfxFile,
			final String password, final Vector<SignItem> items) throws Exception {
	...
	private static X509Certificate signerSelf(final X500Name subject,
...
//		 privateKey.getAlgorithm();
		final String signatureAlgorithm = "SHA512with"
				+ privateKey.getAlgorithm();
		return signer(subject, publicKey, subject, privateKey,
				signatureAlgorithm, notBefore, notAfter);
	}

	private static X509Certificate signer(final X500Name subject,
			final PublicKey subjectPublicKey, final X500Name issuer,
			final PrivateKey issuerPrivateKey, final String signatureAlgorithm,
			final Date notBefore, final Date notAfter) throws Exception {

		final BigInteger sn = new BigInteger(new SimpleDateFormat(
...
//		builder.addExtension(X509Extensions.SubjectKeyIdentifier, false, new SubjectKeyIdentifierStructure(pubKey));
	...
	// 1000);
	public X509Certificate createCACert(final PublicKey publicKey,
			final PrivateKey privateKey, final Date notBefore,
			final Date notAfter, final String x500Name) throws Exception {
		...
		final X500Name issuerName = new X500Name(x500Name);// "CN=www.mockserver.com, O=MockServer, L=London, ST=England, C=UK"
		final X509Certificate cert = signerSelf(issuerName, publicKey,
				privateKey, notBefore, notAfter);
	 ...
	 */
	public static SignItem[] getContentformPfx(final File filePfx,
			final String strPassword) throws Exception{
		FileInputStream fis = null;
		try {
			...
			while (enumas.hasMoreElements()) {
				...
				final PrivateKey prikey = (PrivateKey) ks.getKey(keyAlias,
						pwdChars);
				out.add(prikey);
	...
	private static char[] toPfxPassword(final String strPassword) {
		char[] pwdChars = null;
		if ((strPassword == null) || strPassword.trim().equals("")) {// password is empty string
			pwdChars = null;
		} else {
