...
public class KeySM extends TableSM {
	...
	public KeySM(final AIPersistentManager mgr, final RLabelKeySM rLabelKeySM) throws SQLException {
		...
		queryLableID = mgr.getConnection().prepareStatement("SELECT labelID " +
				"FROM " + rLabelKeySM.tableName + " LEFT JOIN " + tableName + " ON " + rLabelKeySM.tableName + ".keyID = " + tableName + ".id " +
				"WHERE " + tableName + ".keyValue = ?" +
				"GROUP BY " + rLabelKeySM.tableName + ".labelID;");
	}
