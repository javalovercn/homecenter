...
public class J2SESIPContext extends ISIPContext {
	...
	public J2SESIPContext(final J2SESession j2seCoreSS) {
		...
		resender = new UDPPacketResender() {
			...
			public final void sendUDP(final byte ctrlTag, final byte subCtrlTag,
				...
				if (split_num > 50) {
					// LogManager.log("Send out blob UDP data. num:" +
					// split_num);
				}
			}
		};
	...
	public boolean buildUDPChannel(final HCConnection hcConnection) {
		...
		try {
			...
			// LogManager.log("buildUDPChannel : " +
	 ...
	 * @throws Exception
	 */
	// public StunDesc stun(Object iaddress, String stunServer, int
	...
	//
	// try{
	// upnplisten = new Socket(new InetSocketAddress((InetAddress)iaddress,
	// udpPort));
