				...
				try {
					...
					while (it.hasNext()) {
						...
						if (key.isValid() == false) {
							// LogManager.log("Invalid SelectionKey, skip and
							// continue.");
							continue;
						}
						...
						if ((op & SelectionKey.OP_WRITE) != 0) {
							...
							while (true) {
								...
								if (writeBB == null) {
									// LogManager.log("[RelayCache] Close
									// OP_WRITE");
									...
									try {
										...
										if (writeBB.hasRemaining()) {
											...
											// Write , but has remaining");
											sc.setRewriteSet(currChannel, writeBB);
										} else {
											// LogManager.log("[RelayCache]
											// Write , clear , try next write");
											writeBB.clear();
	...
	private void acceptSession(final SelectionKey key) {
		try {
			...
			if (read.isTop(incomingChannel)) {
				...
				// 例如, 如果参数 connectionTime 为 2, 参数 latency 为 1, 而参数bandwidth 为
				// 3, 就表示最高带宽最重要, 其次是最少连接时间, 最后是最小延迟.
				...
				// 与socket.setSoTimeout(2000)不同，这个是设置InputStream上调用
				// read()阻塞超时时间。
