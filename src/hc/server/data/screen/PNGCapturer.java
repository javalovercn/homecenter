...
public abstract class PNGCapturer extends Thread implements ICanvas {
	...
	protected void cycleCapture() {
		while (!isShutDown) {
			...
			// LogManager.log("Auto Refresh Rect:" +
			...
			// LogManager.log("Auto Refresh Rect Finish:" +
			// System.currentTimeMillis());
			...
			for (int i = oriX; i < endX;) {
				...
				// 比较色块
				{
					...
					for (int n = 0; n < tailHeight; n++) {
						...
						// LogManager.log("snap idx:" + clientSnapIdx + ", rgb
						// idx:" + idxPixel + ", locY:" + locY + ", oriY:" +
						// oriY);
						for (int m = 0; m < tailWidth; m++) {
							...
							if (rgb_v != clientSnap[clientSnapIdx]) {
								if (diffTopLeftX == -1) {
									...
									diffDownRightX = m;
									diffDownRightY = n;
									// LogManager.log("first Diff x : " + m + ",
									// y : " + n +", snap idx:" + clientSnapIdx
									// + ", rgb idx:" + idxPixel + ", locY:" +
									// locY + ", oriY:" + oriY);
								} else {
									if (m >= diffTopLeftX) {
										if (m > diffDownRightX) {
					...
					if (diffDownRightX != -1 && (diffDownRightX > diffTopLeftX || diffDownRightY > diffTopLeftY)) {
						...
						if (copyHeight == tailHeight && copyWidth == tailWidth) {
							...
							// 重新编制块信息数据到rgb变量中
							// // LogManager.log("Send Block, width:" +
	...
	void sendBlock(final int clientX, final int clientY, final int[] rgb, final int rgb_length, final int width, final int height,
		...
		synchronized (LOCK) {
			...
			// LogManager.log("Img old:" + rgb.length + ", deflate:" +
			// deflateLen + ", byteLen:" + byteLen);
			// totalPNG += pngDataLen;
