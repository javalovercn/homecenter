	...
	public boolean doBiz(final CoreSession coreSS, final HCURL url) {
		...
		if (protocal == HCURL.SCREEN_PROTOCAL) {
			if (HCURL.REMOTE_HOME_SCREEN.equals(url.getElementIDLower())) {
				...
				// ResourceUtil.getScreenDeviceScale());
				// ss.setHeight(screenHeight /
				// ResourceUtil.getScreenDeviceScale());
				// coreSS.context.send(MsgBuilder.E_SCREEN_REMOTE_SIZE, homeBS,
				// ss.getLength());
				// }
			...
			if (HCURL.DATA_CMD_EXIT.equals(url.getElementIDLower())) {
				...
				if (ScreenServer.popScreen(j2seCoreSS) == false) {
					// System.out.println("Receiv Exit , and Notify exit by
					// mobile");
					...
					public void run() {
						...
						if (tryEnter == false) {
							...
							if (status == LinkProjectStatus.MANAGER_VIA_MOBILE) {
								...
								// system is downloading and upgrading
								// project(s), please wait for a moment.
								msgID = 9161;
							} else {
								msgID = 9263;// server is managing/locking
												// projects!
							}
				...
				if (HCURL.DATA_PARA_RIGHT_CLICK.equals(para1)) {
					...
					// final String v =
					// url.getValueofPara(HCURL.DATA_PARA_CLASS);
					...
					UserThreadResourceUtil.getMobileAgent(j2seCoreSS).set(publishID,
					...
					// url.getValueofPara(HCURL.DATA_PARA_WIFI_MANAGER);
