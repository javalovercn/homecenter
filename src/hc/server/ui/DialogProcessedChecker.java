...
public class DialogProcessedChecker {
	...
	public final boolean isContinueProcess(final J2SESession coreSS) {
		if (isForDialogAndNoCheck) {
			...
			if (resLock.isProcessed(coreSS, dialogID, (String) ResourceUtil.get(coreSS, 9240))) {// Dialog
																									// is
																									// processed
																									// by
																									// other!
			...
			// ServerUIAPIAgent.removeQuestionFromMap(coreSS, dialogID,
			// false);//需要接收事件，不能remove
		}
