...
/**
 * <code>HTMLMlet</code> is a UI interaction component, which contains
 * JComponents and present them as HTML (not snapshot) on mobile, you can set
 * CSS for these JComponents. <BR>
 * <BR>
 ...
 * to support HTML5, Android 4.4 (or above) / iPhone 4s (or above) is required.
 * <BR>
 * <BR>
 * to synchronize status with mobile, AWT/Swing listeners are added to these
 * JComponents by server, please don't remove them. <BR>
 * <BR>
 * it is considered as <code>Mlet</code> and presents as snapshot to mobile,
 * when one of the following conditions is met: <BR>
...
public class HTMLMlet extends Mlet {
	...
	/**
	 * load special styles for current <code>HTMLMlet</code>, it must be invoked
	 ...
	 * <BR>
	 * Know more :<BR>
	 * 1. a <code>HTMLMlet</code> will be set RTL by server for the entire page
	 * if language of mobile is RTL.<BR>
	 ...
	 * 
	 * @return the recommended button height in pixels, it is normal used for
	 *         the size of button area on bottom of mobile.
	 * @since 7.0
