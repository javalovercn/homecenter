...
public class DialogHTMLMlet extends HTMLMlet {
	...
	public final void addDialog(final Dialog dialog) {
		...
		setCSS(this, null, "background-color:rgba(" + r + ", " + g + ", " + b
				+ ", .8); filter : Alpha(opacity=80);");// 仍占据空间，background-color
														// : transparent;
		...
		setCSS(dialog, null, "background-color : #" + HTMLMlet.getColorForBodyByHexString() + ";"
				...
				"-moz-border-radius: " + borderRadius + "px; -webkit-border-radius: " + borderRadius
				+ "px; border-radius: " + borderRadius + "px;" + // border: 1pt
																	// solid
																	// #000000;
				"filter:progid:DXImageTransform.Microsoft.Shadow(color=#000000, Direction=0, Strength="
				+ (shadowHV * 2) + ");" + /// *ie*/direction 阴影角度 0°为从下往上 顺时针方向,
				...
				"-webkit-box-shadow: " + shadowHV + "px " + shadowHV + "px " + blur + "px #000000;"
				+ /// *safari或chrome*/
