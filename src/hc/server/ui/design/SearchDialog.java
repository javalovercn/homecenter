...
public class SearchDialog extends HCDialog implements DesignScriptNodeIterator {
	static Highlighter.HighlightPainter SEARCH_CODE_LINE_LIGHTER = new DefaultHighlighter.
			DefaultHighlightPainter(Color.decode("#ADD8E6"));
	...
	final JScrollPane resultScrollPane = new JScrollPane(resultTable, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED, JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
...
//				if(lastSelectionOffset <= (sr.offset + sr.length) && sr.offset <= lastSelectionEndOffset){
//					try {
//						sr.highlighter.changeHighlight(sr.highLight, sr.highLight.getStartOffset(), sr.highLight.getEndOffset());
...
//		}
	...
	public final Highlighter.Highlight addSearchHighlight(final Highlighter highlighter, final int startIdx, final int endIdx) throws BadLocationException {
		if(startIdx == endIdx){
			return null;
		}
		...
		searchButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(final ActionEvent e) {
				...
				try{
					...
					App.showMessageDialog(SearchDialog.this, ex.toString(), ResourceUtil.getErrorI18N(), App.ERROR_MESSAGE, App.getSysIcon(App.SYS_ERROR_ICON));
					return;
				}
				doSearch(searchText);
			}
