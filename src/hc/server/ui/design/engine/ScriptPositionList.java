...
public class ScriptPositionList {
	 ...
	 * 
	 * @param startIdx
	 * @param endIdx excludes
	 * @param isAddedBySerial
	 */
	public final ScriptPosition addPosition(final boolean isAddStringExcludesPosi, final int startIdx, final int endIdx, final boolean isAddedBySerial) {
		return addPosition(isAddStringExcludesPosi, startIdx, endIdx, null, isAddedBySerial);
	}
	
	public final ScriptPosition addPosition(final boolean isAddStringExcludesPosi, final int startIdx, final int endIdx, final String item, final boolean isAddedBySerial) {
	 ...
	 * @param isAddedBySerial
	 */
	public final ScriptPosition addPosition(final boolean isAddStringExcludesPosi, final int startIdx, final int endIdx, final String item, final String extItem, final boolean isAddedBySerial) {
			...
			for (int i = 0; i < count; i++) {
				if(isMov == false) {
					...
					if(pStartIdx > startIdx) {
						if(isAddStringExcludesPosi && endIdx > pStartIdx) {//"hello#world" + Thread.sleep()
							positions[i] = p;//替换
							return p;
						}
						tailOne = positions[i];
