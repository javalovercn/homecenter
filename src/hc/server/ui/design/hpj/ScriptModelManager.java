...
public class ScriptModelManager {
		...
		if (type == HPNode.TYPE_MENU_ITEM_CONTROLLER) {
			...
			if (url.getURLLower().indexOf(HCURL.DATA_IOT_ROBOT.toLowerCase()) >= 0) {
				final String[] imports = {
						// "# for Robot API,
						// http://homecenter.mobi/download/javadoc/hc/server/msb/Robot.html\n\n"
						// ,"# for demo,
						// http://homecenter.mobi/en/pc/steps_iot.htm\n\n"
				...
				final String[] imports = {
						// "# for Converter API,
						// http://homecenter.mobi/download/javadoc/hc/server/msb/Converter.html\n\n"
						// ,"# for demo,
						// http://homecenter.mobi/en/pc/steps_iot.htm\n\n"
						"import Java::hc.server.msb.Message\n\n" };
				...
				final String[] imports = {
						// "# for Device API ,
						// http://homecenter.mobi/download/javadoc/hc/server/msb/Device.html\n\n"
						// ,"# for demo,
						// http://homecenter.mobi/en/pc/steps_iot.htm\n\n"
						"import Java::hc.server.msb.Message\n\n" };
				...
				final Vector<String>[] codes = new Vector[methods.length];
				{
					...
					// conn.add("#----if you device need set WiFi account,
					// please remove comments of the following codes.----");
					// conn.add("#ctx = getProjectContext()");
					// conn.add("#isWiFiSetted = \"isWiFiSettedOf\" +
					// getName()#name is required, because there may be multiple
					// types of device in same HAR");
					// conn.add("#if ctx.getProperty(isWiFiSetted,
					// \"false\").equals(\"false\")");
					// conn.add("#\taccount = getWiFiAccount()");
					// conn.add("#\tif account == nil");
					// conn.add("#\t\t#if there is no WiFi module on server (but
					// there is a WiFi router), ");
					// conn.add("#\t\t#please connect to server from mobile via
					// WiFi first, ");
					// conn.add("#\t\t#server will broadcast WiFi account via
					// your mobile.");
					// conn.add("#\t\t#you can also add HAR project from mobile
					// by QRcode.");
					// conn.add("#\t\tctx.tipOnTray(\"No WiFi account or WiFi
					// module to broadcast\")");
					// conn.add("#\treturn");
					// conn.add("#\tend");
					// conn.add("#");
					// conn.add("#\tcommands = \"ssid:\" + account.getSSID() +
					// \";pwd:\" + account.getPassword() + \";security:\" +
					// account.getSecurityOption()");
					// conn.add("#\t----you must encrypt the WiFi account here
					// before broadcasting to air, here is just for demo----");
					// conn.add("#\tbroadcastWiFiAccountAsSSID(\"AIRCOND\",
					// commands)");
					// conn.add("#\t----scan WiFi now, you will find following
					// SSIDs on air, decrypt them in your real device for first
					// connection.");
					// conn.add("#\t----AIRCOND56#1#ssid:abc#efg;pwd:123");
					// conn.add("#\t----AIRCOND56#2#4567890;security:WPA");
					// conn.add("#\t----AIRCOND56#3#2_PSK");
					// conn.add("#\t----Note : the length of an SSID should be a
					// maximum of 32, so it is split to three parts.");
					// conn.add("#");
					// conn.add("#\t----start initial task and wait for device
					// connecting");
					// conn.add("#\t----...");
					...
					// conn.add("#else");
					// conn.add("#\t----Note : WiFi account may be changed when
					// server is shutdown.");
					// conn.add("#\t----start initial task and wait for device
					// connecting");
					// conn.add("#end");
					...
					// final Vector<String> newWiFiAccount = new
					// Vector<String>();
	...
	private static Vector<String> buildInitText() {
		...
		// codes.add("");
		// codes.add("#uncomment follow code to change text of some button, if
		// the button is visiable");
