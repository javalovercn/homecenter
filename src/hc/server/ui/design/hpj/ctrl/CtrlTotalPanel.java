			...
			for (int i = 0; i < keys.length; i++) {
				...
				if(bufferedImage.getWidth() > bufferedImage.getHeight()){
					r_img = ResourceUtil.resizeImage(bufferedImage, 
							d.width, bufferedImage.getHeight()*d.width/bufferedImage.getWidth()); 
				}else{
					r_img = ResourceUtil.resizeImage(bufferedImage, 
							bufferedImage.getWidth()*d.height/bufferedImage.getHeight(), d.height);
				}
...
//				graphics2d.setComposite(AlphaComposite.SrcOver);//g.setComposite(AlphaComposite.Src);
				graphics2d.drawImage(r_img, 0, y, r_img.getWidth(), r_img.getHeight(),
						null);
				graphics2d.dispose();
		...
		
		{
			...
			comp.add(new JScrollPane(panle_but_list, 
