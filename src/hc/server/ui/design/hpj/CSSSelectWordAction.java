	...
	public void actionPerformed(final ActionEvent e) {
		...
		if (target != null) {
			...
			try {
				...
				// if(selectedText.length() > 0 && eventSrc instanceof
				// HCTextPane){
				...
				// "$", "\\$");
				// final Pattern varPattern = Pattern.compile("\\b" + matchText
				// + "\\b");
				// final Matcher m = varPattern.matcher(text);
				// while(m.find()){
				// final int start = m.start();
				// final int end = m.end();
				// styleDoc.setCharacterAttributes(start, end - start,
				// SelectWordAction.BG_SELECTED_VAR, false);
				// }
