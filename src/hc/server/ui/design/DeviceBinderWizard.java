...
public class DeviceBinderWizard extends JPanel{
	...
	public static DeviceBinderWizard getInstance(final IoTSource rs, final boolean isNoCancelOperation, 
			final Frame owner, final MobiUIResponsor respo, final ThreadGroup threadPoolToken) throws Exception {
...
//			c.anchor = GridBagConstraints.PAGE_START;
		...
		chooseConverterButton.addActionListener(new HCActionListener(new Runnable() {
			@Override
			public void run() {
//				System.out.println("call chooseConverterButton actionPerformed" + e.toString());
				final ActionListener listener = new HCActionListener(new Runnable() {
					@Override
					public void run() {
						final DefaultMutableTreeNode node = (DefaultMutableTreeNode)converterTree.getSelectionPath().getLastPathComponent();
...
//		editPanel.setBorder(new TitledBorder("Bind Area :"));
		...
		final JSplitPane panelSubMRInfo = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT,
				treePanel, new JScrollPane(editPanel));//如Android标准屏时，不足
	...
	private void buildDevTree() throws Exception {
		...
		devTree.getSelectionModel().setSelectionMode(  
                TreeSelectionModel.SINGLE_TREE_SELECTION); 
	...
	private void buildConverterTree() throws Exception{
		...
		converterTree.getSelectionModel().setSelectionMode(  
                TreeSelectionModel.SINGLE_TREE_SELECTION); 
		...
		for (int i = 0; i < list.size(); i++) {
			...
			final BindDeviceNode userObject = new BindDeviceNode(mobiResp, BindDeviceNode.CONVERTER_NODE, cbi.proj_id, cbi.name, null, null);
	...
	private void showTreeSelectDialog(final JFrame parent, final JComponent relativeTo, final String title, final JTree tree, final int leafNodeType, final ActionListener listener){
	...
	public void selectTreeNodeAction(final JTree tree, final int leafNodeType,
			final JButton ok, final JLabel desc, final TreePath tp, final boolean actionOK) {
		if(tp == null){
			return;
		}
					...
					if(bdn.type == leafNodeType){
						...
						if(leafNodeType == BindDeviceNode.CONVERTER_NODE){
							...
							try{
								...
								final DataDeviceCapDesc devDesc = rs.getDataForDeviceCompatibleDesc(projResponser, realDevInfo.deviceCompatibleDescriptionCache);
	...
	private String buildConverterCapDescStr(final ProjResponser pr, final DeviceCompatibleDescription upDCD, final DeviceCompatibleDescription downDCD){
		...
		return "<html>" +
				"<STRONG>Up Description :</STRONG><BR>" + upDataDCD.desc +
				...
				"<BR><STRONG>Down Compatible :</STRONG><BR>" + downDataDCD.capList +
				"</html>";
	}
	
	private String buildDevCapDescStr(final DataDeviceCapDesc desc) {
		return "<html>" +
				"<STRONG>Description :</STRONG><BR>" + desc.desc +
				"<BR><STRONG>Version :</STRONG><BR>" + desc.ver + 
				"<BR><STRONG>Compatible :</STRONG><BR>" + desc.capList +
				"</html>";
	}
...
class BindDeviceNode extends BindNode{
	...
	BindDeviceNode(final MobiUIResponsor mobiResp, final int type, final String projID, final String robotOrConverterID, final DeviceBindInfo devBindInfo, final IoTSource rs){
		this.type = type;
		this.projID = projID;
