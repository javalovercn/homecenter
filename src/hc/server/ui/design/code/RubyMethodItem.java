...
public class RubyMethodItem {
	...
	public RubyMethodItem(final String method, final String methodForDoc, final boolean isPublic, final boolean isConstant, final RubyClassAndDoc returnType){
	...
	private RubyMethodItem(final String method, final String methodForDoc, final boolean isPublic, final boolean isConstant, String rt, final Class c){
		this.methodOrField = method;
		this.methodOrFieldForDoc = methodForDoc;
		this.isPublic = isPublic;
		this.isConstant = isConstant;
		...
		if(rt != null){
			...
			if(rt.equals("self")){
				...
				if(returnType != null){
	//				L.V = L.WShop ? false : LogManager.log("[RubyReturn] " + rt + " => " + returnType.docShortName);
				}else{
					L.V = L.WShop ? false : LogManager.log("[RubyReturn] unkown return type : " + rt + ", for method : " + method + ", class : " + ((c!=null)?c.getName():""));
				}
			}
		}else{
