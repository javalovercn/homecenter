...
public class DocHelper {
	...
	static final String rubyMethodRule = ".method-callseq {font-weight:bold; font-size: " + DefaultManager.getDesignerDocFontSize() + "pt;}";
	...
	//		                	click href : ../../java/awt/Toolkit.html#createCustomCursor-java.awt.Image-java.awt.Point-java.lang.String-
						...
						if(classEndIdx >= 0){
							try{
								...
								final String fullClassName = preClass.replace('.', '$').replace('/', '.');//java/util/Map.Entry => java.util.Map$Entry
...
//		System.out.println("-----processDocForOneLevel : " + claz.getName());
		...
		if(clazDoc != null){
			read(c, codeHelper, true, clasName, ResourceUtil.getResourceAsStream("hc/res/docs/ruby/Ruby" + clazDoc.docShortName + "2_2_0.htm"));//RubyString2_2_0.htm
		...
		if(className.indexOf('$') > 0){
			fileName = className.replace('$', '.');//java.util.Map$Entry => java.util.Map.Entry
		}else{
			fileName = className;
		}
	...
	private final void read(final Class c, final CodeHelper codeHelper, final boolean isRubyClass, final String clasName, final InputStream in) {
		if(in == null){
	...
	private static final Pattern rubyStringMethodsList = Pattern.compile("<div id=\"method-list-section\" class=\"section\">" +
			"(.*?)" +
			"</div>", Pattern.DOTALL);
	private static final Pattern rubyConstantsList = Pattern.compile("<h3 class=\"section-header\">Constants</h3>\\s*" +
			"<dl>" +
			"(.*?)" +
			"</dl>", Pattern.DOTALL);
	private static final Pattern rubyConstantsItem = Pattern.compile("<dt><a name=\"(.*?)\">(.*?)</a></dt>\\s*<dd class=\"description\">(.*?)</dd>", Pattern.DOTALL);
	
	private static final Pattern rubyStringMethodItem = Pattern.compile("<li><a href=\"#(.*?)\">(#|::)(.*?)</a></li>");
	private static final Pattern rubyReturnTypeEnumeratorPattern = Pattern.compile("<span class=\"method-callseq\">(.*?)(&rarr;|=>) an_enumerator</span>");
	...
	private static void processRubyDoc(final Class c, final HashMap<String, String> docMap, final String docContent, final String simpleClassName){
...
//		final int publicInstanceMethodIdx = docContent.indexOf("<h3 class=\"section-header\">Public Instance Methods</h3>");
		...
		if (matcher.find()) {
			...
			while(itemMatcher.find()){
				...
				final String firstPart = "<span class=\"method-click-advice\">click to toggle source</span>";//相同的方法可能有多个同名，此段 只 用于第一个之后
...
//				System.out.println("\n\nruby method : " + fieldOrMethodName + "\n" + doc);
	...
	private final static void searchAliasMethods(final Vector<RubyAliasMethod> aliasMethods, final String fieldOrMethodName, 
		...
		final Matcher returnMatcher = rubyReturnTypePattern.matcher(doc);//<span class="method-callseq">now &rarr; time</span>
		while(returnMatcher.find()){
			...
			if(searchAlias.length() == 0){
				continue;
			}else if(searchAlias.startsWith("`")){//[`cmd`] at hc.server.ui.design.code.JRubyKernel
				continue;
			}else if(searchAlias.indexOf("::", 0) > 0){//[Kernel::abort] at hc.server.ui.design.code.JRubyKernel
				continue;
			}else if(searchAlias.indexOf(" " + fieldOrMethodName + " ", 0) > 0){//ary <=> other_ary
				continue;
			}
	...
	private static String searchRubyReturnType(final String doc){
		if(rubyReturnTypeEnumeratorPattern.matcher(doc).find()){//<span class="method-callseq">each_byte &rarr; an_enumerator</span>
	...
	public static String buildCodeParameterList(String parameter){
		parameter = StringUtil.replaceBound(parameter, '<', '>', "");//(Map<? extends K, ? extends V> map)=> (Map map)
	...
	private static boolean replaceCode(final String clasName, final String methodForDoc, final String codeParameterList, final ArrayList<CodeItem> list) {
		final int size = list.size();
		for (int i = 0; i < size; i++) {
...
//			fieldOrMethodName = generics_e_to_object_pattern.matcher(fieldOrMethodName).replaceAll(CodeHelper.OBJECT_STR);
			...
			if(isForMethod){
				if(isForConstruct && simpleClassName != NULL_CONSTRUCTOR_SIMPLE_CLASS_NAME){
					fieldOrMethodName = StringUtil.replaceFirst(fieldOrMethodName, simpleClassName, CodeHelper.JRUBY_NEW);//将构造方法转为new()方法，但不将后面的参数。new Properties(Properties) => new(new)
				}
				
				final int kuohaoLeftIdx = fieldOrMethodName.indexOf("(");
				String parameter = fieldOrMethodName.substring(kuohaoLeftIdx);
				parameter = jianKuoHao.matcher(parameter).replaceAll("");//将setToolTipText()参数内"(<a href="../../java/lang/String.html" title="class in java.lang">String</a> text)"=>"(String text)"
				//&lt;(.*?)&gt;//保留generic-type
				parameter = generics_e_left.matcher(parameter).replaceAll("<");
				parameter = generics_e_right.matcher(parameter).replaceAll(">");
//				parameter = generics_e_type.matcher(parameter).replaceAll("");
//				parameter = generics_e_to_object_pattern.matcher(parameter).replaceAll(CodeHelper.OBJECT_STR);
				...
				parameter = replaceVar(parameter, ",");//method(int hello, boolean yes) => method(int, boolean yes)，但不是Map<? extends K, ? extends V>
				parameter = parameter.replaceAll(" \\w+\\)", ")");//method(int, boolean yes) => method(int, boolean)//有可能boolean另起一行
...
//				}
	...
	private final void toCache(final Class c, final CodeHelper codeHelper, final boolean isRubyClass, final String clasName, final String docContent) {
