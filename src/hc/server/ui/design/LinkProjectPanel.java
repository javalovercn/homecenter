...
public class LinkProjectPanel extends ProjectListPanel {
			...
			public void setValueAt(final Object aValue, final int rowIndex, final int columnIndex) {
				if (columnIndex == COL_NO) {
					...
					if (columnIndex == COL_PROJ_ID) {
						...
						// panel.add(new JLabel("root project must be active!",
						// App.getSysIcon(App.SYS_ERROR_ICON),
						// SwingConstants.LEADING), BorderLayout.CENTER);
						// App.showCenterPanel(panel, 0, 0,
						// (String)ResourceUtil.get(IContext.ERROR), false,
						// null, null, null, null, self, true, false, null,
						// false);
						// return;
						// }
						// }
				...
				new AbstractDelayBiz(null) {
					...
					final void removeBiz() {
						...
						App.showCenterPanelMain(askPanel, 0, 0, ResourceUtil.get(9145), true, null, null,
								...
								}, self, true, false, null, false, false);// 不relativeTo
																			// removeBut，提示会盖住OK按钮
					}
				},
				// import
						...
						addFromRemote.addActionListener(new HCActionListener(new Runnable() {
							...
							void doAddOp() {
								...
								try {
									...
									url = App.showInputDialog(self, ResourceUtil.get(9148), url);
								...
								try {
									...
									if ((hadmd5.length() > 0 && ResourceUtil.getMD5(fileHar).toLowerCase().equals(hadmd5.toLowerCase()))
										...
										throw new Exception(ResourceUtil.get(9270));// 9270=file
																					// verification
																					// error,
																					// try
																					// again
	...
	public final void saveUpgradeOptions() {
		...
		PropertiesManager.setValue(PropertiesManager.p_EnableLinkedInProjUpgrade,
				ch_autoUpgrade.isSelected() ? IConstant.TRUE : IConstant.FALSE);
