...
public class Workbench {
	...
	protected final void todo(final Message msg, final int fromProcType) {
		...
		// Workbench.V = Workbench.O ? false : log("todo workbench,
		// header_refer_num : " + msg.header_refer_num);
		// }

		// 分发消息
		if (msg.ctrl_level > MSBException.TOO_MUCH_FORWARD) {
			// Workbench.V = Workbench.O ? false : err("too much times
			// forwarding on Message : " + msg.toString());
	...
	/**
	 * recycle a <code>Message</code> instance. <br>
	 * Important : the object msg must be returned from
	 * {@link #process(Message, boolean, int, Processor)} or
