...
public class DirectServer extends Thread {
	...
	public void run() {
		while (!isShutdown) {
			...
			try {
				...
				if (isDemoServer) {
					ExceptionReporter.printStackTrace(e);// 必须，在OpenJDK
															// DemoServer
				}
	...
	private final void processOneClient(final Socket socket, J2SESession coreSSMaybeNull) {
		try {
			// LogManager.log("Server is build conn for other, cancle the
			// coming");
			// LogManager.log(" Coming, " +
			...
			try {
				...
				// try{
				// Thread.sleep(ThreadPriorityManager.NET_FLUSH_DELAY);//override
				// same token，所以关闭
				// }catch (final Exception e) {
				// }
