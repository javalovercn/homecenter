...
public class LineOnManager {
	...
	private static String printServerInfo(final String currID, final String hideToken) {
		...
		for (int i = 0; i < size; i++) {
			...
			if (dlo.id.equals(currID)) {
				if (dlo.hideIP.equals(IConstant.FALSE) || dlo.hideToken.equals(hideToken)) {
					// 必须要hideToken也吻合
	...
	private static String printNewRelayServers() {
		...
		// FROM `lineon` WHERE nattype = 100 ORDER BY serverNum ASC, lenid DESC
		// LIMIT 20;');
	...
	private static void removeUnalive(final String currID) {
		// if($_GET['id'] == 'root'){
		// mysql_query('DELETE FROM `lineon` WHERE alive < date_sub(now(),
		// INTERVAL 26 hour);');//两小时未刷新
		// }
	...
	public static String process(final HCURL hcurl) {
		...
		if (f.equals("lineon")) {
			...
			// INSERT INTO `lineon`
			// (id,ip,port,nattype,agent,upnpip,upnpport,alive,token,
			// serverNum,relayip,relayport,lenid) VALUES ("' . $id . '", "' .
			// $ip .'",
			// '.$port.','.$nattype.','.$agent.',"' . $upnpip . '",
			// '.$upnpport.',
			// now(),"'.$token.'",0,"'.$_GET['relayip'].'",'.$_GET['relayport'].
			// ',length("'.$id.'")) ON DUPLICATE KEY UPDATE ip = "'.$ip.'",
			// port='. $port . ', nattype='.$nattype.', agent = '.$agent.',
			// upnpip = "'.$upnpip.'", upnpport = '.$upnpport.',
			// token="'.$token.'", serverNum=0,
			// relayip="'.$_GET['relayip'].'",relayport='.$_GET['relayport'].',
			// alive=now(), lenid=length("'.$id.'");');
			...
			for (int i = 0; i < size; i++) {
				final DataLineOn dlo = vector.elementAt(i);
				// DELETE FROM `lineon` WHERE id = "' . $id . '" AND token =
				// "'.$_GET['token'].'";');
				if (dlo.id.equals(para_ID) && dlo.token.equals(para_TOKEN)) {
					if (L.isLogInRelay) {
						LogManager.log(
			...
			for (int i = 0; i < size; i++) {
				final DataLineOn dlo = vector.elementAt(i);
				// alive = now() WHERE id = "'.$_GET['id'].'" AND token =
				// "'.$_GET['token'].'";');
				if (dlo.id.equals(para_ID) && dlo.token.equals(para_TOKEN)) {
			...
			for (int i = 0; i < size; i++) {
				final DataLineOn dlo = vector.elementAt(i);
				// serverNum = ('.$_GET['serverNum'].') WHERE id =
				// "'.$_GET['id'].'" AND token = "'.$_GET['token'].'";');
				if (dlo.id.equals(para_ID) && dlo.token.equals(para_TOKEN)) {
