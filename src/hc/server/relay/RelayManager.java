...
public class RelayManager {
	...
	private static final Exception WriteSetOvelException = new Exception(
			"WriteSet overfull, please increase option [RelayBlockNum]");
	...
	/**
	 * 由于采用数据包直接提取地址法，该法有利于Relay迁移； 而对于服务器初始上线注册，采用特别Tag
	 * 
	 * @param ctx
	 * @param obj
	 */
	public static void relay(final IContext ctx, final SelectionKey key) {
		...
		if (sc != null) {
			...
			try {
				// if(L.isLogInRelay) {
				// LogManager.log("Data In, Check target Channel
				// Server/Client["+targetServerOrClient+"] is
				// reset["+isTargetReset+"]");
				// }
				if ((targetChannel != null)) {
					...
					do {
						...
						// if(L.isLogInRelay) {
						// LogManager.log("DirectBuffer readed " + readLen + "
						// bytes.");
						// }
						if (readLen > 0) {
							// 转发
							...
							if (oldwriteSetSize == 0) {
								// if(L.isLogInRelay) {
								// LogManager.log("targetChannel is WriteSet
								// Empty, write to target");
								// }
								// 直接转发，增加性能
								targetChannel.write(bufferDirect);
								if (bufferDirect.hasRemaining()) {
									// if(L.isLogInRelay) {
									// LogManager.log("[RelayCache] write to
									// target direct, but has remain,
									// toWriteBackend");
									// }
									...
									// if(L.isLogInRelay) {
									// LogManager.log("write to target direct,
									// clear");
									// }
								}
							} else {
								if (oldwriteSetSize < RelayBlockNum) {
									...
									// if(L.isLogInRelay) {
									// LogManager.log("[RelayCache]
									// targetChannel has WriteSet,
									// toWriteBackend");
									// }
				...
				if (L.isLogInRelay) {
					LogManager.log("ServerOrClient[" + (!targetServerOrClient) + ", isMatched : "
							+ sc.isMatched + ", remotePort:" + sourceChannel.socket().getPort()
							+ "] closed.");
				}
			...
			try {
				...
				// 断线
				if (L.isLogInRelay) {
					LogManager.log("lineOff or Exception [" + e.getMessage() + "] [hashCode: "
							+ sourceChannel.socket().hashCode() + ", remotePort:"
							+ sourceChannel.socket().getPort() + "] at channel!");
				}
				try {
	...
	private static void toWriteBackend(final SessionConnector sc, final SocketChannel targetChannel,
			final ByteBuffer bb, final boolean serverOrClientReset) throws ClosedChannelException {
		// 转缓存，进入后备模式
		...
		synchronized (sc) {
			...
			if (lineOffSessionConn(sc) && notifyMinus) {// notifyMinus一定要置于，因为前者(lineOffSessionConn)一定要被执行
				...
				if (isDirectOK == false && isMatched == false && id != null && token != null) {
					...
					RootServerConnector.delLineInfo(id, token);
				}
				minusServNum();
			}
		}
	...
	private static void closeClient(final SessionConnector sc) {
		...
		if (c != null) {
			try {
				// if(L.isLogInRelay) {
				// LogManager.log("close client channel : " +
	...
	private static void closeServer(final SessionConnector sc) {
		...
		if (c != null) {
			try {
				// if(L.isLogInRelay) {
				// LogManager.log("close server channel : " +
				// c.socket().getInetAddress().getHostAddress() + ":" +
				// c.socket().getPort());
				// }
				c.socket().close();
	...
	public static boolean pushMap(final IContext ctx, final SelectionKey key) throws Exception {
		...
		if (headReadLen != MsgBuilder.MIN_LEN_MSG) {
			if (headReadLen == -1) {// || headReadLen == 0
				if (L.isLogInRelay) {
					LogManager.log("Unknow head data reg. readed:" + headReadLen + ", expect:"
							+ MsgBuilder.MIN_LEN_MSG + ", at channel : "
		...
		if (ctrlTag == MsgBuilder.E_TAG_RELAY_REG) {
			...
			if (bodyReadLen != dataLen) {
				if (bodyReadLen == -1) {
					if (L.isLogInRelay) {
						LogManager.log("Unknow data reg, readed:" + bodyReadLen + ", expect:"
								+ dataLen + ", at channel : "
								+ channel.socket().getInetAddress().getHostAddress() + ":"
			...
			dr.setBytes(bs);
			final boolean isServerOnRelay = (dr
					.getFromServer() == MsgBuilder.DATA_IS_SERVER_TO_RELAY);
				...
				if (sc != null) {
					if (isServerOnRelay) {// 服务端
						...
						if (newToken.equals(sc.token)) {// 上次的连接仍alive，待接入者进行等待
							if (L.isLogInRelay) {
								LogManager.log(
										"the same token account seem alive, append second or more, token ["
												+ newToken + "].");
							}
							...
							final boolean isRegedToken = RootServerConnector
									.isRegedToken(uuidString, newToken);
							if (isRegedToken) {
								// 注册级认证Token，强制关闭旧的。
								if (L.isLogInRelay) {
									LogManager.log("Override token id[" + uuidString
											+ "] for reged " + newToken + ", at channel : "
											+ channel.socket().getInetAddress().getHostAddress());
								}
						...
						if (sc.clientSide != null) {
							...
							if (L.isLogInRelay) {
								LogManager.log("try override exist client id[" + sc.getUUIDString()
										+ "] at channel : "
										+ channel.socket().getInetAddress().getHostAddress()
						...
						if (L.isLogInRelay) {
							LogManager
									.log("====> write DATA_ROOT_MATCHED_FOR_CLIENT_ON_RELAY, len : "
											+ matchLen);
						}
					...
					if (isServerOnRelay == false) { // Mobi客户端 且 服务器没上线
						// 因为下面检查并发回错误状态，所以此处注释|| (sc.fromClient !=
						// null))//原旧客户连接仍保持，则新占客户端非法
						// 不回应，以表明此连接为非法。
						if (L.isLogInRelay) {
							LogManager.log(
									"Error client reg status(No server nor exist old client), NO write echo back to cancel this channel.");
						}
						throw BBIOException;
					}
			...
			if (erb != null) {
				if (erb.doExt(key, channel, buffer) == false) {
					if (L.isLogInRelay) {
						LogManager.log("skip unforward packet.");// force
																	// Exception
																	// and close
																	// channel.
					}
	...
	private final static void processUDP() {
		...
		// MsgBuilder.DATA_ROOT_UDP_PORT_NOTIFY;
		//
		// //初始化UDP Header
		...
		// //通知Client UDP Port
		// buffer.clear();
		// ByteUtil.integerToTwoBytes(clientPort, bs,
		...
		// notifyBS, 0, NOTIFY_UDP_DATA_LEN);
		// if(L.isLogInRelay) {
		...
		// //通知Server UDP Port
		// buffer.clear();
		// ByteUtil.integerToTwoBytes(serverPort, bs,
		// MsgBuilder.INDEX_MSG_DATA);
		// buffer.limit(MsgBuilder.INDEX_MSG_DATA + NOTIFY_UDP_DATA_LEN);
		// sc.serverSide.write(buffer);
		//// ContextManager.getContextInstance().send(sc.serverSide.socket().getOutputStream(),
		// notifyBS, 0, NOTIFY_UDP_DATA_LEN);
		// if(L.isLogInRelay) {
	 ...
	 * @throws IOException
	 */
	private static void unforward(final SocketChannel channel) throws IOException {
		if (L.isLogInRelay) {
	...
	public static void continueRead(final ByteBuffer buffer, final SocketChannel channel,
			int readedLen, final int totalReadLen) throws IOException {
		// 数据未完全过来。
		int newRead = 0;
		int sleepCount = 0;
		do {
			...
			readedLen += newRead;
			// if(L.isLogInRelay) {
			// LogManager.log("continueRead newRead:" + newRead + ", readedLen:"
			// + readedLen + ", TotalTryReadLen:" + totalReadLen);
			// }
		} while (readedLen != totalReadLen);
	}

	private static SessionConnector buildSessionConn(final byte[] bs, final int len) {
		SessionConnector sc;
		if (scCacher.size() == 0) {
	...
	public static boolean startMoveNewRelay(final CoreSession coreSocketSession,
			final RelayShutdownWatch watch) {
	...
	/**
	 * 启动中继迁移进程。 停机前，并发现新的Relay，如果成功， 并将现有被服务资源迁移到新的Relay上。
	...
	private static int findNewRelayAndMoveTo(final CoreSession coreSocketSession) {
		...
		// RootServerConnector.delLineInfo(TokenManager.getToken(),
		// false);//多路模式
		...
		synchronized (tdn) {
			...
			final Vector relays = (Vector) RootServerConnector
					.getNewRelayServers(IConstant.getUUID(), TokenManager.getToken());
			if (relays == null) {
				if (L.isLogInRelay) {
				...
				final String slocalIP = HCURLUtil
						.convertIPv46(StarterParameter.getHomeWirelessIP());
				...
				for (int i = 0; i < size; i++) {
					...
					if (StarterParameter.getHomeWirelessPort() == port && slocalIP.equals(ip)) {
						...
						try {
							...
							// 开始输送
							if (L.isLogInRelay) {
								LogManager.log("Move relay clients to new relay server, " + ip + ":"
										+ port);
							}
	...
	static boolean checkIDAlive(final byte[] bs, final int offset, final int len) {
		...
		for (int j = 0; j < sizeStack; j++) {
			...
			try {
				...
				if (isSame) {
					L.V = L.WShop ? false
							: LogManager.log(
									"uuid [" + ByteUtil.toString(bs, offset, len) + "] is alive!");
					return true;
				}
			} catch (final Throwable e) {
			}
		...
		// 删除数据库记录
		L.V = L.WShop ? false
				: LogManager.log("uuid [" + ByteUtil.toString(bs, offset, len) + "] is NOT alive!");
		final String id = "";
		final String token = "";
		RootServerConnector.delLineInfo(id, token);
		return true;
	...
	private static int moveToNewRelay(final String newRelayip, final int newRelayport) {
		...
		for (short i = 0; i < tdn.length; i++) {
			...
			for (int j = 0; j < sizeStack; j++) {
				...
				// sendReceiveRelaySC(newRIP, newRelayport, c, uuid2,
				// uuid_len2);
	...
	private static void sendMoveToNewRelay(final SocketChannel server, final DataNatReqConn nat,
		...
		// if(L.isLogInRelay) {
		// LogManager.log("Move to Relay, IP:" + newRelayServerIP + ":" +
		// newRelayPort);
		// }
		try {
			bb.clear();
