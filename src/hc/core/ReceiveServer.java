...
public class ReceiveServer implements Runnable {
	...
	private static boolean isEnableTestRebuildConnection() {
		return IConstant.serverSide == false && IConstant.TRUE.equals(
				RootBuilder.getInstance().doBiz(RootBuilder.ROOT_GET_MB_PROP,
						"isEnableTestRebuildConn"));
	}
	...
	// private int readFullyShort(final DataInputStream in, final byte b[],
	// final int off, final int len) throws Exception {
	// int c = 0;
	// int index = off;
	...
	// return len;
	// }
	// private void readFully(final DataInputStream in, final byte b[], final
	// int off, final int len) throws Exception {
	...
	//// final int realReadedNum = in.read(b, off + n,
	// bestReadNum>readOnceCount?readOnceCount:bestReadNum);
	// LogManager.log("readOnceCount : " + readOnceCount);
	// final int realReadedNum = readFullyShort(in, b, off + n,
	// bestReadNum>readOnceCount?readOnceCount:bestReadNum);
	// n += realReadedNum;
	//
	// // LogManager.log("receiveFull readOnce:" + realReadedNum + ", readed:" +
	// n + ", targetNum:" + len);
	// }
	// }else{
	...
	static final Exception overflowException = new Exception(
			"Overflow or Error data, maybe clientReset Error");
	static final Exception checkException = new Exception(
			"fail on check integrity of package data");
	String threadID;
	...
	public void run() {
		...
		while (!isShutdown) {
			...
			try {
				...
				if (isEnableTestRebuildConn) {
					if (coreSocketSession.context.cmStatus == ContextManager.STATUS_CLIENT_SELF) {
						...
						if (currMS - last_rebuild_swap_sock_ms > 1000 * 30) {
							L.V = L.WShop ? false
									: LogManager.log(
											"[ReceiveServer] start test close connection and rebuild/renewal.");
							coreSocketSession
									.closeSocket(coreSocketSession.getSocket());
				...
				if (isInWorkshop) {
					LogManager.log("[ReceiveServer]  inputStream : ["
							+ dataInputStream.hashCode()
							+ "] ready receive in ReceiveServer : " + threadID);
				}
				...
				final int dataLenWithCheckLen = isXor ? (dataLen + checkBitLen)
						: dataLen;

				if (isInWorkshop) {
					LogManager.log("[ReceiveServer]  ["
							+ dataInputStream.hashCode() + "]:" + threadID
							+ " Receive One packet [" + ctrlTag + "]["
							+ bs[MsgBuilder.INDEX_CTRL_SUB_TAG]
							+ "], data len : " + dataLen);
				}
				...
				if (bsDataLen < dataLenWithCheckLen) {
					...
					final byte[] temp = recBytesCacher.getFree(
							dataLenWithCheckLen + MsgBuilder.MIN_LEN_MSG);
				...
				if (dataLenWithCheckLen > 0) {
					dataInputStream.readFully(bs, MsgBuilder.MIN_LEN_MSG,
							dataLenWithCheckLen);
				}

				// 先解密再进行check
				if (ctrlTag == MsgBuilder.E_PACKAGE_SPLIT_TCP) {
					final int eachLen = dataLen
							- MsgBuilder.LEN_TCP_PACKAGE_SPLIT_DATA_BLOCK_LEN;
					CUtil.superXor(hcConnection, hcConnection.OneTimeCertKey,
							bs, MsgBuilder.TCP_SPLIT_STORE_IDX, eachLen, null,
							false, true);
				} else {
					if (ctrlTag == MsgBuilder.E_REPLY_TRANS_ONE_TIME_CERT_KEY_IN_SECU_CHANNEL) {
						// 优先提交密码更新
						coreSocketSession.context.doExtBiz(
								IContext.BIZ_REPLY_TRANS_ONE_TIME_CERT_KEY_IN_SECU_CHANNEL,
								null);
					}
					if (isXor == false) {
					} else {
						if (hcConnection.OneTimeCertKey == null) {
							int sleepTotal = 0;
							while (hcConnection.OneTimeCertKey == null
									&& sleepTotal < 3000) {// Android模拟环境下600偏小
								try {
									sleepTotal += ThreadPriorityManager.UI_WAIT_OTHER_THREAD_EXEC_MS;
									Thread.sleep(
											ThreadPriorityManager.UI_WAIT_OTHER_THREAD_EXEC_MS);
								} catch (Exception e) {
								}
							}
						}
						// 解密
						CUtil.superXor(hcConnection,
								hcConnection.OneTimeCertKey, bs,
								MsgBuilder.INDEX_MSG_DATA, dataLen, null, false,
								true);
					}
				}
				...
				if (isXor) {
					...
					// 检查是否为重发包
					final long realXorPackageID = ByteUtil.eightBytesToLong(bs,
							xorPackgeIdxOff);
					// final boolean isForceXorError = isSimuRetransError ==
					// false && isEnableTestRebuildConn && ((realXorPackageID %
					// 50) == 0);
					// if(isForceXorError == false && readyReceiveXorPackageID
					// == realXorPackageID){
					if (realXorPackageID == readyReceiveXorPackageID) {
						if (isInWorkshop) {
							LogManager.log(
									"[ReceiveServer] received XorPackageID : "
											+ realXorPackageID
											+ ", ready to check...");
						}
						...
						if (isInWorkshop) {
							LogManager.log(
									"[ReceiveServer] skip received XorPackageID : "
											+ realXorPackageID
											+ ", expected XorPackageID : "
											+ readyReceiveXorPackageID);
						}
					...
					byte realCheckTotal = checkTotal, realCheckAnd = checkAND,
							realCheckMinus = checkMINUS;
					{
					...
					// LogManager.log("dataLen : " + dataLen + ", data : " +
					// ByteUtil.toHex(bs, 0, dataCheckLen + checkBitLen));
					...
					if (realCheckAnd == bs[dataCheckLen]
							&& realCheckMinus == bs[dataCheckLen + 1]) {
						...
						if (isInWorkshop) {
							LogManager.log(
									"[ReceiveServer] success check integrity xor : "
											+ (readyReceiveXorPackageID - 1));
						}

						System.arraycopy(bs, xorPackgeIdxOff, ackXorPackageID,
								0, MsgBuilder.XOR_PACKAGE_ID_LEN);
						coreSocketSession.context
								.sendWrapWithoutLockForKeepAliveOnly(
										MsgBuilder.E_ACK_XOR_PACKAGE_ID,
										ackXorPackageID, 0,
										MsgBuilder.XOR_PACKAGE_ID_LEN);// 注意：必须走明文且impl层
					} else {// fail on check
						// LogManager.errToLog("check idx : " + dataCheckLen +
						// ", real : " + realCheckAnd + "" + realCheckMinus + ",
						// expected : " + bs[dataCheckLen] + "" +
						// bs[dataCheckLen + 1]);
						recBytesCacher.cycle(bs);
				...
				if (ctrlTag == MsgBuilder.E_PACKAGE_SPLIT_TCP) {
					...
					// TCP合并包
					final int newPackageID = (int) ByteUtil.fourBytesToLong(bs,
							MsgBuilder.INDEX_TCP_SPLIT_SUB_GROUP_ID);
					if (hcConnection.package_tcp_id != 0
							&& hcConnection.package_tcp_id != newPackageID) {
						LogManager.errToLog(
								"[ReceiveServer] invalid TCP sub package id : "
										+ newPackageID + ", expected id : "
										+ hcConnection.package_tcp_id);
					...
					if (hcConnection.package_tcp_id == 0) {
						hcConnection.package_tcp_id = newPackageID;
						hcConnection.packaeg_tcp_num = (int) ByteUtil
								.fourBytesToLong(bs,
										MsgBuilder.INDEX_TCP_SPLIT_SUB_GROUP_NUM);
						if (isInWorkshop) {
							System.out.println(
									"[ReceiveServer] ----[Big Msg]-----package tcp id : "
											+ newPackageID + ", num : "
											+ hcConnection.packaeg_tcp_num);
						}
						hcConnection.package_tcp_bs = new byte[MsgBuilder.MAX_LEN_TCP_PACKAGE_SPLIT
								* hcConnection.packaeg_tcp_num
								+ MsgBuilder.TCP_PACKAGE_SPLIT_EXT_BUF_SIZE];
					...
					final int eachLen = dataLen
							- MsgBuilder.LEN_TCP_PACKAGE_SPLIT_DATA_BLOCK_LEN;
					System.arraycopy(bs, MsgBuilder.TCP_SPLIT_STORE_IDX,
							hcConnection.package_tcp_bs,
							hcConnection.package_tcp_last_store_idx, eachLen);
					hcConnection.package_tcp_last_store_idx += eachLen;

					if (isInWorkshop) {
						System.out.println(
								"[ReceiveServer] ----[Big Msg]-----append data tcp id : "
										+ newPackageID + ", num : "
										+ (hcConnection.packaeg_tcp_appended_num
												+ 1)
										+ ", curr len : " + eachLen);
					}

					if (++hcConnection.packaeg_tcp_appended_num == hcConnection.packaeg_tcp_num) {
						HCMessage.setBigMsgLen(hcConnection.package_tcp_bs,
								hcConnection.package_tcp_last_store_idx
										- MsgBuilder.INDEX_MSG_DATA);// 还原数据块总长度
				...
				if (ctrlTag == MsgBuilder.E_TRANS_ONE_TIME_CERT_KEY_IN_SECU_CHANNEL) {
					coreSocketSession.context.doExtBiz(
							IContext.BIZ_UPDATE_ONE_TIME_KEYS_IN_CHANNEL, bs);
					if (isBigMsg == false) {
						recBytesCacher.cycle(bs);
					}
					continue;
					...
					// 优先以完成，所以此处不作
					// coreSocketSession.context.doExtBiz(IContext.BIZ_REPLY_TRANS_ONE_TIME_CERT_KEY_IN_SECU_CHANNEL,
					// bs);
					if (isBigMsg == false) {
						recBytesCacher.cycle(bs);
					}
					continue;
				} else if (ctrlTag == MsgBuilder.E_TAG_ROOT) {// 服务器客户端都走E_TAG_ROOT捷径
																// isClient &&
					// 由于大数据可能导致过载，所以此处直接处理。
					coreSocketSession.context.rootTagListener.action(bs,
							coreSocketSession, hcConnection);
					if (isBigMsg == false) {
						recBytesCacher.cycle(bs);
					}
					continue;
					...
					L.V = L.WShop ? false
							: LogManager.log(
									"[ReceiveServer] receive E_RE_TRANS_XOR_PACKAGE, resend UnReachable packages!!!");
					coreSocketSession.hcConnection.resendUnReachablePackage();
					continue;
				} else if (ctrlTag == MsgBuilder.E_ACK_XOR_PACKAGE_ID) {
					coreSocketSession.hcConnection.ackXorPackage(bs,
							coreSocketSession);
					...
					HCURLUtil.processGotoUrlForNormalAndSuperLevel(bs,
							coreSocketSession);
					if (isBigMsg == false) {
						recBytesCacher.cycle(bs);
					}
					...
					if (ContextManager
							.isServerStatus(coreSocketSession.context)) {// 数据交换状态忽略，仅限新连接且未会话。
					} else {
						if (coreSocketSession.context.isServerSide) {
							coreSocketSession.synXorPackageID(bs);// 仅服务器
							L.V = L.WShop ? false
									: LogManager.log(
											"[ReceiveServer] shutdown ReceiveServer :"
													+ threadID
													+ " for done swap socket.");
							isShutdown = true;
						}
					}
					if (isBigMsg == false) {
					...
					if (isExchangeStatus == false) {
						...
						if (isExchangeStatus == false) {
							L.V = L.WShop ? false
									: LogManager.log(
											"[ReceiveServer] process event in Receive, because not in exchange status.");
							eb.watch();
							continue;
						}
					}
				...
				if (isInWorkshop) {
					if (dataInputStream != null) {
						LogManager.errToLog("[ReceiveServer] Receive Server ["
								+ dataInputStream.hashCode() + "]:" + threadID
								+ " exception : " + e.toString());
					} else {
						LogManager.errToLog("[ReceiveServer] : " + threadID
				...
				if (hcConnection.isInitialCloseReceiveForJ2ME) {
					// 比如：需要返回重新登录
					LogManager.log(
							"[ReceiveServer] close is initial closed, ready to connection.");
					continue;
				}
				...
				if (System.currentTimeMillis() - receiveUpdateMS < 500) {// Android环境下100太小，
					L.V = L.WShop ? false
							: LogManager.log(
									"[ReceiveServer] Exception, receive is changing new socket. continue");
					// try{
					// Thread.sleep(100);
					// }catch (Exception ex) {
					// }
				...
				if (cmStatus == ContextManager.STATUS_EXIT
						|| cmStatus == ContextManager.STATUS_READY_EXIT) {
					try {
						Thread.sleep(100);
				...
				if (hcConnection.sipContext.isNearDeployTime()) {
					LogManager.log(
							"[ReceiveServer] ReceiveServer Exception near deploy time");

					// if(IConstant.serverSide
					// && (ContextManager.cmStatus ==
					// ContextManager.STATUS_NEED_NAT)
					// && (ContextManager.cmStatus ==
				...
				if (e != checkException) {
					...
					if (System.currentTimeMillis()
							- last_rebuild_swap_sock_ms > ThreadPriorityManager.REBUILD_SWAP_SOCK_MIN_MS) {
						L.V = L.WShop ? false
								: LogManager.log(
										"[ReceiveServer] [ConnectionRebuilder] exception on receiver : "
												+ threadID + ", inputStream : "
												+ dataInputStream.hashCode());
						final boolean out = hcConnection.connectionRebuilder
								.notifyBuildNewConnection(false, cmStatus);// 注意：成功后，本receive将复用
						if (out) {
							last_rebuild_swap_sock_ms = System
									.currentTimeMillis();
							L.V = L.WShop ? false
									: LogManager.log(
											"[ReceiveServer] [ConnectionRebuilder] successful build new connection in ReceiveServer : "
													+ threadID);
							continue;
						}
						LogManager.errToLog(
								"[ReceiveServer] [ConnectionRebuilder] fail to build new connection in ReceiveServer : "
										+ threadID);
					} else {
						LogManager.errToLog(
								"[ReceiveServer] [ConnectionRebuilder] last rebuild swap socket is very frenquence, cancel rebuild swap.");
					}
				}

				LogManager.errToLog("[ReceiveServer] Receive Exception:["
						+ e.getMessage() + "], maybe skip receive.");

				try {
					hcConnection.sipContext
							.deploySocket(coreSocketSession.hcConnection, null);
				} catch (final Exception ex) {
				}
				...
				if (overflowException == e) {
					// 请求关闭
					LogManager.log(
							"[ReceiveServer] Unvalid data len, stop application");
		...
		L.V = L.WShop ? false
				: LogManager
						.log("[ReceiveServer] Receiver shutdown " + threadID);
	}
	...
	private final void doFailCheck(final boolean isAddNextXorPackageID)
			throws Exception {
		final long currMS = System.currentTimeMillis();
		...
		if (diffMS < 10000) {// 最近多个包连续错误
			L.V = L.WShop ? false
					: LogManager.log(
							"[ReceiveServer] fail on check integrity of package data in next ten seconds!");
			// 注意：要置于下段之前
		} else if (diffMS < 15000) {
			LogManager.errToLog(
					"[ReceiveServer] fail on check integrity of package data, force close current connection!");
			coreSocketSession.context.doExtBiz(IContext.BIZ_DATA_CHECK_ERROR,
					null);
			...
			L.V = L.WShop ? false
					: LogManager.log("[ReceiveServer] fail check xorPackage : "
							+ readyReceiveXorPackageID
							+ ", send E_RE_TRANS_XOR_PACKAGE!!!");
	...
	public void shutDown() {
		L.V = L.WShop ? false
				: LogManager.log(
						"[ReceiveServer] notify shutdown ReceiveServer threadName : "
								+ threadID);
		isShutdown = true;
		synchronized (LOCK) {
			LOCK.notify();
	...
	void setUdpServerSocket(final Object tcpOrUDPsocket,
			final boolean isCloseOld) {
		...
		if (tcpOrUDPsocket != null) {
			L.V = L.WShop ? false
					: LogManager
							.log("[ReceiveServer] update socket threadName : "
									+ threadID + " setDataInputStream : "
