...
public abstract class ThreadPool {
	...
	public final Object runAndWait(final ReturnableRunnable run,
			final Object threadToken, final boolean isThrowError) {
			...
			public void run() {
				try {
					...
					if (isServerLevelPool) {
						ExceptionReporter.printStackTrace(e, null, null,
		...
		if (nestThrowable != null) {
			if (isThrowError || ExceptionReporter
					.isCauseByLineOffSession(nestThrowable)) {
				final RootBuilder instance = RootBuilder.getInstance();
				final Boolean throwForProjAlso = isThrowError
						? IConstant.BOOL_TRUE
						: IConstant.BOOL_FALSE;
				if (((Boolean) instance.doBiz(
						RootBuilder.ROOT_IS_CURR_THREAD_IN_SESSION_OR_PROJ_POOL,
						throwForProjAlso)).booleanValue()) {
					instance.doBiz(RootBuilder.ROOT_THROW_CAUSE_ERROR,
							nestThrowable);
				}
			}
		}
	...
	public ThreadPool(Object threadGroup, final boolean isServerLevelPool,
			final int poolType) {
	...
	public final RecycleThread run(Runnable run, Object token) {
		...
		// System.out.println("[" + name + "] -> RecycleThead : " +
		// rt.toString());
