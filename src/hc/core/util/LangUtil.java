...
public class LangUtil {
	...
	public static final String LOCALE_SPLIT = String
			.valueOf((char) LOCALE_SPLIT_CHAR);
	...
	public final static String[] oneThreeEquals = { "zh-CN", "zh-CN", "zh-SG",
			"zh-TW" };
	public final static String[] oneTwoEquals = { "zh-SG", "zh-Hans", "zh-Hans",
			"zh-Hant" };
	 ...
	 *            true表示使用可替换lang，比如旧遗留和二码和三码相等
	 * @return
	 */
	public static boolean isSameLang(final String locale,
			final String compareLocale, final boolean isMaybeEqualLang) {
		if (locale.equals(compareLocale)) {
			return true;
		}

		final int lastSplitIdx = locale.lastIndexOf(LOCALE_SPLIT_CHAR);
		final int lastCompareSplitIdx = compareLocale
				.lastIndexOf(LOCALE_SPLIT_CHAR);
		...
		if (lastSplitIdx >= 0) {
			final String[] cmpParts = StringUtil.splitToArray(compareLocale,
					LOCALE_SPLIT);
			...
			if (isMaybeEqualLang) {
				...
				for (int i = 0; i < equalLocale.length; i++) {
					if (part0.equals(equalLocale[i])) {
						return isSameLang(
								buildEqualLocale(parts, equalLocaleTo[i]),
								compareLocale, false);
					}
				}
				for (int i = 0; i < equalLocaleTo.length; i++) {
					if (part0.equals(equalLocaleTo[i])) {
						return isSameLang(
								buildEqualLocale(parts, equalLocale[i]),
								compareLocale, false);
					}
				}
			}
			...
			if (cmpParts.length == 3) {// zh-Hans-CN => zh-CN
				final String oneTwo = cmpParts[0] + LangUtil.LOCALE_SPLIT
						+ cmpParts[1];
				...
				final String oneThree = cmpParts[0] + LangUtil.LOCALE_SPLIT
						+ cmpParts[2];
			...
			if (isMaybeEqualLang) {
				for (int i = 0; i < equalLocale.length; i++) {
					if (locale.equals(equalLocale[i])) {
						return isSameLang(equalLocaleTo[i], compareLocale,
								false);
					}
				}
			...
			if (lastCompareSplitIdx >= 0) {
				final String[] cmpParts = StringUtil.splitToArray(compareLocale,
						LOCALE_SPLIT);
				if (cmpParts.length == 3) {// zh-Hans-CN => zh-CN
					return isSameLang(locale,
							cmpParts[0] + LOCALE_SPLIT + cmpParts[2], false);
				} else if (cmpParts.length == 2) {// zh-CN => zh
	...
	public static String buildEqualLocale(final String[] oldParts,
			final String equalLang) {
