...
public class RecycleRes {
	...
	public RecycleRes(final String name, final ThreadPool pool,
			final HCConditionWatcher sequenceWatcher) {
		this.threadPool = pool;
		if (sequenceWatcher == null) {
			this.sequenceWatcher = new HCConditionWatcher(
					name + "SequenceConditionWatcher",
					ThreadPriorityManager.SEQUENCE_SCRIPT_PRIORITY);
		} else {
	...
	public static HCConditionWatcher getSequenceTempWatcher() {
		if (sequenceTempWatcher == null) {
			sequenceTempWatcher = new HCConditionWatcher(
					"Temp" + "SequenceConditionWatcher",
					ThreadPriorityManager.SEQUENCE_SCRIPT_PRIORITY);
		}
