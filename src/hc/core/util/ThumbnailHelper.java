...
public class ThumbnailHelper {
	 ...
	 * @return
	 */
	public static int[] calcThumbnail(int m_w, int m_h, final int pc_w,
			final int pc_h) {
	...
	public static int[] calNewLocXY(int oldZoomMultiple, int zoomMultiples,
			int locX, int locY, int pcW, int pcH, int mobileW, int mobileH) {
		...
		final int zoomWidth = (zoomMultiples != 1) ? (mobileW / zoomMultiples)
				: mobileW;
		final int zoomHeight = (zoomMultiples != 1) ? (mobileH / zoomMultiples)
				: mobileH;
		...
		// LogManager.log("oldZoomMulti : " + oldZoomMultiple +
		// "currentZoomMulti : " + zoomMultiples + ", oldLocX : " + oldLocX + ",
		// oldLocY : " + oldLocY
		// + ", locX : " + locX + ", locY : " + locY + ", mobileW : " + mobileW
		// + ", mobileH : " + mobileH + ", zoomW : " + zoomWidth + ", zoomH : "
		// + zoomHeight +
		// "maxRightX : " + pcMaxRightX + ", bottomY : " + pcBottomY);

		final int[] back = { locX, locY, zoomWidth, zoomHeight, pcMaxRightX,
				pcBottomY, (zoomWidth * zoomMultiples) == mobileW ? 0 : 1, // 返回1表示，要补一个像素宽
				(zoomHeight * zoomMultiples) == mobileH ? 0 : 1 };// 返回1表示，要补一个像素高
		return back;
	}

	public static final void zoomOutRGB(final int[] zoomInData, final int zw,
			final int zh, final int zoomOut) {
		// System.out.println("zoomOutRGB zw : " + zw + ", zh : " + zh + ", zoom
		// : " + zoomOut);
		...
		for (int y = 0; y < zoomOutHeight; y++) {
			for (int x = 0; x < zoomOutWidth; x++) {
				zoomInData[y * zoomOutWidth + x] = zoomInData[y * zw * zoomOut
						+ x * zoomOut];
			}
		}
	}
