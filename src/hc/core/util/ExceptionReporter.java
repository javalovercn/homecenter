...
public class ExceptionReporter {
	...
	public static final void printStackTraceFromThreadPool(
			final Throwable throwable) {
		printStackTrace(throwable, null, null, INVOKE_THREADPOOL);
	}

	public static final void printStackTraceFromHAR(final Throwable throwable,
			final String script, final String errMessage) {
	...
	public static boolean isCauseByLineOffSession(final Throwable throwable) {
		...
		while (t != null) {
			...
			t = (Throwable) RootBuilder.getInstance()
					.doBiz(RootBuilder.ROOT_GET_CAUSE_ERROR, t);
		}
		return false;
	}
	 ...
	 * @param errMessage
	 * @param invokeFrom
	 *            one of {@link #INVOKE_NORMAL}, {@link #INVOKE_HAR},
	 *            {@link #INVOKE_THREADPOOL}
	 */
	public static final void printStackTrace(final Throwable throwable,
			final String script, final String errMessage,
			final int invokeFrom) {
		...
		if (L.isInWorkshop) {
			RootBuilder.getInstance().doBiz(
					RootBuilder.ROOT_PRINT_STACK_WITH_FULL_CAUSE, throwable);// printStackTrace
																				// or
																				// cause
																				// by
																				// without
		...
		if (SERVER_SIDE == false) {
			...
			// 手机端
			final ExceptionJSON json = builder.buildJSON(harHelper, checker,
					throwable, ExceptionJSON.HC_EXCEPTION_URL, script,
					errMessage);
			if (json == null) {
				return;
			}
			...
			if (exceptionURLOrEmail != null) {
				...
				isEmail = exceptionURLOrEmail != null && atIdx > 0
						&& maoHaoIdx < 0;
			}
		}

		// if(isInvokedFromHAR){//排除不发送的情形
		// if(SERVER_SIDE && receiveHarException == false && exceptionURL ==
		// null){
		// return;
		// }
		// }

		if (isEmail || ((invokeFrom == INVOKE_NORMAL)
				|| (invokeFrom == INVOKE_THREADPOOL
						&& exceptionURLOrEmail == null))) {
		...
		if (exceptionURLOrEmail != null && isEmail == false) {
			// 满足向Har provider的条件
			final ExceptionJSON json = builder.buildJSON(harHelper, checker,
					throwable, exceptionURLOrEmail, script, errMessage);
			if (json == null) {
				return;
			}
