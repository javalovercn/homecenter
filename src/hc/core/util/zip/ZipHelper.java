...
public final class ZipHelper {
	...
	// constants:
	public static final int[] LENGTH_CODE = { 0, 3, 0, 4, 0, 5, 0, 6, 0, 7, 0,
			8, 0, 9, 0, 10, 1, 11, 1, 13, 1, 15, 1, 17, 2, 19, 2, 23, 2, 27, 2,
			31, 3, 35, 3, 43, 3, 51, 3, 59, 4, 67, 4, 83, 4, 99, 4, 115, 5, 131,
			5, 163, 5, 195, 5, 227, 0, 258 };
	public static final int[] DISTANCE_CODE = { 0, 1, 0, 2, 0, 3, 0, 4, 1, 5, 1,
			7, 2, 9, 2, 13, 3, 17, 3, 25, 4, 33, 4, 49, 5, 65, 5, 97, 6, 129, 6,
			193, 7, 257, 7, 385, 8, 513, 8, 769, 9, 1025, 9, 1537, 10, 2049, 10,
			3073, 11, 4097, 11, 6145, 12, 8193, 12, 12289, 13, 16385, 13,
			24577 };
	 ...
	 */
	public static void genHuffTree(int[] huffmanCode,
	...
	public static void revHuffTree(int[] huffmanCode,
			byte[] huffmanCodeLength) {
		// reverse all:
		int tmp;
		int reversed;
	 ...
	 * @param max_len
	 */
	public static void genTreeLength(int[] count, byte[] huffmanCodeLength,
			int max_len) {
		...
		// fix the overflow
		if (overflowCount != 0) {
			// System.out.println(" fixing " + overflowCount + " overflows
			// because of max=" + max_len);
			...
			for (int i = 0; i < huffmanCodeLength.length; i++) {
				if (huffmanCodeLength[i] != 0
						&& huffmanCodeLength[minNode] > huffmanCodeLength[i]) {
					minNode = i;
				}
			}
			...
			while (overflowCount != 0) {
				...
				for (int i = 0; i < huffmanCodeLength.length; i++) {
					if (huffmanCodeLength[i] < max_len
							&& huffmanCodeLength[exendableNode] < huffmanCodeLength[i]) {
						exendableNode = i;
					}
				}
	 ...
	 *            resulting tree
	 */
	public static void convertTable2Tree(int[] huffmanCode,
			byte[] huffmanCodeLength, int[] huffmanData, short[] huffmanTree) {
		...
		for (short i = 0; i < huffmanCode.length; i++) {
			if (huffmanCodeLength[i] != 0) {
				...
				if (pointer < 0) {
					// #debugSystem.out.println("error pointer=-1");
				}
	 ...
	 * @throws IOException
	 */
	public static int deHuffNext(long[] smallCodeBuffer, short[] huffmanTree)
			throws IOException {
		// //#debugSystem.out.println("deHuffNext(long[], short[])");
		// #if polish.debug.error
		if (smallCodeBuffer[1] < 15) {
			// #debugSystem.out.println("smallCodebuffer is too small");
		}
		// #endif
		...
		while (huffmanTree[pointer * 2] != -1) {
			...
			if (pointer == 0) {
				// System.out.println("small code buffer DAta
				// "+smallCodeBuffer[0] + "\n");
	 ...
	 */
	public static int crc32(int[] table, int crc, byte[] buffer, int off,
			int len) {
		if (table[2] == 0) {
