...
public class GZipOutputStream extends OutputStream {
	 ...
	 * @see #TYPE_GZIP
	 */
	public GZipOutputStream(OutputStream outputStream, int size,
			int compressionType, int plainWindowSize, int huffmanWindowSize)
			throws IOException {
		...
		if (plainWindowSize >= 100) {
			this.plainDataWindow = new byte[(plainWindowSize / HASHMAP_COUNT)
					* HASHMAP_COUNT];
	...
	public void close() throws IOException {
		...
		if (this.BTYPE == 2) {
			...
			if ((this.outProcessed + 8 + (this.inEnd - this.inStart) * 8
					/ 3 > this.outputWindow.length)) {
				compileOutput();
			}
	...
	public void write(byte[] b, int off, int len) throws IOException {
		...
		while (processed != len) {
			...
			if (this.inputBuffer.length - this.inEnd >= len - processed) {
				...
				System.arraycopy(b, processed + off, this.inputBuffer,
						this.inEnd, len - processed);
				this.inEnd += len - processed;
				processed = len;
			} else {
				System.arraycopy(b, processed + off, this.inputBuffer,
						this.inEnd, this.inputBuffer.length - this.inEnd);
	...
	private boolean search4LZ77(int[] bestPointer, int position) {
		...
		for (int i = 0; i < HASHMAP_COUNT; i++) {
			...
			found = null;
			found = this.HM[i].get((128 + this.inputBuffer[position]) << 16
					| (128 + this.inputBuffer[position + 1]) << 8
					| (128 + this.inputBuffer[position + 2]));
	 ...
	 */
	private void searchHM4LZ77(ZipIntMultShortHashMap.Element found,
			int[] pointer, int position) {
		int length;
		...
		for (int k = found.size - 1; k >= 0; k--) {
			...
			while (length < 258
					&& position + length < this.inputBuffer.length) {
				if (found.values[k] < this.plainPointer) {
					comparePointer = (found.values[k]
							+ (length % (this.plainPointer - found.values[k])))
							% this.plainDataWindow.length;
				} else {
					comparePointer = (found.values[k] + (length
							% (this.plainPointer + this.plainDataWindow.length
									- found.values[k])))
							% this.plainDataWindow.length;
				}

				// break, if the compared chars differ
				if (this.inputBuffer[position
						+ length] == this.plainDataWindow[comparePointer]) {
					length++;
				} else {
					break;
				}
		...
		// encode the pointer
		pointer[0] = /* distance= */ (this.plainPointer - found.values[bestK]
				+ this.plainDataWindow.length) % this.plainDataWindow.length;
	...
	private void encodePointer(int distance, int length) throws IOException {
		...
		if (this.outputWindow.length != 0) {
			...
			this.outputWindow[this.outProcessed + 3] = (byte) (di);// id of
																	// Table ==
																	// code
			this.outputWindow[this.outProcessed
					+ 4] = (byte) (distExtra & 0xFF);
			this.outputWindow[this.outProcessed
					+ 5] = (byte) (distExtra >> 8 & 0xFF);
			this.outputWindow[this.outProcessed
					+ 6] = (byte) (distExtra >> 16 & 0xFF);
			...
			pushSmallBuffer(this.huffmanCode[litlen],
					this.huffmanCodeLength[litlen]);
			pushSmallBuffer(litextra,
					(byte) ZipHelper.LENGTH_CODE[2 * (litlen - 257)]);
	...
	private void LZ77(boolean finish) throws IOException {
		...
		if (this.inStart != 0) {
			System.arraycopy(this.inputBuffer, this.inStart, this.inputBuffer,
					0, this.inEnd - this.inStart);
		...
		for (i = 0; i < upTo;) {
			...
			if (this.lz77active) {
				for (int k = 0; k < length; k++) {
					this.plainDataWindow[this.plainPointer] = this.inputBuffer[i
							+ k];

					// add the bytes to the hashmap
					this.HM[this.plainPointer
							/ (this.plainDataWindow.length / HASHMAP_COUNT)]
									.put((128 + this.inputBuffer[i + k]) << 16
											| (128 + this.inputBuffer[i + k
													+ 1]) << 8
											| (128 + this.inputBuffer[i + k
													+ 2]),
											(short) this.plainPointer);
	...
	private void newBlock() throws IOException {
		...
		if (this.lastBlock) {
			...
			// #debugSystem.out.println("final block");
		} else {
		...
		if (this.BTYPE == 1) {
			ZipHelper.genFixedTree(this.huffmanCode, this.huffmanCodeLength,
					this.distHuffCode, this.distHuffCodeLength);
			...
			ZipHelper.genTreeLength(this.distCount, this.distHuffCodeLength,
					15);
	...
	private void compileOutput() throws IOException {
		// #debugSystem.out.println(" compile Output; new Block");
		...
		for (int i = 0; i < this.outProcessed; i++) {
			...
			if (val != 255) {
				pushSmallBuffer(this.huffmanCode[val],
						this.huffmanCodeLength[val]);
			} else {
				if (val == 255) {
					i++;
					if (this.outputWindow[i] == 0) {
						// 255 char
						pushSmallBuffer(this.huffmanCode[255],
								this.huffmanCodeLength[255]);
						...
						i++;
						distExtra = ((this.outputWindow[i] + 256) & 255)
								| ((this.outputWindow[i + 1] + 256) & 255) << 8
								| ((this.outputWindow[i + 2] + 256)
										& 255) << 16;
						i += 3;
						...
						pushSmallBuffer(this.huffmanCode[litlen],
								this.huffmanCodeLength[litlen]);
						pushSmallBuffer(litextra, (byte) ZipHelper.LENGTH_CODE[2
								* (litlen - 257)]);

						// write distance code + extra info
						pushSmallBuffer(this.distHuffCode[di],
								this.distHuffCodeLength[di]);
						pushSmallBuffer(distExtra,
								(byte) ZipHelper.DISTANCE_CODE[di * 2]);
	...
	private void writeFooter() throws IOException {
		// write current 256
		pushSmallBuffer(this.huffmanCode[256], this.huffmanCodeLength[256]);
		// #debugSystem.out.println(" wrote final 256;");
		...
		// #debugSystem.out.println(" output finished");
	}
	 ...
	 * @throws IOException
	 */
	private void compressTree(byte[] huffmanCodeLength,
			byte[] distHuffCodeLength) throws IOException {
		...
		int[] miniHuffData = { 16, 17, 18, 0, 8, 7, 9, 6, 10, 5, 11, 4, 12, 3,
				13, 2, 14, 1, 15 };
		...
		for (int i = 0; i < len.length; i++) {
			...
			if (i + 3 < len.length && len[i] == len[i + 1]
					&& len[i] == len[i + 2] && len[i] == len[i + 3]) {
				if (len[i] == 0) {
					...
					while (i + k < len.length && len[i] == len[i + k]
							&& k < 139) {
						k++;
					}
		...
		for (i = 0; i < outCount; i++) {
			...
			pushSmallBuffer(miniHuffCode[outLitLenDist[i]],
					miniHuffCodeLength[outLitLenDist[i]]);
