...
/**
 * <p>
 * Provides the functionality of the J2SE java.util.HashMap for J2ME
 * applications and uses reference checks for comparing keys.
 * </p>
 * <p>
 * WARNING: Only use this implementation when you can ensure that you always use
 * the original keys! (see below)
 * </p>
 * <p>
 * In contrast to the java.util.Hashtable (which is available on J2ME
 * platforms), this implementation is not synchronized and faster. This
 * implementation also uses only reference checks (==) for testing keys and
 * values on equality instead of calling equals() for comparing them. This is
 * considerably faster, but you need to ensure that you only use references
 * (=the original) keys when storing or retrieving values.
 * </p>
 * <p>
 * This implementation uses chains for resolving collisions, that means when a
 * key-value pair has the same hash code as a previous inserted item, the new
 * item is linked to the previous item. Depending on your situation The
 * OpenAddressingHashMap implementation might be better, especially when you do
 * not have many collisions (items with the same hash code).
 * </p>
 *
 * <p>
...
public class ZipIntMultShortHashMap {
	 ...
	 */
	/*
	 * public ZipIntMultShortHashMap() { this( DEFAULT_INITIAL_CAPACITY,
	 * DEFAULT_LOAD_FACTOR ); }
	 */
			...
			do {
				if (element.key == key) {
					...
					if (element.size == element.values.length) {
						...
						short[] newValues = new short[element.values.length
								* 2];// TODO constant
						System.arraycopy(element.values, 0, newValues, 0,
	 ...
	 */
	/*
	 * public boolean containsValue( short value ) { for (int i = 0; i <
	 * this.buckets.length; i++) { Element element = this.buckets[i]; while
	 * (element != null) { if (element.values == value ) { return true; }
	 ...
	 */
	/*
	 * public short[] values() { return values( new short[ this.size ] ); }
	 */
	 ...
	 */
	/*
	 * public short[] values(short[] objects) { int index = 0; for (int i = 0; i
	 * < this.buckets.length; i++) { Element element = this.buckets[i]; while
	 * (element != null) { objects[index] = element.values; index++; element =
	 * element.next; } } return objects; }
	 */
	/*
	 * (non-Javadoc)
	 * 
	 ...
	 * short[] values = values(); for (int i = 0; i < values.length; i++) {
	 * buffer.append( values[i] ); buffer.append('\n'); } buffer.append('}');
