...
public class CtrlMap {
	...
	public void updateButtonOnCanvas(final int keyValue, final int center_x,
			final int center_y) {
		map.put(HEAD_CENTER_X + keyValue, String.valueOf(center_x));
		map.put(HEAD_CENTER_Y + keyValue, String.valueOf(center_y));
		// LogManager.log("updateButtonOnCanvas x : " + center_x + ", y : " +
		// center_y);
	}

	public final void addButtonOnCanvas(final int keyValue, final int center_x,
			final int center_y) {
		synchronized (this) {
		...
		// LogManager.log("addButtonOnCanvas size : " + size + ", x : " +
		// center_x + ", y : " + center_y);
	}
				...
				while (it.hasMoreElements()) {
					...
					if (key.startsWith(HEAD_KEY)) {
						int keyValue = Integer
								.parseInt(key.substring(START_IDX_HEAD_KEY));
	...
	public final void setPNGBase64(final int keyValue,
			final String pngBase64Data) {
