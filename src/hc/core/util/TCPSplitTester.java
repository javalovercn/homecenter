...
public class TCPSplitTester {
	...
	public static void printBigData(final byte[] bs) {
		...
		ByteUtil.encodeFileXOR(bs, MsgBuilder.INDEX_MSG_DATA, len, code);
		LogManager.log("----[Big Msg]-----data len : " + len
				+ ", code base64 : " + ByteUtil.encodeBase64(code));
	}

	static int i = 0;

	public static void sendBigTest(final FastSender fastSender,
			final int lastSendLen) {
		...
		ByteUtil.encodeFileXOR(testData, 0, len, code);
		LogManager.log("----[Big Msg]-----data len : " + len
				+ ", code base64 : " + ByteUtil.encodeBase64(code));
		fastSender.sendWrapAction(MsgBuilder.E_BIG_MSG_JS_TO_MOBILE, testData,
				0, len);
