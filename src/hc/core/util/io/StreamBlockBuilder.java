...
public class StreamBlockBuilder {
	...
	public static final byte[] BS_HEADER_FILE_EXTSION = HEADER_FILE_EXTSION.getBytes();
	public static final byte[] BS_HEADER_FILE_NAME = HEADER_FILE_NAME.getBytes();
	public static final byte[] BS_HEADER_AUDIO_FORMAT = HEADER_AUDIO_FORMAT.getBytes();
	public static final byte[] BS_HEADER_IMAGE_FORMAT = HEADER_IMAGE_FORMAT.getBytes();
	public static final byte[] BS_HEADER_IMAGE_AZIMUTH = HEADER_IMAGE_AZIMUTH.getBytes();
	public static final byte[] BS_HEADER_IMAGE_PITCH = HEADER_IMAGE_PITCH.getBytes();
	public static final byte[] BS_HEADER_IMAGE_ROLL = HEADER_IMAGE_ROLL.getBytes();
	public static final byte[] BS_HEADER_IMAGE_LATITUDE = HEADER_IMAGE_LATITUDE.getBytes();
	public static final byte[] BS_HEADER_IMAGE_LONGITUDE = HEADER_IMAGE_LONGITUDE.getBytes();
	public static final byte[] BS_HEADER_IMAGE_ALTITUDE = HEADER_IMAGE_ALTITUDE.getBytes();
	
	private static final byte[] BS_HEADER_CLOSE = HEADER_TAG_CLOSE.getBytes();
	public static final byte[] BS_STREAM_CONTENT = HEADER_TAG_STREAM_CONTENT.getBytes();
	
	private static final byte[] BS_HEADER_SPLITTER = HEADER_SPLITTER.getBytes();
	
	private static final String[] HEAD_TAGS = {HEADER_TAG_CLOSE, HEADER_TAG_STREAM_CONTENT};
	private static final byte[][] HEAD_TAGS_BS = {BS_HEADER_CLOSE, BS_STREAM_CONTENT};
	 ...
	 */
	public static void addDataBlock(final HCOutputStream osSnap, final IOBuffer iobuffer, final IOBuffer iodata, 
	...
	public static void addHeaderKeyBlock(final IOBuffer iobuffer, final byte[] keyBS, final byte[] valueBS, final byte[] lengthBS) throws IOException{
		final int len = keyBS.length + BS_HEADER_SPLITTER.length + valueBS.length;
		ByteUtil.integerToFourBytes(len, lengthBS, 0);
	...
	public static void addCloseHeader(final IOBuffer iobuffer, final byte[] lengthBS) throws IOException{
	 ...
	 * @return
	 */
	public static int readBlockLength(final HCInputStream is, final byte[] lengthBS) throws IOException{
	 ...
	 * @return
	 */
	public static String readHeaderBlock(final HCInputStream is, final byte[] lengthBS) throws IOException{
