...
public class StreamBuilder {
	...
	private static final IOException releaseStreamException = new IOException(
			"System Line Off Exception");
	...
	public final void notifyExceptionForReleaseStreamResources() {
		...
		L.V = L.WShop ? false
				: LogManager
						.log("done notifyExceptionForReleaseStreamResources.");
	}
			...
			if (hashtable.size() > 0) {
				...
				try {
					while (e.hasMoreElements()) {
						final IHCStream stream = (IHCStream) hashtable
								.get(e.nextElement());
	...
	public final HCInputStream buildInputStream(final String className,
			final byte[] parameter, final int offset, final int len,
		...
		if (isInitial) {
			manageRemoteStream(false, streamID, className, parameter, offset,
					len);
		}
		return is;
	}

	public final OutputStream buildOutputStream(final String className,
			final byte[] parameter, final int offset, final int len,
			final boolean isInitial) {
		if (coreSS.isExchangeStatus() == false) {
			throw new IllegalStateException("session is line-off!");
		...
		if (isInitial) {
			manageRemoteStream(true, streamID, className, parameter, offset,
					len);
		}
		return is;
	}
	...
	public final void manageRemoteStream(final boolean isInputStream,
			final int streamID, final String className, final byte[] parameter,
			final int offset, final int len) {
		final byte[] classNameBS = ByteUtil.getBytes(className,
				IConstant.UTF_8);
	...
	public final Object closeStream(final boolean isInputStream,
			final int streamid) {
		final Hashtable streamTable = isInputStream ? inputStreamTable
				: outputStreamTable;
	...
	public final void notifyCloseRemoteStream(final boolean isInputStream,
			final int streamID) {
		manageRemoteStream(isInputStream, streamID, TAG_CLOSE_STREAM, EMPTY_BS,
				0, 0);
	}
