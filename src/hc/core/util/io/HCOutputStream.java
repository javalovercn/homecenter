...
public class HCOutputStream extends OutputStream implements IHCStream {
	...
	public final void notifyClose() {
		...
		L.V = L.WShop ? false
				: LogManager.log("notify close HCOutStream : " + streamID);
	}
	...
	final void write_0(final byte[] b, int offset, final int len)
			throws IOException {
		synchronized (this) {
			if (exception != null) {
				throw exception;
			...
			while (offset < endIdx) {
				...
				sendBlockLen = (sendBlockLen > MAX_BLOCK_LEN) ? MAX_BLOCK_LEN
						: sendBlockLen;

				{
					final int flushLen = sendBlockLen
							+ StreamBuilder.STREAM_ID_LEN;
					final byte[] flushBS = cacher.getFree(flushLen);
					ByteUtil.integerToFourBytes(streamID, flushBS, 0);
					System.arraycopy(b, offset, flushBS,
							StreamBuilder.STREAM_ID_LEN, sendBlockLen);
					fastSender.sendWrapAction(MsgBuilder.E_STREAM_DATA, flushBS,
							0, flushLen);
				}
