...
public class ExceptionJSON {
	...
	private static final char[] CS_YINHAO_HTML_REPLACE_TO = YINHAO_HTML_REPLACE_TO
			.toCharArray();
	...
	public static final String HC_EXCEPTION_URL = RootServerConnector.AJAX_HTTPS_44X_URL_PREFIX
			+ "exception.php";
	...
	/**
	 * if HomeCenter will NOT receive exception, only send email, this tag is
	 * used.
	 * 
	 * @param receiveHar
	 */
	public final void setReceiveExceptionForHC(final boolean receiveHar) {
		set(ExceptionJSON.tag_isReceiveException,
				receiveHar ? IConstant.TRUE : IConstant.FALSE);
	}
	...
	static final char[] searchBytes = { '"', '<', '>', '\\', '\n', '\t', '\b',
			'\f' };
	static final char[][] replaceToBytes = { CS_YINHAO_HTML_REPLACE_TO,
			LT.toCharArray(), GT.toCharArray(), "&#92;".toCharArray(), CS_BR,
			FOUR_EMPTY_SPACE_HTML.toCharArray(), __B.toCharArray(),
			__F.toCharArray() };
	...
	public final void initData(final String threadName, final String className,
			final String message, final String stackTrace,
			final String sysProperties, final String harID,
			final String harVersion, final String jreVersion,
			final String hcVersion, final String jrubyVersion,
			final String script, final String scriptErrMessage) {
	...
	public static String replaceForTail(String value) {
		...
		if (value.indexOf(doubleHao) >= 0) {
			value = StringUtil.replace(value, doubleHao,
					YINHAO_HTML_REPLACE_TO);
		}
	...
	public final String toEmailHtml() {
		...
		// 表头
		sb.append(
				"<tr style=\"white-space:nowrap; background: #e6e6e6;\"><td><p style=\"text-align: center\" align=\"center\">item</p></td><td><p style=\"text-align: center\" align=\"center\">value</p></td></tr>");
		// 表行
		final int size = table.size();
		for (int i = 0; i < size; i++) {
			final KeyValue kv = (KeyValue) table.elementAt(i);
			sb.append(
					"<tr style=\"white-space:nowrap;\"><td><p style=\"text-align: center\" align=\"center\">");
			sb.append(kv.key);
			sb.append("</p></td><td>");
			sb.append(kv.value);
			sb.append("</td></tr>");
