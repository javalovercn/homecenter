...
public abstract class WiFiDeviceManager {
	 ...
	 * @return
	 */
	public abstract void startWiFiAP(String ssid, String pwd, String securityOption) throws Exception;
	
	public abstract void clearWiFiAccountGroup(String cmdGroup);
	
	public abstract void broadcastWiFiAccountAsSSID(final String[] commands, final String cmdGroup);
	
	public abstract OutputStream createWiFiMulticastStream(final String multicastIP, final int port);
	
	public abstract boolean hasWiFiModule();
	
	/**
	 * 服务器端可能存在有WiFi模块，但不能创建AP；
	 * 客户端可能存在有WiFi模块，但不能创建AP
	 * @return
	 */
	...
	public abstract InputStream listenFromWiFiMulticast(final String multicastIP, final int port);
	
	/**
	 * 返回J2SE中WiFiAccount中的[UUID, Password, ConfigManager.WIFI_SECURITY_OPTION_NO_PASSWORD|WIFI_SECURITY_OPTION_WEP|WIFI_SECURITY_OPTION_WPA_WPA2_PSK]
	 * @return
	 */
