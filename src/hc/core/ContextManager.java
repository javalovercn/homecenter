...
public class ContextManager {
	...
	public static void setContextInstance(final IContext ic) {
		...
		// IConstant.serverSide){//手机端初次连接失败后，再次连接会重新调用本方法
		// if(L.isInWorkshop){
			...
			public void run() {
				try {
					if (ic != null) {
						...
						if (L.isInWorkshop) {
							LogManager.errToLog(
									"notifyShutdown IContext is null");
						}
	...
	// public static void displayMessage(final String caption, final String
	// text, final int type, final int timeOut){
	// final CoreSocketSession coreSS =
	// SessionManager.getPreparedSocketSession();
	...
	public static boolean isServerStatus(final IContext ic) {
		return ic != null
				&& ic.getStatus() == ContextManager.STATUS_SERVER_SELF;
	}

	public static boolean isClientStatus(final IContext ic) {
		if (ic != null && ic.getStatus() == ContextManager.STATUS_CLIENT_SELF) {
			return true;
		}
		return false;
	}
	...
	public static boolean isClientExit(final IContext ic) {
		if (ic != null && ic.getStatus() == ContextManager.STATUS_EXIT) {
			return true;
		}
		return false;
	}
	...
	public static final short STATUS_CLIENT_IN = 3;// 服务器端，client可能直联、WiFi或中继进入
	public static final short STATUS_READY_MTU = 4;// 打通，但是尚未进行MTU
	...
	final static int[] notWorking = { STATUS_READY_TO_LINE_ON,
			STATUS_READY_EXIT, STATUS_EXIT, STATUS_NEED_NAT, STATUS_LINEOFF };

	final static int[] readyServOrSering = { STATUS_CLIENT_IN, STATUS_READY_MTU,
			STATUS_SERVER_SELF };

	public static int[] getReadyServOrServingStatus() {
		CCoreUtil.checkAccess();
		return readyServOrSering;
	}
	...
	public static byte[] cloneDatagram(final byte[] randomBS) {
		...
		System.arraycopy(randomBS, 0, event, 0,
				(event.length > randomBS.length) ? randomBS.length
						: event.length);
		return event;
	}
	...
	public static final void setThreadPool(final ThreadPool tp,
			final Object t) {
		CCoreUtil.checkAccess();
