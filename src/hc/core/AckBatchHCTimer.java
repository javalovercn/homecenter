...
public class AckBatchHCTimer extends HCTimer {
	...
	final int batchByteLen = (MsgBuilder.UDP_INTERNET_MIN_MSS - 40) / msgIDLen
			* msgIDLen;

	public AckBatchHCTimer(String name, int ms, boolean enable,
			UDPPacketResender resend) {
	...
	public final void doBiz() {
		...
		while (storeIdx > batchByteLen) {
			...
			resender.sendUDP(MsgBuilder.E_TAG_ACK, MsgBuilder.NULL_CTRL_SUB_TAG,
					ackIDS, storeIdx, batchByteLen, 0, true);
		}
		if (storeIdx != 0) {
			resender.sendUDP(MsgBuilder.E_TAG_ACK, MsgBuilder.NULL_CTRL_SUB_TAG,
					ackIDS, 0, storeIdx, 0, true);
			storeIdx = 0;
		}
		setEnable(false);
		// }
	...
	public void ack(final byte[] bs, int startIdx) {
		...
		for (final int endStoreIdx = storeIdx
				+ msgIDLen; storeIdx < endStoreIdx;) {
