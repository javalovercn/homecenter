...
public class EnumNAT {
	...
	/**
	 * 当内网主机创建一个UDP socket并通过它第一次向外发送UDP数据包时， NAT会为之分配一个固定的公网{IP:端口}。
	 ...
	 * 即是说，NAT维护了一个内网{IP:端口}到公网{IP:端口}的映射，还维护了一个{外部主机IP,
	 ...
	 * 即是说，NAT维护了一个从内网{IP:端口}到公网{IP:端口}的映射， 还维护了一个从{外部主机{IP:端口},
	 * 公网{IP:端口}}到内网{IP:端口}的映射。
	 */
	...
	public static String getNatDesc(int natType) {
		if (natType == OPEN_INTERNET)
			return "Net status : Open access to the Internet";
		if (natType == UDP_BLOCKED)
			return "Net status : Firewall blocks UDP";
		if (natType == FULL_CONE_NAT)
			return ("Net status : Full Cone NAT handles connections");
		if (natType == RESTRICTED_CONE_NAT)
			return ("Net status : Restricted Cone NAT handles connections");
		if (natType == PORT_RESTRICTED_CONE_NAT)
			return ("Net status : Port restricted Cone NAT handles connections");
		if (natType == SYMMETRICT_NAT)
			return ("Net status : Symmetric Cone NAT handles connections");
		if (natType == SYMMETRIC_FIREWALL)
			return ("Net status : Symmetric UDP Firewall handles connections");
