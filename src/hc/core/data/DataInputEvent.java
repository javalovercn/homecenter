...
public class DataInputEvent extends HCData {
	...
	public static final int MAX_MOBI_UI_TXT_LEN = MsgBuilder.UDP_BYTE_SIZE
			- screen_id_index - 2 - MAX_SCREEN_ID_LEN;

	public int getLength() {
		final int out = screen_id_index - type_index + getScreenIDLen() + 2
				+ getTextDataLen();
		// if(L.isInWorkshop){
	...
	public String getTextDataAsString() {
		try {
			return new String(bs, getTextIdx(), getTextDataLen(),
					IConstant.UTF_8);
	...
	public String getScreenIDAsString() {
		try {
			return new String(bs, screen_id_index, getScreenIDLen(),
					IConstant.UTF_8);
		} catch (UnsupportedEncodingException e) {
			ExceptionReporter.printStackTrace(e);
		}
