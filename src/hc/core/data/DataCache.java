...
public class DataCache extends HCData {
	...
	public final int setCacheInfo(final byte[] projID, final int projIdx,
			final int projLen, final byte[] urlID, final int urlIdx,
			final int urlLen, final byte[] codeID) {
		final int codeIdx = 0;
		...
		System.arraycopy(codeID, codeIdx, bs, getCodeIndex(codeLenIndex),
				codeLen);
	...
	public int getLength() {
		...
		final int eleIndex = getEleIndex(eleLenIndex);
		final int codeLenIndex = getCodeLenIndex(eleIndex,
				getEleLength(eleLenIndex));
		return getCodeIndex(codeLenIndex) + getCodeLength(codeLenIndex)
