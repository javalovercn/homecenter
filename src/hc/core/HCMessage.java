...
public class HCMessage {
	 ...
	 * @return
	 */
	public final static int setSplitPara(final byte[] bs, int groupid,
			final int splitNo, final int splitNum) {
		bs[MsgBuilder.INDEX_PACKET_SPLIT_NO_HIGH] = (byte) ((splitNo >>> 8)
				& 0xFF);
		bs[MsgBuilder.INDEX_PACKET_SPLIT_NO_LOW] = (byte) (splitNo & 0xFF);

		bs[MsgBuilder.INDEX_PACKET_SPLIT_NUM_HIGH] = (byte) ((splitNum >>> 8)
				& 0xFF);
	 ...
	 * @param len
	 */
	public final static void setMsgBody(final byte[] bs, final int storeIdx,
			final byte[] src, final int offset, final int len) {
		setMsgLen(bs, len);
		System.arraycopy(src, offset, bs, storeIdx, len);
	}

	public final static void setMsgTcpSplitCtrlData(final byte[] bs,
			short storeIdx, final byte ctrlTag, final byte subTag,
			final int group_id, final int package_no) {
		bs[storeIdx] = ctrlTag;
