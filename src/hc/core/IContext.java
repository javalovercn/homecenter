...
public abstract class IContext {
	...
	public final void setStatus(final short newCmStatus) {
		synchronized (lock) {
			...
			if ((newCmStatus != ContextManager.STATUS_EXIT)
					&& (oldCmStatus == ContextManager.STATUS_EXIT)) {
				LogManager.log("forbid change status from [" + oldCmStatus
						+ "] to [" + newCmStatus + "]");
				return;
			}

			L.V = L.WShop ? false
					: LogManager.log("Change Status, From [" + oldCmStatus
							+ "] to [" + newCmStatus + "]");
	...
	public IContext(final CoreSession coreSocketSession,
			final EventCenter eventCenter) {
		rootTagListener = new RootTagEventHCListener(
		...
		eventCenter.addListener(new IEventHCListener() {
			...
			public final boolean action(byte[] bs, final CoreSession coreSS,
				...
				final boolean isRight = UDPPacketResender.checkUDPBlockData(bs,
						MsgBuilder.UDP_MTU_DATA_MIN_SIZE);
				if (isRight && (hcConnection.isDoneUDPChannelCheck == false)) {
					LogManager.log(
							"Done UDP Channel Check by E_TAG_ROOT_UDP_ADDR_REG");
	...
	public final void send(final byte event_type, final String body) {
		L.V = L.WShop ? false
				: LogManager.log(
						"send [" + event_type + "], message body : " + body);
	...
	public final void sendWrap(final byte ctrlTag, final byte[] jcip_bs,
			final int offset, final int len) {
		synchronized (sendLock) {
			if (connectionRebuilder.isEnterBuildWaitNewConnection) {
				connectionRebuilder.waitBeforeSend();
			}
			hcConnection.sendWrapActionImpl(ctrlTag, jcip_bs, offset, len,
					cmStatus);// 注意：与下段重建连接重发，同步
		}
	}

	public final void send(final byte ctrlTag, byte[] bsModi,
			final int data_len) {
		synchronized (sendLock) {
			if (connectionRebuilder.isEnterBuildWaitNewConnection) {
				connectionRebuilder.waitBeforeSend();
	...
	public final void send(OutputStream os, final byte ctrlTag,
			final byte subTag) {
		synchronized (sendLock) {
			if (connectionRebuilder.isEnterBuildWaitNewConnection) {
				connectionRebuilder.waitBeforeSend();
	...
	public final void sendWrapWithoutLockForKeepAliveOnly(final byte ctrlTag,
	...
	public final void sendWithoutLockForKeepAliveOnly(OutputStream os,
			final byte ctrlTag, final byte subTag) {// 因为可能在传送大数据时，keepAlive
		...
		public final void run() {
			...
			while (true) {
				synchronized (this) {
					...
					if (bs != null) {
						cmdLen = ((Integer) sendBSLenBuffer.getFirst())
								.intValue();
	...
	public final void sendMobileUIEventToBackServer(final byte[] cmdBS,
			final int offset, final int cmdLen, final byte[] screenIDBS) {
	...
	public final void sendMobileUIEventToServer(final byte[] cmdBS,
			final int offset, final int cmdLen, final byte[] screenIDBS) {
		...
		synchronized (sendBSBuffer) {
			...
			System.arraycopy(screenIDBS, 0, toServerBS,
					toServerScreenIDStoreIdx, idLen);

			final int cmdStoreIdx = toServerScreenIDStoreIdx + idLen;
			System.arraycopy(cmdBS, offset, toServerBS, cmdStoreIdx, cmdLen);

			sendWrap(MsgBuilder.E_JS_EVENT_TO_SERVER, toServerBS, 0,
					cmdStoreIdx + cmdLen);
		}
	}
	...
	public abstract void displayMessage(boolean isFromServerAlertMsg,
			String caption, String text, int type, Object imageData,
			int timeOut);
	...
	public abstract void notifyStreamReceiverBuilder(
			final boolean isInputStream, final String className,
			final int streamID, final byte[] bs, final int offset,
			final int len);

	public final FastSender getFastSender() {
		final IContext ictx = this;
		return new FastSender() {
			public final void sendWrapAction(final byte ctrlTag,
					final byte[] jcip_bs, final int offset, final int len) {
