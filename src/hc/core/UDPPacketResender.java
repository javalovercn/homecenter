...
public abstract class UDPPacketResender {
	...
	public abstract void sendUDP(byte p_ctrlTag, byte subCtrlTag,
			byte[] jcip_bs, int offset, int len, int mtuLen,
			boolean isFlushNow);
	...
	public final void ackAtSend(final byte[] bs, short startIdx,
			final int endIdx) {
	...
	/**
	 * 返回0表示第一次确认，并由方法外进行确认消息发送； 返回1表示以前确认过，并由方法外进行确认消息发送，无需后续逻辑处理，并在方法外进行回收操作
		...
		if (ackReceiveMaxMsgID == msgid) {
							...
							if (llwaitEnd == llwaitStart) {
								...
								if (preTop == null) {
									...
									if (ackTop == null) {
										...
										// msgID:33, ackReceiveMaxMsgID:48,
										// ackReceiveMinMsgID:32
										// 33-33, 35-35, 40-40, 42-42, 45-45,
										// msgID:35, ackReceiveMaxMsgID:48,
										// ackReceiveMinMsgID:32
										ackReceiveMinMsgID = tmpTop.waitStart
												- 1;
									}
								} else {
									preTop.next = tmpTop;
			...
			if (ackTop == null) {
				...
				while (ackTop.waitEnd < newMinMsgID) {
					// msgID:4746, ackReceiveMaxMsgID:4746,
					// ackReceiveMinMsgID:1339
	...
	public static void sendUDPREG(final HCConnection hcConnection) {
		sendUDPBlockData(hcConnection, MsgBuilder.E_TAG_ROOT_UDP_ADDR_REG,
				MsgBuilder.UDP_MTU_DATA_MIN_SIZE);
	}

	public static boolean checkUDPBlockData(final byte[] bs,
			final int dataLen) {
	...
	public static void sendUDPBlockData(final HCConnection hcConnection,
			final byte ctrlTag, final int dataLen) {
		...
		hcConnection.udpSender.sendUDP(ctrlTag, MsgBuilder.NULL_CTRL_SUB_TAG,
				byte1472, 0, dataLen, dataLen, true);
		ByteUtil.byteArrayCacher.cycle(byte1472);
	}

	private final int p_PacketLossDeepMsgIDNum = Integer.parseInt(RootConfig
			.getInstance().getProperty(RootConfig.p_PacketLossDeepMsgIDNum));
	private final int resend_ms = Integer.parseInt(RootConfig.getInstance()
			.getProperty(RootConfig.p_Packet_Resend_MS));

	public final DatagramPacketCacher cacher = DatagramPacketCacher
			.getInstance();

	private int getPacketResendMaxTimes() {
		final int time = Integer.parseInt(RootConfig.getInstance()
				.getProperty(RootConfig.p_PacketResendMaxTimes));
		return IConstant.serverSide ? time : time / 4;
	}

	public final HCTimer resenderTimer = new HCTimer("Resender",
			HCTimer.HC_INTERNAL_MS, false) {
		final int packet_resend_expired_ms = Integer
				.parseInt(RootConfig.getInstance()
						.getProperty(RootConfig.p_Packet_Resend_Expired_MS));

		// final int p_PacketResendMaxTimes =
		// Integer.parseInt(RootConfig.getInstance().
		// getProperty(RootConfig.p_PacketResendMaxTimes));
		...
		public void doBiz() {
			...
			synchronized (needACK) {
				if (size > 0) {
					for (; sendCount < NUM_EACH_SEND && count < size
							&& thisStartIdx < NEED_MAX_SIZE; thisStartIdx++) {
						if (packetMsgID[thisStartIdx] != -1) {
							if (expMS < storeMS[thisStartIdx]) {
								count++;
								if ((resendMSArr[thisStartIdx]) <= lastMS) {
									// LogManager.log("Resend msgid:" +
									// packetMsgID[thisStartIdx] + ", idx:" +
									// thisStartIdx);
								...
								// LogManager.log("throw overtime msgid:" +
								// packetMsgID[thisStartIdx]);
					...
					if (count < size && thisStartIdx < NEED_MAX_SIZE) {
						...
						lastStartIdx = 0;
						// LogManager.log("=======================To Zero UDP
						// Send. size:" + size);
					}
				} else {
					setEnable(false);
					// LogManager.log("===========No data, Disable
					// UDPResender.");
				}
			}
		}
