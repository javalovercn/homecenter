...
public abstract class CoreSession {
	...
	public final void dispatchStream(final byte[] bs) {
		...
		final int streamID = (int) ByteUtil.fourBytesToLong(bs,
		...
		if (is != null) {
			((HCInputStream) is).appendStream(bs,
					MsgBuilder.INDEX_MSG_DATA + StreamBuilder.STREAM_ID_LEN,
					dataLen);
		}
	}
	...
	public final void manageStream(final byte[] bs) {
		...
		offsetidx += 1;
		final String className = ByteUtil.buildString(bs, offsetidx,
				classNameLen, IConstant.UTF_8);
		if (className.equals(StreamBuilder.TAG_CLOSE_STREAM)) {
			final Object stream = streamBuilder.closeStream(isInputStream,
					streamID);
		...
		context.notifyStreamReceiverBuilder(isInputStream, className, streamID,
				paraBS, 0, paraBSLen);
	...
	public final void notifyLineOff(final boolean isClientRequest,
		...
		// coreSS.context.doExtBiz(IContext.BIZ_START_WATCH_KEEPALIVE_FOR_RECALL_LINEOFF,
		// null);
		// }
		// return;
		// }
		...
		RootBuilder.getInstance().doBiz(RootBuilder.ROOT_RELEASE_EXT_J2SE,
				this);
		startLineOffForce(isClientRequest);
	}

	public final void startLineOffForce(final boolean isClientRequest) {
		RootServerConnector.notifyLineOffType(this,
				RootServerConnector.LOFF_LineEx_STR);
	...
	public final Object buildSocket(int localPort, String targetServer,
			int targetPort) {
		return hcConnection.sipContext.buildSocket(localPort, targetServer,
				targetPort);
	}

	public final DataOutputStream getOutputStream(Object socket)
			throws IOException {
	...
	public final DataInputStream swapSocket(final HCConnection keepConn1,
			final HCConnection dropConn2, final boolean isShutdownReceive) {
	...
	public final boolean isContextStatus(final short status) {
		final IContext snap = context;
		return snap != null && snap.cmStatus == status;
	}
	...
	public void setOneTimeCertKey(final byte[] bs) {
		// LogManager.log("successful set OneTimeCertKey : " +
		// ByteUtil.toHex(bs));
