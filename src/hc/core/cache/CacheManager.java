...
public class CacheManager {
	...
	private static int getMinCacheSizeFromRC() {
		final int newMin = RootConfig.getInstance()
				.getIntProperty(RootConfig.p_CacheMinSize);
		if (newMin < MIN_SIZE) {
			return MIN_SIZE;
		} else {
			return newMin;
	...
	public static final String ELE_PROJ_ID_HTML_PROJ = CCoreUtil.SYS_PREFIX
			+ "HTMLPRO";
	public static final byte[] ELE_PROJ_ID_HTML_PROJ_BS = ELE_PROJ_ID_HTML_PROJ
			.getBytes();

	public static final String ELE_URL_ID_HTML_BODY = CCoreUtil.SYS_PREFIX
			+ "HTML_BODY";// 竖屏
	public static final byte[] ELE_URL_ID_HTML_BODY_BS = ELE_URL_ID_HTML_BODY
			.getBytes();
	...
	public static final int CACHE_CODE_LEN = CACHE_CODE_XOR_LEN
			+ CACHE_CODE_DATA_LEN;
	...
	public static void encodeStoreRespAtMobileSide(final CoreSession coreSS,
			final byte[] screenIDBS, final int screenIDIdx,
			...
			final int scriptLen, final String projID, final String urlID,
			final String uid) {
		if (isMeetCacheLength(scriptLen) == false) {
			LogManager.log("cancle cache for [" + projID + "/" + uid + "/"
					+ urlID + "] for length lower than min length.");
			return;
		}
		...
		if (coreSS.mobileUidBSForCache == null) {
			coreSS.mobileUidBSForCache = ByteUtil.getBytes(uid,
					IConstant.UTF_8);
		}

		LogManager.log(
				"save a cache for [" + projID + "/" + uid + "/" + urlID + "]");

		ByteUtil.encodeFileXOR(scriptBS, scriptIndex, scriptLen,
				codeBSforMobileSave);

		CacheManager.storeCache(projID, uid, urlID, projIdBS, projIdIdx,
				projIdLen, coreSS.mobileUidBSForCache, 0,
				coreSS.mobileUidBSForCache.length, screenIDBS, screenIDIdx,
				screentIDLen, codeBSforMobileSave, 0, codeBSforMobileSaveLength,
				scriptBS, scriptIndex, scriptLen, false);

		coreSS.context.sendWrap(MsgBuilder.E_RESP_CACHE_OK, codeBSforMobileSave,
				0, codeBSforMobileSaveLength);
		// LogManager.log("send cache responce. [" +
		// ByteUtil.toHex(codeBSforMobileSave) + "]");
	}

	/**
	 * 仅限于客户端的功能。 通知cache系统，最新的工程名称列表，以供删除不被使用的工程cache数据。
	 * 
	 * @param projIDS
	 * @return 返回删除的工程标识
	 */
	...
	public static String getRMSNameForFileCode(String projID, String uuid,
			String urlID) {
		return RS_PRE_URLID_FILECODE_LIST + projID + "_" + uuid + "_" + urlID
				+ _LIST;
	}

	public static String getRMSNameForFile(String projID, String uuid,
			String urlID) {
		return RS_PRE_URLID_FILE_LIST + projID + "_" + uuid + "_" + urlID
				+ _LIST;
	}
	...
	public static synchronized void removeProjCacheAndRMS(String projID) {
		LogManager.log(
				"[cache] delete all cache data of project [" + projID + "].");
		...
		// 注意：不能先删除，因为有可能存储故障，而导致问题，可以下次再次进行删除
		{
			...
			for (int i = 1; i < size; i++) {
				final CacheDataItem dataItem = (CacheDataItem) projs
						.elementAt(i);
				if (dataItem.isEmpty) {
					continue;
				}
		...
		LogManager.log(
				"[cache] remove all cache for project : [" + projID + "].");
	}
	 ...
	 * @return
	 */
	public synchronized static int getRecordNum(final String projID,
	 ...
	 * @param softUID
	 */
	public synchronized static boolean removeUIDFrom(final String projID,
			final String softUID) {
		...
		for (int uuidIdx = 1; uuidIdx < sizeUUID; uuidIdx++) {
			final CacheDataItem dataItem = (CacheDataItem) uuid
					.elementAt(uuidIdx);
			if (dataItem.isEmpty) {
				continue;
			}
	...
	private static void delCacheAndRMSForURL(String projID, String uuid,
			String urlID) {
		final String rmsNameForFile = getRMSNameForFile(projID, uuid, urlID);
		final String rmsNameForFileCode = getRMSNameForFileCode(projID, uuid,
				urlID);
	...
	// public static Vector getCacheCoder(String projID, String uuid, String
	// urlID){
	// return getCacheVector(getRMSNameForFileCode(projID, uuid, urlID));
	// }

	private static void setDelOnCacheCoderForStoreProblem(String projID,
			String uuid, Vector vectorCoder, String cacheXorCoder) {
		...
		for (int i = size - 1; i >= 1; i--) {
			...
			if (item.toStringValue().equals(cacheXorCoder)) {
				item.setDel();
				CacheStoreManager.storeData(getRMSNameForURLID(projID, uuid),
						vectorCoder);
				return;
			}
		}
	 ...
	 * @return
	 */
	public synchronized static CacheDataItem getCacheFileItem(
			final String projID, String softUID, String urlID,
			final byte[] codeBS, final int codeOffset, final int codeLen) {
		final Vector vectorCode = getCacheVector(
				getRMSNameForFileCode(projID, softUID, urlID));
		final int matchCodeIdx = CacheDataItem.searchMatchVectorIdx(vectorCode,
				codeBS, codeOffset, codeLen);
		if (matchCodeIdx > 0) {
			Vector vectorFile = getCacheVector(
					getRMSNameForFile(projID, softUID, urlID));
			if (vectorFile.size() <= matchCodeIdx) {
				LogManager.errToLog("over size index[" + matchCodeIdx
						+ "] for getCacheFileItem");
				return null;
			}
			final CacheDataItem elementAt = (CacheDataItem) vectorFile
					.elementAt(matchCodeIdx);
			if (elementAt.isEmpty) {
				LogManager.errToLog("size index[" + matchCodeIdx
						+ "] for getCacheFileItem is empty record.");
				return null;
			} else {
				return elementAt;
			}
		...
		// LogManager.log("[cache] no file cache for [" + projID + "/" + uuid +
		// "/" + urlID + "]");
		return null;
	}
	 ...
	 * @return
	 */
	public static byte[] getCacheFileBS(final String projID,
			final String softUID, final String urlID, final byte[] codeBS) {
		CacheDataItem item = getCacheFileItem(projID, softUID, urlID, codeBS, 0,
				codeBS.length);
		if (item != null) {
			return item.bs;
		} else {
	 ...
	 * 获得指定特征码的数据字符串。
	 */
	public static String getCacheFileString(final String projID,
			final String softUID, final String urlID, final byte[] codeBS) {
		CacheDataItem item = getCacheFileItem(projID, softUID, urlID, codeBS, 0,
				codeBS.length);
		if (item != null) {
	 ...
	 * 存储一条完整的记录cache中
	 */
	public static synchronized void storeCache(final String projID,
			final String softUID, final String urlID, final byte[] projIDBS,
			final int projIdOffset, final int projLen, final byte[] softUidBS,
			final int softUidOffset, final int softUidLen, final byte[] urlIDBS,
			final int urlIDOffset, final int urlIDLen, final byte[] codeBS,
			final int codeOffset, final int codeLen, final byte[] valueBS,
			final int valueOffset, final int valuelen, boolean valueIsCopyed) {

		// LogManager.log("projID : " + projID + ", uuid : " + uuid + ", urlID :
		// " + urlID);

		// 检查是否存在projID，如果没有，则更新到库中
		{
			Vector vectorProj = getCacheVector(RS_PROJ_LIST);
			CacheDataItem.addRecrodIfNotExists(RS_PROJ_LIST, vectorProj,
					projIDBS, projIdOffset, projLen);
		}

		{
			final String rmsUUID = getRMSNameForUUID(projID);
			Vector vectorUUID = getCacheVector(rmsUUID);
			CacheDataItem.addRecrodIfNotExists(rmsUUID, vectorUUID, softUidBS,
					softUidOffset, softUidLen);
		}
		...
		// 检查urlID是否存在，
		{
			...
			CacheDataItem.addRecrodIfNotExists(rmsNameForURLID, vectorURLID,
					urlIDBS, urlIDOffset, urlIDLen);
		}

		int storeIdx = 0;
		final String rmsNameForFileCode = getRMSNameForFileCode(projID, softUID,
				urlID);
		...
		if (fileSize == vectorCoder.size() + 1) {// 先存file，后存code
			...
			if (fileSize >= 2) {
				CacheDataItem item = (CacheDataItem) vectorFile
						.elementAt(fileSize - 1);
		...
		storeIdx = CacheDataItem.searchMatchVectorIdx(vectorCoder, codeBS,
				codeOffset, codeLen);
		final boolean isMatch = (storeIdx > 0);
		if (isMatch == false) {
			storeIdx = CacheDataItem.addRecrodAndSave(rmsNameForFile,
					vectorFile, valueBS, valueOffset, valuelen, valueIsCopyed,
					CacheDataItem.isEmptyFirst);
		} else {
			...
			if (valueIsCopyed) {
				...
				System.arraycopy(valueBS, valueOffset, copyValueBS, 0,
						copyValueBS.length);
		...
		if (isMatch == false) {
			if (storeIdx == vectorCoder.size()) {// 后一个条件是由于保存时，可能不同步，导致的差错
				CacheDataItem.addRecrodAndSave(rmsNameForFileCode, vectorCoder,
						codeBS, codeOffset, codeLen, false, false);// 注意：此处isEmptyFirst必须为false
			} else {
				// 重用empty record
				CacheDataItem item = (CacheDataItem) vectorCoder
						.elementAt(storeIdx);

				if (item.isEmpty == false) {
					LogManager.errToLog(
							"rmsNameForFileCode should be empty at idx : "
									+ storeIdx + ", but it is used.");
				}

				final byte[] copyValueBS = new byte[codeLen];
				System.arraycopy(codeBS, codeOffset, copyValueBS, 0,
						copyValueBS.length);
				item.setUpdate(copyValueBS);
			...
			LogManager.log("[cache] successful store cache item for [" + projID
					+ "/" + softUID + "/" + urlID + "/"
					+ (storeIdx < 0 ? fileSize : storeIdx) + "]");
		}
	}
	 ...
	 * @return
	 */
	private static boolean isOverflowProjectTotalCacheNum(final String projID,
		...
		for (int uuidIdx = 1; uuidIdx < uuidSize; uuidIdx++) {
			...
			final Vector urlVector = getCacheVector(
					getRMSNameForURLID(projID, uuidStr));
			final int size = urlVector.size();
			for (int i = 1; i < size; i++) {
				final CacheDataItem itemUrlID = (CacheDataItem) urlVector
						.elementAt(i);
				if (itemUrlID.isEmpty) {
					continue;
				}
				total += CacheStoreManager
						.getRecordNumByStoreName(getRMSNameForFileCode(projID,
								uuidStr, itemUrlID.toStringValue()));
			}
		}
	...
	public static synchronized final void searchAndRemoveTooLongSleep(
			CacheTooLongChecker checker) {
	 ...
	 * @return
	 */
	public static synchronized Vector checkAndDelCacheOverflow(
			final String matchUID) {
		...
		for (int i = 1; i < size; i++) {
			...
			if (isOverflowProjectTotalCacheNum(itemProjID, matchUID)) {// 检查存储记录数是否过载
				LogManager.log("cache [" + itemProjID + "/" + matchUID
						+ "] is overflow (up threshold), clear it.");
				removeUIDFrom(itemProjID, matchUID);// 仅删除关联，不能全删工程

				delProjVector.addElement(itemProjID);
			} else {
				// LogManager.log("cache [" + itemProjID + "/" + matchUID + "]
				// is not overflow (up threshold).");
			}
		}
