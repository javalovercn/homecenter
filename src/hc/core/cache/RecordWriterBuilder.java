...
public abstract class RecordWriterBuilder {
	private static final String TABLE_PREFIX = "table"
			+ CacheManager.TABLE_NAME_SPLITTER;
	protected final String tableNameForMapper = "SYS_TableMapper";
	protected final String end_tableNameForMapper = CacheManager.TABLE_NAME_SPLITTER
			+ tableNameForMapper;
	...
	public RecordWriterBuilder(final String rsPrefixForMobile) {
		...
		mapSnap = CacheStoreManager.getDataList(buildMapStore(),
				tableNameForMapper);
	...
	public final String getTableRealName(String rmsName) {
		...
		if (tableNameForMapper.equals(rmsName) // for J2SE的不转换特殊表
				|| (isEndTableNameMapper = rmsName
						.endsWith(end_tableNameForMapper))// for J2ME关联登录账号
				|| isMapName == false) {
		...
		synchronized (mapSnap) {
			...
			for (int i = 1; i < size; i++) {
				...
				if (di.isEmpty == false && di.toStringValue().equals(rmsName)) {
					tableIdx = i;
					// LogManager.log("successful find MapTableName for : " +
					// rmsName + "[" + (TABLE_PREFIX + tableIdx) + "], on record
					// number : " + tableIdx);
			...
			for (int i = 1; i < size; i++) {
				...
				if (di.isEmpty) {
					...
					tableIdx = i;
					// LogManager.log("successful reuse MapTableName for : " +
					// rmsName + "[" + (TABLE_PREFIX + tableIdx) + "], on record
					// number : " + tableIdx);
			...
			if (tableIdx == -1) {
				CacheDataItem di = new CacheDataItem(
						StringUtil.getBytes(rmsName),
						CacheDataItem.STATUS_TO_ADD);
				mapSnap.addElement(di);
				CacheStoreManager.storeData(tableNameForMapper, mapSnap);

				tableIdx = size;

				// LogManager.log("successful add MapTableName for : " + rmsName
				// + "[" + (TABLE_PREFIX + tableIdx) + "], left record number :
				// " + tableIdx);
			}
		}
	...
	public abstract RecordWriter openRecordStore(String rmsName,
			boolean createIfNecessary) throws Exception;
	...
	public void deleteRecordStore(final String rmsName) throws Exception {
		...
		try {
			// LogManager.log("try delete MapTableName in deleteRecordStore for
			// : [" + rmsName + "]");
			...
			synchronized (mapSnap) {
				final int size = mapSnap.size();
				if (int_idx < size) {
					CacheDataItem di = (CacheDataItem) mapSnap
							.elementAt(int_idx);
					di.setDel();
					CacheStoreManager.storeData(tableNameForMapper, mapSnap);

					// LogManager.log("successful delete MapTableName for : " +
					// rmsName + ", left record number : " + (size - 1));
					return;
				}
			}
		} catch (Exception e) {
