...
public class CacheStoreManager {
	...
	public static RecordWriter openRecordStore(final String dbname,
			final boolean createIfNecessary) throws Exception {
	...
	public static void storeData(final String rsName, final Vector vector) {
		synchronized (vector) {
			...
			try {
				final CacheDataItem firstDataItem = (CacheDataItem) vector
						.elementAt(0);
				...
				for (int logicRecordNo = 1; logicRecordNo < vectorSize; logicRecordNo++) {
					CacheDataItem nextItem = (CacheDataItem) vector
							.elementAt(logicRecordNo);
					if ((nextItem.status == CacheDataItem.STATUS_TO_ADD
							|| nextItem.status == CacheDataItem.STATUS_TO_UPDATE)) {
						if (nextItem.isEmpty || logicRecordNo <= oldRecordNum) {
							rs.setRecord(logicRecordNo + 1, nextItem.bs, 0,
									nextItem.bs.length);// 首记录RecordNo=1为记录数，所以实际存储从第2开始
						} else {
						...
						rs.setRecord(logicRecordNo + 1, EMPTY_BS, 0,
								EMPTY_BS.length);// 注意：没有物理删除，只是将数据特殊处理。
						nextItem.setEmpty();
					}
				...
				if (oldRecordNum != newRecordNum) {
					...
					rs.setRecord(1, firstDataItem.bs, 0,
							firstDataItem.bs.length);// 总记录数只能在存储时，进行更新。
				}
			} catch (Exception e) {
				ExceptionReporter.printStackTrace(e);
			} finally {
	...
	private final static void setRecordNumToBS(final byte[] bs,
			final int newNum) {
		if (bs.length == 2) {
	 ...
	 * @return DataItem
	 */
	public static Vector getDataList(final RecordWriter rs,
		...
		try {
			...
			{
				CacheDataItem item = new CacheDataItem(bs,
						CacheDataItem.STATUS_EXISTS);// 第一条recordNo=1，为总记录数
				v.addElement(item);
			}
			...
			for (int idx = 1; idx < endIdx; idx++) {
				...
				CacheDataItem item = new CacheDataItem(bs,
						CacheDataItem.STATUS_EXISTS);
				// if(item.isEmpty){
				// LogManager.log(logicTableName + " recordID [" + idx + "] is
				// empty!");
				// }else{
				// LogManager.log(logicTableName + " recordID [" + idx + "] len
				// : " + item.bs.length);
				// }
				v.addElement(item);
			}

			// LogManager.log(logicTableName + " total record num : " +
			// totalProjNum);
		} catch (Exception e) {
			isException = true;
			ExceptionReporter.printStackTrace(e);
