...
public class MsgBuilder {
	 ...
	 * 最大的坏处就是PPPoE导致MTU变小了以太网的MTU是1500，再减去PPP的包头包尾的开销（8Bytes），
	 * 就变成1492 bytes for PPPoE。1450 for PPTP connections
	 * 又因为UDP数据报的首部8字节,所以UDP数据报的数据区最大长度为1472字节. 这个1472字节就是我们可以使用的字节数。:) 
	 * 那么系统将会使用一系列的机制来调整MTU值,使数据报能够顺利到达目的地,这样就会做许多不必要的操作.
	 * 鉴于 Internet上的标准MTU值为576字节,所以我建议在进行Internet的UDP编程时.
	 * 最好将UDP的数据长度控件在540字节 (576-8-20-PPPoE8)以内.
	 * 
	 * 本参数须置于最前
	 */
	
	public static final int UDP_INTERNET_BEST_MSS = 1414;//(含PPPoE8位, 1464), (1450 - 28 = 1422 - 8 = 1414)
	...
	public final static int UDP_BYTE_SIZE = Integer.parseInt(
			RootConfig.getInstance().getProperty(
					RootConfig.p_DefaultUDPSize));//484;//548;//1464;
	
	/**
	 * 服务器如果发现可以切换的Relay，则进行连接并挂上；
	 * 具体：服务器在等待客户机未到时，发现可供其中继的，向其先发送本信令，
	 * 中继服务回应其信令，完成中继过程
	 */
...
//	public static final byte E_TAG_NOTIFY_TO_NEW_RELAY_BACK = 10;
	...
	public static final short INDEX_UUID_VALUE_START = INDEX_UUID_LEN + LEN_UUID_LEN_STORE;//值开始的地方，如上为j所在。
	...
	public static final short INDEX_PACKET_SPLIT_NUM_LOW = INDEX_PACKET_SPLIT_NUM_HIGH + LEN_PACKET_SPLIT_NUM_HIGH;
	
	public static final short INDEX_PACKET_SPLIT_NO_HIGH = INDEX_PACKET_SPLIT_NUM_LOW + LEN_PACKET_SPLIT_NUM_LOW;
	public static final short INDEX_PACKET_SPLIT_NO_LOW = INDEX_PACKET_SPLIT_NO_HIGH + LEN_PACKET_SPLIT_NO_HIGH;
