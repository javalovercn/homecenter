...
public class EventCenterDriver extends HCConditionWatcher {
	...
	public EventCenterDriver() {
		super("EventCenterDriver",
				ThreadPriorityManager.GECD_THREADGROUP_PRIORITY);
		synchronized (lock) {
			aliveCount++;
			if (notifyShutdown) {
				notifyShutdown();
	...
	public void notifyAllDoneAfterShutdown() {
		...
		L.V = L.WShop ? false
				: LogManager.log(
						"a EventCenterDriver instance AllDoneAfterShutdown.");
	}
	...
	public static void waitForAllDriverDone() {
		synchronized (lock) {
			...
			L.V = L.WShop ? false
					: LogManager.log(
							"EventCenterDriver aliveCount : " + aliveCount);
			if (aliveCount > 0) {
				try {
