...
public class EventBack implements IWatcher {
	...
	private final static EventBackCacher ebCacher = EventBackCacher
			.getInstance();
	...
	public boolean watch() {
		...
		if (datagramPacket != null) {
			...
			if (bs[IDX_HEADER1] == hcConnection.udpHeader[0]
					&& bs[IDX_HEADER2] == hcConnection.udpHeader[1]) {
			} else {
				cancel();
				return true;
			}
			...
			if (isInWorkshop) {
				LogManager.log("EventBack Receive ctrlTag : [" + ctrlTag
						+ "] in session [" + coreSocketSession.hashCode()
						+ "].");
			}
			...
			if (serverSide && ctrlTag > MsgBuilder.UN_XOR_MSG_TAG_MIN) {
				if (coreSocketSession
						.isContextStatus(ContextManager.STATUS_SERVER_SELF)) {
				} else {
					LogManager.log("Invalid statue tag received[" + ctrlTag
							+ "], cmStatus : "
							+ coreSocketSession.context.getStatus());
					cancel();
					return true;
				}
			}
			...
			if (ctrlTag == MsgBuilder.E_TAG_ACK) {
				...
				hcConnection.sipContext.resender.ackAtSend(bs,
				...
				hcConnection.sipContext.ackbatchTimer.ack(bs,
						MsgBuilder.INDEX_MSG_ID_HIGH);
				...
				// LogManager.log("msgID(Send from other):" + msgID + " is
				// Received.");
			...
			if (dataLen == 0 || ctrlTag <= MsgBuilder.UN_XOR_MSG_TAG_MIN) {
				...
				CUtil.superXor(hcConnection, hcConnection.OneTimeCertKey, bs,
						MsgBuilder.INDEX_UDP_MSG_DATA, dataLen, null, false,
						true);
			}
			...
			if (bs[MsgBuilder.INDEX_PACKET_SPLIT] == MsgBuilder.DATA_PACKET_SPLIT) {
				...
				if (subPacket != null) {
					...
					// LogManager.log("comp split:[" +
					// bs[MsgBuilder.INDEX_CTRL_TAG] + "], dataLen:" + merLen);

					System.arraycopy(bs, MsgBuilder.INDEX_UDP_MSG_DATA, bs,
							MsgBuilder.INDEX_MSG_DATA, merLen);
				} else {
	 ...
	 * 
	 * @param dp
	 *            如果传入null，则表示为TCP接收的数据，否则为UDP接收的数据，它们存在结构上的差别
	 * @param b
	 * @param len
	 */
	public void setBSAndDatalen(final CoreSession coreSocketSession,
			final Object dp, final byte[] b, final int len) {
