...
public class HCConditionWatcher {
	...
	private HCConditionWatcher(final String tName, final boolean isNewThread,
			final int newThreadPrority) {
		this.timeName = tName;
		isInWorkshop = L.isInWorkshop;

		watcherTimer = new HCTimer(timeName, HCTimer.HC_INTERNAL_MS, false,
				isNewThread, newThreadPrority) {
			...
			public final void doBiz() {
				...
				do {
					...
					if (temp == null) {
						if (isNotifyShutdown) {// 直到所有任务完成后，移除自己
							// if(L.isInWorkshop){
							// LogManager.log("shutdown HCConditionWatcher [" +
							// watcherTimer.getName() + "].");
							// }
				...
				if (isAddUnUsed) {
					if (isNotifyShutdown && isInWorkshop) {
						LogManager.log("[" + timeName
								+ "] processing unUsed watcher.");
					}

					synchronized (watchers) {
						Object rewatcher;
						while ((rewatcher = usedRewatchers
								.getFirst()) != null) {
	...
	private final void cancelOp(final LinkedSet ls) {
		...
		while ((watcher = (IWatcher) ls.getFirst()) != null) {
			if (watcher.isCancelable()) {// 专为EventBack之用
				if (isInWorkshop) {
					System.out.println("----------cancle EventBack : "
		...
		while (unCancelSet != null
				&& (watcher = (IWatcher) unCancelSet.getFirst()) != null) {
			ls.addTail(watcher);
		}
	}
	...
	public final void waitForAllDone() {
		synchronized (watchers) {
			if (isEmptyImpl() == false) {
				if (L.isInWorkshop) {
					LogManager.log("waitForAllDone(), watcherTimer enable : "
							+ watcherTimer.isEnable + ", watchers.size : "
							+ watchers.size() + ", usedRewatchers.size : "
							+ usedRewatchers.size());
				}
				try {
					watchers.wait(
							ThreadPriorityManager.SEQUENCE_TASK_MAX_WAIT_MS);
	...
	public final void addWatcher(final IWatcher watcher) {
		...
		// if(isInWorkshop){
		// LogManager.log("[" + timeName + "] add watcher : " +
		// watcher.hashCode());
		// }
	...
	protected void processingItem(IWatcher temp) {
		...
		synchronized (temp) {
			try {
				...
				LogManager
						.errToLog("Error IWatcher class : " + temp.getClass());
