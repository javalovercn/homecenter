...
public class EventCenter {
	private final int event_listener_max_size = (IConstant.serverSide ? 1000
			: 200);
	...
	public final void removeListener(IEventHCListener listener) {
		...
		synchronized (listens) {
			for (int i = 0; i < size; i++) {
				if (isRemoved == false && listens[i] == listener) {
					L.V = L.WShop ? false
							: LogManager
									.log("sucessful remove EventHCListener ["
											+ listener.getEventTag() + "].");
					isRemoved = true;
				} else if (isRemoved) {
		...
		if (isRemoved == false) {
			L.V = L.WShop ? false
					: LogManager.log("fail to remove EventHCListener ["
							+ listener.getEventTag() + "], NOT found.");
		}
	}
	...
	public final void addListener(IEventHCListener listener) {
		...
		synchronized (listens) {
			if (size == event_listener_max_size) {
				LogManager.err("EventCenter over size listers : "
						+ event_listener_max_size);
				return;
			}
			...
			for (int i = 0; i < size; i++) {
				if (listen_types[i] == eventTag) {
					if (enableSameEventTag) {
						LogManager.log("EventTag:" + eventTag
								+ ", register twice or more!");
					} else {
						// 因为旧的可能含有不正确的数据，所以要以新的覆盖旧的。
						listens[i] = listener;
						LogManager.log("Rewrite EventTag:" + eventTag
								+ " EventHCListener!");
						return;
					}
				}
	...
	// 不回收
	final void action(final byte ctrlTag, final byte[] event,
			final NestAction nestAction) {
	...
	public final NestAction nestAction = (NestAction) ConfigManager
