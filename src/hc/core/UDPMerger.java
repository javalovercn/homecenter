...
public class UDPMerger {
   private static int getReceiveSplitMaxSize(){
    ...
    /**
     * 最大分包缓存数要考虑以下几个因素：
     * 1.由于采用服务器小块分传技术，会降低对一次大包的要求。
     * 2.服务器采用图片间相差发现，从而只传送变化的数据块，以降低了分包的大小。
     * 3.重传技术和缓存丢弃技术会导致缓存的包会存在较长的时间，从而增加了缓存空间的要求。
     * 4.不同硬件环境可提供的硬件区别较大。
     * 5.降低传送频度，可以减少缓存空间要求。
     */
     ...
     */
    public static HCUDPSubPacketEvent tryFindCompletSplit(HCUDPSubPacketEvent eventNew){
    	synchronized(LOCK){
...
//	    			System.out.println("Try full msgID:" + SPLIT_MSG.getMsgID() + ", no:" + SPLIT_MSG.getSplitNO());
    			TMP_SPLIT_EVENTS[splitNO] = eventNew;
//	    			TMP_SPLIT_MSG[splitNO] = msgNew;//由于MSG和Event具有相同索引，故不建TMP_SPLIT_MSG
    			
    			//不入缓存的，表示新参数传入，且不在SPLIT_EVENTS之列，释放时以示区别
//	    			TMP_SPLIT_EVENTS_INDEX[splitNO] = -1;
    			
    			int findPacketNum = 1;
		    	for (int i = 0, findEvent = 0; findEvent < STORE_SPLIT_NUM && i < SPLIT_MAX_SIZE; i++) {
		    		...
		    		if(event == null){
		    			...
		    			if(message.BlobGroupID == currGroupID){
		    				...
		    				if((++findPacketNum) == splitBlockNum){
		    					//全部找集
//			    					System.out.println("Full splitBlockNum:" + splitBlockNum + ", MsgID:" + currMsgID);
		    					//取第一个
		    					...
		    					int totalInIndex = splitPackageMaxDataSize + MsgBuilder.INDEX_UDP_MSG_DATA;
								System.arraycopy(splitBS, 0, totalbs, 0, totalInIndex);
								...
								for (int j = 1; j < splitBlockNum; j++) {
									...
									try{
										...
										System.err.println("Desc:" + totalInIndex + ", copyLen:" + copyLen);
										return null;
									}
								...
								//不回收，但要参与清空逻辑
								//SPLIT_HCEVENT[TMP_SPLIT_EVENTS_INDEX[0]] = null;
...
//					    				System.out.println("Cmplit:" + t_index);
				    				if(t_index != -1){
										...
										if(eventTotal == event_split){
				    			    		...
				    			    		packetCacher.cycle(event_split.datagram);
...
//				    						SPLIT_MSG[t_index] = null;//不需要置null
										...
										if(eventTotal == event_total_tmp){
				    						...
				    						packetCacher.cycle(event_total_tmp.datagram);
...
//											System.out.println("Unused split hcevent msgID:" + SPLIT_MSG.getMsgID() + ", no:" + SPLIT_MSG.getSplitNO() );
//										}
//									}
								return eventTotal;
		    				}else{
			...
			for (int i = 0; i < SPLIT_MAX_SIZE; i++) {
				if(SPLIT_HCEVENT[i] == null){
					//入缓存，以备完整时之用
//						System.out.println("Push Temp Store blob msgID:" + SPLIT_MSG.getMsgID() + ", no:" + SPLIT_MSG.getSplitNO() + ", store Num:" + STORE_SPLIT_NUM);
