...
public class RootConfig extends HCConfig {
				...
				Thread t = new Thread() {
					...
					public void run() {
						...
						RootBuilder.getInstance().doBiz(
								RootBuilder.ROOT_START_NEW_IDEL_SESSION, null);
		...
		do {
			...
			if (msg != null) {
				RootBuilder.getInstance()
						.doBiz(RootBuilder.ROOT_SET_LAST_ROOT_CFG, msg);
			} else {
				final String lastRootCfg = (String) RootBuilder.getInstance()
						.doBiz(RootBuilder.ROOT_GET_LAST_ROOT_CFG, null);
				if (lastRootCfg != null) {
					RootServerConnector.stopRepairTipTimer();
					LogManager.errToLog("shift to OFF-LINE mode!!!");
					LogManager.errToLog(
							"fail to get root config from net, use last root config!!!");
					msg = lastRootCfg;
				...
				if (rc == null) {
					...
					synchronized (listenerVector) {
						...
						for (int i = size - 1; i >= 0; i--) {
							RootConfigListener listener = (RootConfigListener) listenerVector
									.elementAt(i);
							listener.onRefresh();
						}
					}
	...
	// deprecated
	public static final short p_Sample_Ver = 36;// stop online,
	...
	public static final short p_GGSpeechServerIP = 49;
