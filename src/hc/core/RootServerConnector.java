...
public class RootServerConnector {
	...
	public static final String PORT_44X_WITH_MAOHAO = ":"
			+ String.valueOf(PORT_44X);
	public static final String PORT_808044X = PORT_8080_WITH_MAOHAO
			+ PORT_44X_WITH_MAOHAO;
	...
	public static final String HTTS_HC_44X = "https://" + HOST_HOMECENTER_MOBI
			+ PORT_44X_WITH_MAOHAO;
	private static final String AJAX_HTTP_URL_PREFIX = "http://"
			+ HOST_HOMECENTER_MOBI + "/ajax/";
	public static final String AJAX_HTTPS_44X_URL_PREFIX = HTTS_HC_44X
			+ "/ajax/";

	// http://homecenter.mobi/ajax/call.php?
	public final static String CALL_STR = AJAX_HTTPS_44X_URL_PREFIX
			+ "call.php?";
	// id=
	...
	public final static String MULTICAST_IPV4 = "232.28.7.76";// see App.main
	...
	public final static boolean isQueryDirectServerIPCmd(final byte[] bs,
		...
		if (len >= MC_QUERY_DIRECT_SERVER_IP.length) {
			return isCmd(bs, off, MC_QUERY_DIRECT_SERVER_IP.length,
					MC_QUERY_DIRECT_SERVER_IP);
		} else {
			return false;
		}
	}
	...
	public static boolean isRegedToken(String uuid, String token) {
		...
		// "f=isRegTK&" +
				unObfuscate("=fsieRTg&K") + ID_STR + encryptePara(uuid, token)
				+ "&" + TOKEN_STR + token + "&" + ENCRYPTER_STR + "true");
	...
	public static String buildWaitForCheckURL(final String id,
		...
		return CALL_STR + "f=waitForCheck&" + ID_STR
				+ encryptePara(id, tokenNotVerifyToken) + "&" + TOKEN_STR
				+ tokenNotVerifyToken + "&" + ENCRYPTER_STR + "true";
	}

	/**
	 * IPV6 最长IPv6形式 X:X:X:X:X:X:d.d.d.d 3组16进制整数，每组4个16进制数的字符形态； 6个冒号；
	 * 一个IPv4地址（不算末尾的NULL，就是15位） 还有一个NULL结束符。 因此， 最大可能长度 = 6 * 4 + 6 + 15 + 1 =
	 * 46 所以， INET6_ADDRSTRLEN定义为46 homecenter.mobi存储包括[X:X:X:X:X:X:d.d.d.d] 48
	 * = 46 + 2(][) RFC 2396 [URL]. It defines a syntax for IPv6 addresses and
	 * allows the use of "[" and "]" within a URI
	 * 
	 * @param uuid
	 * @param hostAddress
	 * @param port
	 ...
	 */
	public static String lineOn(String uuid, String hostAddress, int port,
			int nattype, int agent, String upnp_ip, int upnp_port,
			String relayip, int relayport, String token, boolean hideIP,
			String hideToken) {
		if (hostAddress == null) {
			hostAddress = "";
		}
		...
		return retry(CALL_STR + lineon_STR + ID_STR + encryptePara(uuid, token)
				+ "&" + IP_STR + encryptePara(hostAddress, token) + "&"
				+ PORT_STR + port + "&" + nattype_STR + nattype + "&"
				+ agent_STR + agent + "&" + upnpip_STR + upnp_ip + "&"
				+ upnpport_STR + upnp_port + "&" + relayip_STR + relayip + "&"
				+ relayport_STR + relayport + "&" + TOKEN_STR + token + "&"
				+ HIDE_IP_STR + hideIP + "&" + HIDE_TOKEN_STR + hideToken + "&"
				+ ENCRYPTER_STR + "true");
	}

	public static String submitEmail(final String id, final String token,
			final String verifytoken) {
		return retry(CALL_STR + "f=submitEmail&" + ID_STR
				+ encryptePara(id, token) + "&" + TOKEN_STR + token + "&"
				+ "verifytoken=" + verifytoken + "&" + ENCRYPTER_STR + "true");
	}
	...
	public static void serverNum(int serverNum, String token) {
		// LogManager.log("change serverNum : " + serverNum + ", id=" +
		// IConstant.uuid );
		retry(CALL_STR + f_serverNum_STR + ID_STR
				+ encryptePara(IConstant.uuid, token) + "&" + serverNum_STR
				+ serverNum + "&" + TOKEN_STR + token + "&" + ENCRYPTER_STR
				+ "true");
	}
	...
	public static String bindDonateKey(String email, String donateKey) {
		...
		// "f=bindDonateKey&"
				unObfuscate("=fibdnoDaneteK&y") + unObfuscate("meia=l")
				+ encryptePara(email, donateKey) + "&" +
				// "email="
				// unObfuscate("meia=l") + email + "&" +
				// "donateKey="
				unObfuscate("odaneteK=y") + donateKey + "&" + ENCRYPTER_STR
				+ "true");
	}

	public static String getHideToken() {
		final byte[] bs = (byte[]) IConstant.getInstance()
				.getObject(IConstant.CertKey);
	 ...
	 * @param token
	 */
	public static void changeRelay(String uuid, String relayIP,
			String relayPort, String token) {
		relayIP = HCURLUtil.convertIPv46(relayIP);

		retry(CALL_STR + chngrelay_STR + ID_STR + encryptePara(uuid, token)
				+ "&" + IP_STR + relayIP + "&" + PORT_STR + relayPort + "&"
				+ TOKEN_STR + token + "&" + ENCRYPTER_STR + "true");
	}

	/**
	 * 服务器：如果没有客户机上线，则返回可供使用的中继Vector[ip, port]； 客户机：如果没有服务器上线，则返回null {IP,
	 * port, [1:http方式;2:代理上线（即取代1，则不是HomeCenter.mobi来实现双方IP交换）], upnpip,
	 * upnpport} 其中upnp仅服务器有效 注意：本方法已被getServerIPAndPortV2替代
	 * 
	 * @return
	 */
	public static Object getServerIPAndPort(String hideToken) {// 早期版本，现停用，不能获得多Session模式下，客户并发上线的问题
		String msg = retry(CALL_STR + sip_STR + ID_STR
				+ encryptePara(IConstant.uuid, hideToken) + "&" + HIDE_TOKEN_STR
				+ hideToken + "&" + ENCRYPTER_STR + "true");
	 ...
	 * 服务器：如果没有客户机上线，则返回可供使用的中继Vector[ip, port]； 客户机：如果没有服务器上线，则返回null {IP,
	 * port, [1:http方式;2:代理上线（即取代1，则不是HomeCenter.mobi来实现双方IP交换）], upnpip,
	 * upnpport} 其中upnp仅服务器有效
	 * 
	 * @return
	 */
	public static Object getServerIPAndPortV2(String hideToken) {// 能获得多Session模式下，客户并发上线的问题
		String msg = retry(CALL_STR + sipV2_STR + ID_STR
				+ encryptePara(IConstant.uuid, hideToken) + "&" + HIDE_TOKEN_STR
				+ hideToken + "&" + ENCRYPTER_STR + "true");

		if (msg == null || msg.length() == 0) {
			if (RootConfig.isFailToGetAliveRootCfg()) {
				final String relayDisable = UIUtil.getUIString("1035",
						"Off-Line mode, net relay is not available!");
				RootBuilder.getInstance().doBiz(RootBuilder.ROOT_SPEAK_VOICE,
						relayDisable);
				LogManager.info(relayDisable);
			}
			final String lastAlive = (String) RootBuilder.getInstance()
					.doBiz(RootBuilder.ROOT_GET_LAST_ALIVE_IP_INFO, null);
			if (lastAlive != null) {
				LogManager.info(UIUtil.getUIString("m130",
						"line off, try last connection."));// 仅在离线信息时，
				LogManager.errToLog(
						"fail to get alive IP/Port, use last saved alive IP/Port!!!");
				msg = lastAlive;
			}
		} else {
			SIPManager.infoServerOnline();
			RootBuilder.getInstance()
					.doBiz(RootBuilder.ROOT_SET_LAST_ALIVE_IP_INFO, msg);
		}
	...
	public static Object getNewRelayServers(final String uuid,
			final String token) {
		String msg = retry(CALL_STR + newrelay_STR + "&" + ID_STR
				+ encryptePara(uuid, token) + "&" + TOKEN_STR + token + "&"
				+ ENCRYPTER_STR + "true");
	...
	// public static String sendTCPToRoot(String ip, int port, byte ctrlTag,
	// String msg){
	// Object socket = null;
	// try{
	...
	// 5];//DatagramPacketCacher.getInstance().getFree();
	//
	...
	//
	// isDone[0] = true;
	//// System.out.println("Receive SendTCP PNGDataLen : " +
	// dataStr.getPNGDataLen());
	// final String out_str = new String(bs, DataPNG.png_index,
	// dataStr.getPNGDataLen(), IConstant.UTF_8);
	...
	public static String getRootConfig() {
		return retry(CALL_STR + rootcfg_STR);
	}
	...
	public static String delLineInfo(String uuid, String token) {
		...
		return retry(CALL_STR + lineOffStr + ID_STR + encryptePara(uuid, token)
				+ "&" + TOKEN_STR + token + "&" + ENCRYPTER_STR + "true");
	}

	public static String refreshRootAlive_impl(String token, boolean hideIP,
			String hideToken) {
		return retry(CALL_STR + alive_STR + ID_STR
				+ encryptePara(IConstant.uuid, token) + "&" + TOKEN_STR + token
				+ "&" + HIDE_IP_STR + hideIP + "&" + HIDE_TOKEN_STR + hideToken
				+ "&" + ENCRYPTER_STR + "true");
	}
	 ...
	 * @param type
	 */
	public static void notifyLineOffType(final CoreSession coreSS,
			final String type) {
		...
		if (LangUtil.isTourPartOrMore(locale)) {
			ContextManager.getThreadPool().run(new Runnable() {
				public void run() {
					...
					sb.append(CALL_STR).append(REP_Locale_STR).append(locale)
							.append("_").append(os).append("_");
					if (version != null) {
						sb.append(version);
					}
	...
	public static String encryptePara(final String str,
			final String hideToken) {
		...
		try {
			return URLEncoder.encode(new String(str_bs, IConstant.UTF_8),
					IConstant.UTF_8);
		} catch (Exception e) {
			return "";
		}
	...
	public static final String convertToHttpAjax(String https_url) {
		if (IConstant.serverSide == false && ((ConfigManager.isJ2MEClient())
				|| (ConfigManager.getOSType() == ConfigManager.OS_IPHONE
						&& ((Boolean) RootBuilder.getInstance()
								.doBiz(RootBuilder.ROOT_BIZ_IS_SIMU, null))
										.booleanValue()))) {
			https_url = StringUtil.replaceStartStr(https_url,
					AJAX_HTTPS_44X_URL_PREFIX, AJAX_HTTP_URL_PREFIX);
	...
	public static void errorRepareNetwork() {
		final Object repare_net = RootBuilder.getInstance()
				.doBiz(RootBuilder.ROOT_GET_RESOURCE, new Integer(100));
