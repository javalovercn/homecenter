...
public class RootTagEventHCListener extends IEventHCListener {
	...
	public final boolean action(final byte[] bs, final CoreSession coreSS,
		...
		if (subTag == MsgBuilder.DATA_ROOT_LINE_WATCHER_ON_RELAY) {
			...
			// coreSS.context.doExtBiz(IContext.BIZ_MATCHED_FOR_CLIENT_ON_RELAY,
			// null);
			return true;
			...
			if (IConstant.serverSide == false) {
				...
				// LogManager.log("Send back line watch at
				...
				if (L.isInWorkshop) {
					LogManager.log(
							"[keepalive] Receive line watch at RootTagEventHCListener");
				}
			}
			...
			if (IConstant.serverSide == false) {
				coreSS.context.doExtBiz(
						IContext.BIZ_NOTIFY_SERVER_IN_DIRECT_MODE, null);
			}
			return true;
		} else if (subTag == MsgBuilder.DATA_ROOT_SAME_ID_IS_USING) {
			String sameIDMsg = (String) RootBuilder.getInstance()
					.doBiz(RootBuilder.ROOT_GET_RESOURCE, new Integer(9259));
			if (sameIDMsg == null) {
			...
			int port = ByteUtil.twoBytesToInteger(bs,
			...
			if (hcConnection.sipContext.buildUDPChannel(hcConnection)) {
				...
				if (IConstant.serverSide) {
					UDPPacketResender.sendUDPBlockData(hcConnection,
							MsgBuilder.E_TAG_MTU_1472,
