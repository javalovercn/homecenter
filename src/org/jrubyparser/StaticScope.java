 ...
 * 
 * At a very high level every scopes enclosing scope contains variables in the next outer lexical
 * layer. The enclosing scopes variables may or may not be reachable depending on the scoping rules
 * for variables (governed by BlockStaticScope and LocalStaticScope).
 * 
 * StaticScope also keeps track of current module/class that is in scope. previousCRefScope will
 * point to the previous scope of the enclosing module/class (cref).
 * 
 */
public abstract class StaticScope implements Serializable {
	 ...
	 * relationship to its child. If I am in a localScope and then I enter something which creates
	 * another localScope the enclosing scope will be the first scope, but there are no valid
	 * scoping relationships between the two. Methods which walk the enclosing scopes are
	 * responsible for enforcing appropriate scoping relationships.
	 * 
	...
	/**
	 * Get all visible variables that we can see from this scope that have been assigned (e.g. seen
	 * so far)
	 * 
	 ...
	 * 
	 * @param position
	 *            the location that in the source that the new node will come from
	 * @param name
