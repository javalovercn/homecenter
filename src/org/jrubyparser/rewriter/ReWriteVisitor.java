...
public class ReWriteVisitor implements NodeVisitor {
	...
	private boolean sourceRangeContains(SourcePosition pos, String searched) {
		return pos.getStartOffset() < config.getSource().length() && pos.getEndOffset() < config.getSource().length() + 1
	...
	private boolean isAssignment(Node n) {
		return (n instanceof DAsgnNode || n instanceof GlobalAsgnNode || n instanceof InstAsgnNode || n instanceof LocalAsgnNode
				|| n instanceof ClassVarAsgnNode);
	}
	...
	private boolean isShortIfStatement(IfNode iVisited) {
		return (isOnSingleLine(iVisited.getCondition(), iVisited.getElseBody()) && !(iVisited.getElseBody() instanceof IfNode)
	...
	private boolean isSimpleNode(Node n) {
		return (n instanceof LocalVarNode || n instanceof AssignableNode || n instanceof InstVarNode || n instanceof ClassVarNode
				|| n instanceof GlobalVarNode || n instanceof ConstDeclNode || n instanceof VCallNode || isNumericNode(n));
	}
	...
	public Object visitOpAsgnNode(OpAsgnNode op) {
		...
		print(op.isLazy() ? "&." : ".").print(op.getVariableName()).print(' ').print(op.getOperatorName()).print("=")
				.print(config.getFormatHelper().afterAssignment());
	...
	private void printOpAsgnNode(Node n, String operator) {
		...
		print(((INameNode) n).getName()).print(config.getFormatHelper().beforeAssignment()).print(operator)
				.print(config.getFormatHelper().afterAssignment());
	...
	private boolean isSpecialRegexNotation(Node n) {
		return getStartOffset(n) >= 2 && !(config.getSource().length() < getStartOffset(n))
