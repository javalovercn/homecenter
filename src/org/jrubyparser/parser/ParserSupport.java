...
public class ParserSupport {
	...
	protected void getterIdentifierError(SourcePosition position, String identifier) {
		throw new SyntaxException(PID.BAD_IDENTIFIER, position, "identifier " + identifier + " is not valid", identifier);
	}
	...
	public AssignableNode assignable(Token lhs, Node value) {
		...
		throw new SyntaxException(PID.BAD_IDENTIFIER, lhs.getPosition(), "identifier " + (String) lhs.getValue() + " is not valid",
				lhs.getValue());
	}
	...
	public Node getOperatorCallNode(Node firstNode, String operator, Node secondNode, SourcePosition defaultPosition) {
		...
		return new CallNode(union(firstNode.getPosition(), secondNode.getPosition()), firstNode, operator,
				new ArrayNode(secondNode.getPosition(), secondNode));
	}
	...
	public boolean checkExpression(Node node) {
		...
		while (node != null) {
			...
			case RETRYNODE:
				if (!conditional) {
					throw new SyntaxException(PID.VOID_VALUE_EXPRESSION, node.getPosition(), "void value expression");
				}
				return false;
			case BLOCKNODE:
	...
	public boolean isLiteral(Node node) {
		return node != null && (node instanceof FixnumNode || node instanceof BignumNode || node instanceof FloatNode
				|| node instanceof SymbolNode || (node instanceof RegexpNode && ((RegexpNode) node).getOptions().isLiteral()));
	}
	...
	private boolean checkAssignmentInCondition(Node node) {
		...
		} else if (node instanceof LocalAsgnNode || node instanceof DAsgnNode || node instanceof GlobalAsgnNode
				|| node instanceof InstAsgnNode) {
			Node valueNode = ((AssignableNode) node).getValue();
			if (valueNode instanceof ILiteralNode || valueNode instanceof NilNode || valueNode instanceof TrueNode
					|| valueNode instanceof FalseNode) {
	...
	private Node cond0(Node node) {
		...
		case DOTNODE: {
			...
			return new FlipNode(node.getPosition(), getFlipConditionNode(((DotNode) node).getBegin()),
					getFlipConditionNode(((DotNode) node).getEnd()), dotNode.isExclusive(), slot);
		}
		case REGEXPNODE:
	...
	public AndNode newAndNode(SourcePosition position, Node left, Node right) {
		...
		if (left == null && right == null)
			return new AndNode(position, left, right);
	...
	public OrNode newOrNode(SourcePosition position, Node left, Node right) {
		...
		if (left == null && right == null)
			return new OrNode(position, left, right);

		return new OrNode(union(left, right), left, right);
	}

	/**
	 * Ok I admit that this is somewhat ugly. We post-process a chain of when nodes and analyze them
	 * to re-insert them back into our new CaseNode the way we want. The grammar is being difficult
	 * and until I go back into the depths of that this is where things are.
	 *
	 * @param position
	 *            the position
	 * @param expression
	 *            of the case node (e.g. case foo)
	 * @param firstWhenNode
	...
	public Node newOpAsgn(SourcePosition position, Node receiverNode, String callType, Node valueNode, String variableName,
			String operatorName) {
	...
	public Node new_call(Node receiver, Token name, Node args, Node iter) {
		...
		if (args == null)
			args = new ListNode(name.getPosition().makeEmptyPositionAfterThis());
	...
	public void frobnicate_fcall_args(FCallNode fcall, Node args, Node iter) {
		if (args instanceof BlockPassNode) {
			if (iter != null) {
				throw new SyntaxException(PID.BLOCK_ARG_AND_BLOCK_GIVEN, iter.getPosition(), lexer.getCurrentLine(),
	...
	public Node new_fcall(Token operation, Node args, Node iter) {
		...
		if (args == null)
			args = new ListNode(operation.getPosition().makeEmptyPositionAfterThis());
	 ...
	 * 
	 * @param inSingle
	 *            New value of property inSingle.
	 */
	...
	public Node asSymbol(SourcePosition position, Node value) {
		...
		if (value instanceof StrNode)
			return new SymbolNode(position, ((StrNode) value).getValue().toString());

		return new DSymbolNode(position, (DStrNode) value);
	}

	public ArgsTailHolder new_args_tail(SourcePosition position, ListNode keywordArg, Token keywordRestArgName, BlockArgNode blockArg) {
	...
	public Node new_args(SourcePosition position, ListNode pre, ListNode optional, RestArgNode rest, ListNode post, ArgsTailHolder tail) {
		if (tail == null)
			return new_args(position, pre, optional, rest, post, (BlockArgNode) null);
	...
	public BlockArgNode newBlockArg(SourcePosition position, Token nameToken) {
		...
		if (getCurrentScope().getLocalScope().isDefined(identifier) >= 0) {
			throw new SyntaxException(PID.BAD_IDENTIFIER, position, lexer.getCurrentLine(), "duplicate block argument name");
		}
	...
	public ArgumentNode arg_var(Token identifier) {
		...
		return new ArgumentNode(identifier.getPosition(), name, getCurrentScope().addVariableThisScope(name));
	}
	 ...
	 * Otherwise return null. This allows grammar to not splat into a Ruby Array if splatting a
	 * literal array.
	 * 
	 * @param node
	 *            to be splatted
	...
	public Node arg_append(Node node1, Node node2) {
		...
		if (node1 instanceof BlockPassNode)
			return arg_append(((BlockPassNode) node1).getBody(), node2);
