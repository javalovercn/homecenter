...
public class StringTerm extends StrTerm {
	...
	private RegexpOptions parseRegexpFlags(final LexerSource src) throws java.io.IOException {
		...
		for (c = src.read(); c != Lexer.EOF && Character.isLetter(c); c = src.read()) {
			switch (c) {
			case 'i':
		...
		if (unknownFlags.length() != 0) {
			throw new SyntaxException(PID.REGEXP_UNKNOWN_OPTION, src.getPosition(),
					"unknown regexp option" + (unknownFlags.length() > 1 ? "s" : "") + " - " + unknownFlags.toString(),
	...
	private static String toFuncString(int flags) {
		...
		if ((flags & Lexer.STR_FUNC_ESCAPE) != 0)
		...
		if ((flags & Lexer.STR_FUNC_SYMBOL) != 0)
			builder.append("symbol|");
		if ((flags & Lexer.STR_FUNC_INDENT) != 0)
	...
	@Override
	public String toString() {
		return "StringTerm[flags=" + toFuncString(flags) + ",end=" + end + ",begin=" + (int) begin + ",nest=" + nest + ",embed="
				+ processingEmbedded + "]";
	}
