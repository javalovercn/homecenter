...
public abstract class StrTerm {
	 ...
	 * quoting rules). E.g. are we dealing with a "" string or a '' string (or their alternate
	 * representations)
	 * 
	...
	/**
	 * Flag set while we're processing embedded Ruby expressions. It will be 0 when we are not, or
	 * otherwise set to the the relevant embedded type (EMBEDDED_DVAR or EMBEDDED_DEXPR)
	 */
	 ...
	 * through a call to {@link #setMutableState} later on. Necessary for incremental lexing where
	 * we may restart lexing parts of a string (since they can be split up due to Ruby embedding
	 * like "Evaluated by Ruby: #{foo}".
	 * 
