...
/**
 * This is a port of the MRI lexer to Java it is compatible to Ruby 1.8.x and Ruby 1.9.x depending
 * on compatibility flag.
 */
public class Lexer {
	...
	private int getFloatToken(String number, int suffix) {
		if ((suffix & SUFFIX_R) != 0) {
			...
			try {
				...
				throw new SyntaxException(PID.RATIONAL_OUT_OF_RANGE, getPosition(), getCurrentLine(),
	...
	public enum Keyword {
		END("end", Tokens.kEND, Tokens.kEND, LexState.EXPR_END), ELSE("else", Tokens.kELSE, Tokens.kELSE, LexState.EXPR_BEG), CASE("case",
				Tokens.kCASE, Tokens.kCASE,
				LexState.EXPR_BEG), ENSURE("ensure", Tokens.kENSURE, Tokens.kENSURE, LexState.EXPR_BEG), MODULE("module", Tokens.kMODULE,
						Tokens.kMODULE, LexState.EXPR_BEG), ELSIF("elsif", Tokens.kELSIF, Tokens.kELSIF, LexState.EXPR_BEG), DEF("def",
								Tokens.kDEF, Tokens.kDEF,
								LexState.EXPR_FNAME), RESCUE("rescue", Tokens.kRESCUE, Tokens.kRESCUE_MOD, LexState.EXPR_MID), NOT("not",
										Tokens.kNOT, Tokens.kNOT,
										LexState.EXPR_BEG), THEN("then", Tokens.kTHEN, Tokens.kTHEN, LexState.EXPR_BEG), YIELD("yield",
												Tokens.kYIELD, Tokens.kYIELD,
												LexState.EXPR_ARG), FOR("for", Tokens.kFOR, Tokens.kFOR, LexState.EXPR_BEG), SELF("self",
														Tokens.kSELF, Tokens.kSELF, LexState.EXPR_END), FALSE("false", Tokens.kFALSE,
																Tokens.kFALSE, LexState.EXPR_END), RETRY("retry", Tokens.kRETRY,
																		Tokens.kRETRY, LexState.EXPR_END), RETURN("return", Tokens.kRETURN,
																				Tokens.kRETURN, LexState.EXPR_MID), TRUE("true",
																						Tokens.kTRUE, Tokens.kTRUE, LexState.EXPR_END), IF(
																								"if", Tokens.kIF, Tokens.kIF_MOD,
																								LexState.EXPR_BEG), DEFINED_P("defined?",
																										Tokens.kDEFINED, Tokens.kDEFINED,
																										LexState.EXPR_ARG), SUPER("super",
																												Tokens.kSUPER,
																												Tokens.kSUPER,
																												LexState.EXPR_ARG), UNDEF(
																														"undef",
																														Tokens.kUNDEF,
																														Tokens.kUNDEF,
																														LexState.EXPR_FNAME), BREAK(
																																"break",
																																Tokens.kBREAK,
																																Tokens.kBREAK,
																																LexState.EXPR_MID), IN(
																																		"in",
																																		Tokens.kIN,
																																		Tokens.kIN,
																																		LexState.EXPR_BEG), DO(
																																				"do",
																																				Tokens.kDO,
																																				Tokens.kDO,
																																				LexState.EXPR_BEG), NIL(
																																						"nil",
																																						Tokens.kNIL,
																																						Tokens.kNIL,
																																						LexState.EXPR_END), UNTIL(
																																								"until",
																																								Tokens.kUNTIL,
																																								Tokens.kUNTIL_MOD,
																																								LexState.EXPR_BEG), UNLESS(
																																										"unless",
																																										Tokens.kUNLESS,
																																										Tokens.kUNLESS_MOD,
																																										LexState.EXPR_BEG), OR(
																																												"or",
																																												Tokens.kOR,
																																												Tokens.kOR,
																																												LexState.EXPR_BEG), NEXT(
																																														"next",
																																														Tokens.kNEXT,
																																														Tokens.kNEXT,
																																														LexState.EXPR_MID), WHEN(
																																																"when",
																																																Tokens.kWHEN,
																																																Tokens.kWHEN,
																																																LexState.EXPR_BEG), REDO(
																																																		"redo",
																																																		Tokens.kREDO,
																																																		Tokens.kREDO,
																																																		LexState.EXPR_END), AND(
																																																				"and",
																																																				Tokens.kAND,
																																																				Tokens.kAND,
																																																				LexState.EXPR_BEG), BEGIN(
																																																						"begin",
																																																						Tokens.kBEGIN,
																																																						Tokens.kBEGIN,
																																																						LexState.EXPR_BEG), __LINE__(
																																																								"__LINE__",
																																																								Tokens.k__LINE__,
																																																								Tokens.k__LINE__,
																																																								LexState.EXPR_END), CLASS(
																																																										"class",
																																																										Tokens.kCLASS,
																																																										Tokens.kCLASS,
																																																										LexState.EXPR_CLASS), __FILE__(
																																																												"__FILE__",
																																																												Tokens.k__FILE__,
																																																												Tokens.k__FILE__,
																																																												LexState.EXPR_END), LEND(
																																																														"END",
																																																														Tokens.klEND,
																																																														Tokens.klEND,
																																																														LexState.EXPR_END), LBEGIN(
																																																																"BEGIN",
																																																																Tokens.klBEGIN,
																																																																Tokens.klBEGIN,
																																																																LexState.EXPR_END), WHILE(
																																																																		"while",
																																																																		Tokens.kWHILE,
																																																																		Tokens.kWHILE_MOD,
																																																																		LexState.EXPR_BEG), ALIAS(
																																																																				"alias",
																																																																				Tokens.kALIAS,
																																																																				Tokens.kALIAS,
																																																																				LexState.EXPR_FNAME), __ENCODING__(
																																																																						"__ENCODING__",
																																																																						Tokens.k__ENCODING__,
																																																																						Tokens.k__ENCODING__,
																																																																						LexState.EXPR_END);
	...
	public enum LexState {
		EXPR_BEG, EXPR_END, EXPR_ARG, EXPR_CMDARG, EXPR_ENDARG, EXPR_MID, EXPR_FNAME, EXPR_DOT, EXPR_CLASS, EXPR_VALUE, EXPR_ENDFN
	}
	...
	/*
	 * In normal JRuby, there is a "spaceSeen" flag which is local to yylex. It is used to interpret
	 * input based on whether a space was recently seen. Since I now bail -out- of yylex() when I
	 * see space, I need to be able to preserve this flag across yylex() calls. In most cases,
	 * "spaceSeen" should be set to false (as it previous was at the beginning of yylex(). However,
	 * when I've seen a space and have bailed out, I need to set spaceSeen=true on the next call to
	 * yylex(). This is what the following flag is all about. It is set to true when we bail out on
	 * space (or other states that didn't previous bail out and spaceSeen is true).
	 */
	private boolean setSpaceSeen;

	/**
	 * Set whether or not the lexer should be "space preserving" - in other words, whether the
	 * parser should consider whitespace sequences and code comments to be separate tokens to return
	 * to the client. Parsers typically do not want to see any whitespace or comment tokens - but an
	 * IDE trying to tokenize a chunk of source code does want to identify these separately. The
	 * default, false, means the parser mode.
	 *
	 * @param preserveSpaces
	 *            If true, return space and comment sequences as tokens, if false, skip these
	...
	/**
	 * Return whether or not the lexer should be "space preserving". For a description of what this
	 * means, see {@link #setPreserveSpaces}.
	 *
	 * @return preserveSpaces True iff space and comment sequences will be returned as tokens, and
	 *         false otherwise.
	 *
	...
	public void reset() {
		...
		if (parserSupport != null)
			isTwoZero = parserSupport.getConfiguration().getVersion().is2_0();
	}
	...
	/**
	 * Get position information for Token/Node that follows node represented by startPosition and
	...
	/**
	 * Parse must pass its support object for some check at bottom of yylex(). Ruby does it this way
	 * as well (i.e. a little parsing logic in the lexer).
	 * 
	 * @param parserSupport
	 *            set support so lexer can use it
	 */
	...
	private boolean isBEG() {
		return lex_state == LexState.EXPR_BEG || lex_state == LexState.EXPR_MID || lex_state == LexState.EXPR_CLASS
				|| (!isOneEight && lex_state == LexState.EXPR_VALUE);
	}

	private boolean isEND() {
		return lex_state == LexState.EXPR_END || lex_state == LexState.EXPR_ENDARG || (!isOneEight && lex_state == LexState.EXPR_ENDFN);
	}
	 ...
	 * 
	 * @param c
	 *            first character the the quote construct
	...
	private int hereDocumentIdentifier() throws IOException {
		...
		if (c == '\'' || c == '"' || c == '`') {
			...
			if (c == EOF) {
				throw new SyntaxException(PID.STRING_MARKER_MISSING, getPosition(), getCurrentLine(),
	...
	/*
	 * This impl is a little sucky. We basically double scan the same bytelist twice. Once here and
	 * once in parseMagicComment.
	 */
	...
	/**
	 * Read a comment up to end of line. When found each comment will get stored away into the
	 * parser result so that any interested party can use them as they seem fit. One idea is that
	 * IDE authors can do distance based heuristics to associate these comments to the AST node they
	...
	private void addSyntax(SyntaxNode node) {
		...
		if (parserSupport != null)
			parserSupport.getResult().addSyntax(node);
	}

	/*
	 * Not normally used, but is left in here since it can be useful in debugging grammar and lexing
	 * problems.
	 *
	 */
	private void printToken(int token) {
		...
		case Tokens.tRATIONAL:
			System.err.print("tRATIONAL,");
			break;
		case Tokens.tIMAGINARY:
			System.err.print("tIMAGINARY,");
			break;
		...
		loop: for (;;) {
			...
			case '#': /* it's a comment */
				if (preserveSpaces) {
					...
					if (!(lex_state == LexState.EXPR_BEG || lex_state == LexState.EXPR_FNAME || lex_state == LexState.EXPR_DOT
					...
					if (src.matchMarker(BEGIN_DOC_MARKER, false, false)) {
						...
						if (Character.isWhitespace(c)) {
							...
							for (;;) {
								...
								if (c == EOF) {
									throw new SyntaxException(PID.STRING_HITS_EOF, getPosition(), getCurrentLine(),
								...
								if (src.wasBeginOfLine() && src.matchMarker(END_DOC_MARKER, false, false)) {
									if (collectComments())
										tokenBuffer.append(END_DOC_MARKER);
									String list = src.readLineBytes();
									if (collectComments())
										tokenBuffer.append(list);
									src.unread('\n');
									break;
								}
							}
	...
	private int identifierToken(LexState last_state, int result, String value) {
		...
		if (result == Tokens.tIDENTIFIER && last_state != LexState.EXPR_DOT && parserSupport != null
	...
	private int getIdentifier(int c) throws IOException {
		do {
			tokenBuffer.append(c);
			/*
			 * no special multibyte character handling is needed in Java if (ismbchar(c)) { int i,
			 * len = mbclen(c)-1;
			 * 
			 * for (i = 0; i < len; i++) { c = src.read(); tokenBuffer.append(c); } }
			 */
			c = src.read();
	...
	private int identifier(int c, boolean commandState) throws IOException {
		...
		if (!isOneEight && ((lex_state == LexState.EXPR_BEG && !commandState) || lex_state == LexState.EXPR_ARG
	...
	private int leftCurly() {
		...
		yaccValue = new Token("{", getPosition());
		if (!isOneEight && c != Tokens.tLBRACE)
			commandStart = true;
		return c;
	}
	...
	private int lessThan(boolean spaceSeen) throws IOException {
		int c = src.read();
		if (c == '<' && lex_state != LexState.EXPR_DOT && lex_state != LexState.EXPR_CLASS && !isEND() && (!isARG() || spaceSeen)) {
	...
	private int questionMark() throws IOException {
		...
		c = src.read();
		if (c == EOF)
			throw new SyntaxException(PID.INCOMPLETE_CHAR_SYNTAX, getPosition(), getCurrentLine(), "incomplete character syntax");
		...
		if (Character.isWhitespace(c)) {
			 ...
			 * support.unread(c); lexState = LexState.EXPR_BEG; return '?';
			 */
		...
		if (c == '0') {
			...
			case 'X': //  hexadecimal
				...
				if (tokenBuffer.length() == startLen) {
					throw new SyntaxException(PID.BAD_HEX_NUMBER, getPosition(), getCurrentLine(),
							"Hexadecimal number without hex-digits.");
				} else if (nondigit != '\0') {
					throw new SyntaxException(PID.TRAILING_UNDERSCORE_IN_NUMBER, getPosition(), getCurrentLine(),
			...
			case 'B': // binary
				...
				if (tokenBuffer.length() == startLen) {
					throw new SyntaxException(PID.EMPTY_BINARY_NUMBER, getPosition(), getCurrentLine(), "Binary number without digits.");
				} else if (nondigit != '\0') {
					throw new SyntaxException(PID.TRAILING_UNDERSCORE_IN_NUMBER, getPosition(), getCurrentLine(),
							"Trailing '_' in number.");
				}
			...
			case '8':
			case '9':
				throw new SyntaxException(PID.BAD_OCTAL_DIGIT, getPosition(), getCurrentLine(), "Illegal octal digit.");
			case '.':
			case 'e':
			case 'E':
	...
	private int getNumberToken(String number, boolean seen_e, boolean seen_point, int nondigit) throws IOException {
		...
		if (nondigit != '\0') {
			throw new SyntaxException(PID.TRAILING_UNDERSCORE_IN_NUMBER, getPosition(), getCurrentLine(), "Trailing '_' in number.");
		} else if (isFloat) {
		...
		if (src.peek('{')) { // handle \\u{...}
			do {
				...
				if (scanHexLiteral(buffer, 6, false, "invalid Unicode escape") > 0x10ffff) {
					throw new SyntaxException(PID.INVALID_ESCAPE_SYNTAX, getPosition(), getCurrentLine(),
			...
			int c = src.read();
			if (c != '}') {
				throw new SyntaxException(PID.INVALID_ESCAPE_SYNTAX, getPosition(), getCurrentLine(), "unterminated Unicode escape");
			}
		...
		if (src.peek('{')) { // handle \\u{...}
			do {
				...
				if (codepoint >= 0x80) {
					...
					if (stringLiteral)
						tokenAddMBC(codepoint, buffer);
				} else if (stringLiteral) {
					if (codepoint == 0 && symbolLiteral) {
						throw new SyntaxException(PID.INVALID_ESCAPE_SYNTAX, getPosition(), getCurrentLine(),
								"symbol cannot contain '\\u0000'");
					}
	...
	public int readEscape() throws IOException {
		...
		case 'x': // hex constant
			...
			if (i == 0) {
				throw new SyntaxException(PID.INVALID_ESCAPE_SYNTAX, getPosition(), getCurrentLine(), "Invalid escape character syntax");
			}
			return hexValue;
		case 'b': // backspace
			return '\010';
	 ...
	 * exception will be thrown. This will also return the codepoint as a value so codepoint ranges
	 * can be checked.
	 */
	private char scanHexLiteral(CStringBuilder buffer, int count, boolean strict, String errorMessage) throws IOException {
		int i = 0;
		char hexValue = '\0';
		...
		if (i == 0 || strict && count != i) {
			throw new SyntaxException(PID.INVALID_ESCAPE_SYNTAX, getPosition(), getCurrentLine(), errorMessage);
		}
	...
	/**
	 * Read up to count hexadecimal digits. If strict is provided then count number of hex digits
	 * must be present. If no digits can be read a syntax exception will be thrown.
	 */
