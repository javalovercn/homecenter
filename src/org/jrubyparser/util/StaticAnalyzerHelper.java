...
public class StaticAnalyzerHelper {
	...
	/**
	 * There are four possible conditions: 1. rhs: [b, c] - ListNode 2. rhs: [*b, c] - ArgsPush(b,
	 * c) 3. rhs: [c, *b] - ArgsCat(c, b) 4. rhs: [c, *b, d] - ArgsPush(ArgsCat(c, b), d)
	 * 
	 * For cases 2 and 3 we can additionally return the rhs splat as a single value if the argscat
	 * or argspush will end up completely being assigned into a rest of the lhs. These four cases
	 * can have any number of pre or post nodes (these examples only use one for simplicity). This
	 * method really just flattens the tree into pre, rest, and post. Now in actuality these three
	 * values will not neccesarily fit into the lhs in the same fashion.
	 *
	 * @param rhs
	...
	public static Node[] flattenRHSValues(Node rhs) {
		...
		// 1.8-only logic
		if (rhs.getNodeType() == NodeType.TOARYNODE)
			rhs = ((ToAryNode) rhs).getValue();
