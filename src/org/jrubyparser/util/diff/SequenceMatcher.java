...
/**
 * The SequenceMatcher class is used to produce a list of matching nodes. It does most of the work
...
public class SequenceMatcher {
	 ...
	 * @param oldNode
	 *            The original version of the node being checked.
	 */
	 ...
	 * <p>
	 * <code>isJunk</code> is an object which implements the {@link IsJunk} interface and the
	 * <code>#checkJunk()</code> method. <code>checkJunk</code> is a method which takes a Node and
	 * determines whether or not it should be compared against the other node.
	 * <p>
	 * We pass in two nodes. Later, we can use the <code>#setSequence</code>,
	 * <code>#setNewNode</code> and <code>#setOldNode</code> methods to change out one or both of
	 * the nodes and create a new set of matches.
	 *
	 * @param isJunk
	...
	/**
	 * Here we deal with the case where two nodes are mismatched. This can be due to the insertion
	 * or deletion of a node in the new source. We make an initial attempt to match them back up
	 * here.
	 *
	 * @param childNew
	 *            The current version of the node being diffed.
	 ...
	 *
	 * @param newNode
	 *            The Node from the new AST which was modified.
	 * @param oldNode
	 *            The original version of the Node, from the old AST.
	 */
	...
	protected void findChanges(Node newNode, Node oldNode) {
		...
		loopForNewNode: for (Node childNew : newNode.childNodes()) {
			childNew = stripOutNewlines(childNew);
			if (checkForJunk(childNew))
	...
	/**
	 * BlockNodes are problematic because they are often mismatched in source code where nodes have
	 * been moved around. However, they can't just be skipped like the NewLineNode, because they
	 * often have more than one child. This is how we deal with them.
	 *
	 * @param childNew
	 *            The current version of the node.
	 * @param childOld
	 ...
	 *
	 * @param node
	 *            A Node to be checked against provided callback.
	 * @return Returns a boolean indicating whether or not Node should be skipped.
	 */
	 ...
	 *
	 * @param node
	 *            node is a Node that needs checked to see if it is a Newline.
	...
	/**
	 * If a node has been moved, our initial diff will contain two changes: one insertion and one
	 * deletion. Here we match those up, then send their children back in to check for changes
	 * internal to the moved node. This is how we can handle a method which has been both moved and
	 * modified, something impossible for most text-based diff tools.
	 */
				...
				if (diffNodes.indexOf(change) != diffNodes.indexOf(newChange)) {
					if (oNode.isSame(nNode)) {
						if (diffNodes.contains(change)) {
							diffNodes.set(diffNodes.indexOf(change),
									new Change(nNode, calcComplexity(nNode), oNode, calcComplexity(oNode)));
