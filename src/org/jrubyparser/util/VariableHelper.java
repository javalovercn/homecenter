...
public class VariableHelper {
	public static boolean isParameterUsed(Node node, String name, boolean isMethod) {
		if (node == null)
			return false; // Empty methods have null bodies (other nodes may also pass in null)

		for (Node child : node.childNodes()) {
			if (child instanceof INameMatchable && child instanceof ILocalVariable && ((INameMatchable) child).isNameMatch(name)) {
			...
			if (isParameterUsed(child, name, isMethod))
				return true;
		}
	...
	/**
	 * Can we find a parameter named by name in the arguments node (presumably from an iter or
	 * methoddef)?
	 *
	 * @param argsNode
	 *            to get name from
	...
	private static ILocalVariable getParameterNameInner(Node node, String name) {
		for (Node child : node.childNodes()) {
			if (child instanceof ILocalScope)
				return null; // Any nested scope in an ARGS context will stop the search
			if (child instanceof ILocalVariable && ((INameMatchable) child).isNameMatch(name))
				return (ILocalVariable) child;
