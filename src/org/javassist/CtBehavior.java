...
/**
 * <code>CtBehavior</code> represents a method, a constructor, or a static constructor (class
 * initializer). It is the abstract super class of <code>CtMethod</code> and
...
public abstract class CtBehavior extends CtMember {
	 ...
	 * <code>org.javassist.CtBehavior.stBody(String)</code>.
	 *
	 * @since 3.5
	 */
	...
	/**
	 * Returns the <code>MethodInfo</code> representing this method/constructor in the class file.
	 *
	 * <p>
	 * If you modify the bytecode through the returned <code>MethodInfo</code> object, you might
	 * have to explicitly rebuild a stack map table. Javassist does not automatically rebuild it for
	 * avoiding unnecessary rebuilding.
	 *
	 * @see org.javassist.bytecode.MethodInfo#rebuildStackMap(ClassPool)
	 */
	...
	/**
	 * Returns the <code>MethodInfo</code> representing the method/constructor in the class file
	 * (read only). Normal applications do not need calling this method. Use
	 * <code>getMethodInfo()</code>.
	 *
	 * <p>
	 * The <code>MethodInfo</code> object obtained by this method is read only. Changes to this
	 ...
	 *
	 * <p>
	 * This method is available even if the <code>CtClass</code> containing this method is frozen.
	 * However, if the class is frozen, the <code>MethodInfo</code> might be also pruned.
	 *
	 * @see #getMethodInfo()
	 ...
	 *
	 * <p>
	 * Changing the modifiers may cause a problem. For example, if a non-static method is changed to
	 * static, the method will be rejected by the bytecode verifier.
	 *
	 * @see Modifier
	 */
	 ...
	 * annotation <code>@Author</code> is associated with this method/constructor, an
	 * <code>Author</code> object is returned. The member values can be obtained by calling methods
	...
	/**
	 * Returns the annotations associated with this method or constructor. If any annotations are
	 * not on the classpath, they are not included in the returned array.
	 * 
	 * @return an array of annotation-type objects.
	 * @see #getAnnotations()
	 * @since 3.3
	 ...
	 *
	 * @return an array of annotation-type objects. The length of the returned array is equal to the
	 *         number of the formal parameters. If each parameter has no annotation, the elements of
	 *         the returned array are empty arrays.
	 *
	...
	/**
	 * Returns the parameter annotations associated with this method or constructor. If any
	 * annotations are not on the classpath, they are not included in the returned array.
	 * 
	 * @return an array of annotation-type objects. The length of the returned array is equal to the
	 *         number of the formal parameters. If each parameter has no annotation, the elements of
	 *         the returned array are empty arrays.
	 ...
	 * specification. If two methods/constructors have the same parameter types and the return type,
	 * <code>getSignature()</code> returns the same string (the return type of constructors is
	 * <code>void</code>).
	 *
	 * <p>
	...
	/**
	 * Returns the generic signature of the method. It represents parameter types including type
	 * variables.
	 *
	 ...
	 * variables. See {@link org.javassist.CtClass#setGenericSignature(String)} for a code sample.
	 *
	 * @param sig
	 ...
	 *
	 * @param src
	 *            the source code representing the body. It must be a single statement or block. If
	 *            it is <code>null</code>, the substituted body does nothing except returning zero
	 *            or null.
	 */
	 ...
	 *
	 * <p>
	 * Note that an attribute is a data block specified by the class file format. It is not an
	 * annotation. See {@link org.javassist.bytecode.AttributeInfo}.
	 *
	 * @param name
	 *            attribute name
	 */
	...
	/**
	 * Declares to use <code>$cflow</code> for this method/constructor. If <code>$cflow</code> is
	 * used, the class files modified with Javassist requires a support class
	 * <code>org.javassist.runtime.Cflow</code> at runtime (other Javassist classes are not required
	 * at runtime).
	 *
	 * <p>
	 * Every <code>$cflow</code> variable is given a unique name. For example, if the given name is
	 * <code>"Point.paint"</code>, then the variable is indicated by
	 * <code>$cflow(Point.paint)</code>.
	 *
	 * @param name
	 *            <code>$cflow</code> name. It can include alphabets, numbers, <code>_</code>,
	 *            <code>$</code>, and <code>.</code> (dot).
	 *
	 * @see org.javassist.runtime.Cflow
	 */
	public void useCflow(String name) throws CannotCompileException {
		...
		try {
			CtClass type = pool.get("org.javassist.runtime.Cflow");
	...
	/**
	 * Declares a new local variable. The scope of this variable is the whole method body. The
	 * initial value of that variable is not set. The declared variable can be accessed in the code
	 * snippet inserted by <code>insertBefore()</code>, <code>insertAfter()</code>, etc.
	 *
	 * <p>
	 * If the second parameter <code>asFinally</code> to <code>insertAfter()</code> is true, the
	 * declared local variable is not visible from the code inserted by <code>insertAfter()</code>.
	 *
	 * @param name
	 ...
	 *
	 * <p>
	 * While executing this method, only <code>replace()</code> in <code>Expr</code> is available
	 * for bytecode modification. Other methods such as <code>insertBefore()</code> may collapse the
	 * bytecode because the <code>ExprEditor</code> loses its current position.
	 *
	 * @param editor
	 *            specifies how to modify.
	 * @see org.javassist.expr.Expr#replace(String)
	 * @see #insertBefore(String)
	 */
	 ...
	 *
	 * <p>
	 * If this object represents a constructor, the bytecode is inserted before a constructor in the
	 * super class or this class is called. Therefore, the inserted bytecode is subject to
	 * constraints described in Section 4.8.2 of The Java Virtual Machine Specification (2nd ed).
	 * For example, it cannot access instance fields or methods although it may assign a value to an
	 * instance field directly declared in this class. Accessing static fields and methods is
	 * allowed. Use <code>insertBeforeBody()</code> in <code>CtConstructor</code>.
	 *
	 * @param src
	...
	/**
	 * Inserts bytecode at the end of the body. The bytecode is inserted just before every return
	 * insturction. It is not executed when an exception is thrown.
	 *
	 * @param src
	 *            the source code representing the inserted bytecode. It must be a single statement
	 *            or block.
	 ...
	 *            or block.
	 * @param asFinally
	 *            true if the inserted bytecode is executed not only when the control normally
	 *            returns but also when an exception is thrown. If this parameter is true, the
	 *            inserted code cannot access local variables.
	 */
	 ...
	 * 
	 * public void insertAfter(String src) throws CannotCompileException {
	 * declaringClass.checkModify(); CodeAttribute ca = methodInfo.getCodeAttribute(); CodeIterator
	 * iterator = ca.iterator(); Bytecode b = new Bytecode(methodInfo.getConstPool(),
	 * ca.getMaxStack(), ca.getMaxLocals()); b.setStackDepth(ca.getMaxStack()); Javac jv = new
	 * Javac(b, declaringClass); try { jv.recordParams(getParameterTypes(),
	 * Modifier.isStatic(getModifiers())); CtClass rtype = getReturnType0(); int varNo =
	 * jv.recordReturnType(rtype, true); boolean isVoid = rtype == CtClass.voidType; if (isVoid) {
	 * b.addOpcode(Opcode.ACONST_NULL); b.addAstore(varNo); jv.compileStmnt(src); } else {
	 * b.addStore(varNo, rtype); jv.compileStmnt(src); b.addLoad(varNo, rtype); }
	 * 
	 * byte[] code = b.get(); ca.setMaxStack(b.getMaxStack()); ca.setMaxLocals(b.getMaxLocals());
	 * while (iterator.hasNext()) { int pos = iterator.next(); int c = iterator.byteAt(pos); if (c
	 * == Opcode.ARETURN || c == Opcode.IRETURN || c == Opcode.FRETURN || c == Opcode.LRETURN || c
	 * == Opcode.DRETURN || c == Opcode.RETURN) iterator.insert(pos, code); } } catch
	 * (NotFoundException e) { throw new CannotCompileException(e); } catch (CompileError e) { throw
	 * new CannotCompileException(e); } catch (BadBytecode e) { throw new CannotCompileException(e);
	 * } }
	 */

	/**
	 * Adds a catch clause that handles an exception thrown in the body. The catch clause must end
	 * with a return or throw statement.
	 *
	 * @param src
	 *            the source code representing the catch clause. It must be a single statement or
	 *            block.
	 * @param exceptionType
	 *            the type of the exception handled by the catch clause.
	 */
	 ...
	 *            the type of the exception handled by the catch clause.
	 * @param exceptionName
	 *            the name of the variable containing the caught exception, for example,
	 *            <code>$e</code>.
	 */
	 ...
	 *
	 * @param lineNum
	 *            the line number. The bytecode is inserted at the beginning of the code at the line
	 *            specified by this line number.
	 * @param src
	 *            the source code representing the inserted bytecode. It must be a single statement
	 *            or block.
	 ...
	 *
	 * <p>
	 * If there is not a statement at the specified line, the bytecode might be inserted at the line
	 * including the first statement after that line specified. For example, if there is only a
	 * closing brace at that line, the bytecode would be inserted at another line below. To know
	 * exactly where the bytecode will be inserted, call with <code>modify</code> set to
	 * <code>false</code>.
	 *
	 * @param lineNum
	 *            the line number. The bytecode is inserted at the beginning of the code at the line
	 *            specified by this line number.
	 * @param modify
	 *            if false, this method does not insert the bytecode. It instead only returns the
	 *            line number at which the bytecode would be inserted.
	 * @param src
	 *            the source code representing the inserted bytecode. It must be a single statement
	 *            or block. If modify is false, the value of src can be null.
	 * @return the line number at which the bytecode has been inserted.
	 */
	public int insertAt(int lineNum, boolean modify, String src) throws CannotCompileException {
		...
		try {
			...
			/*
			 * We assume that there is no values in the operand stack at the position where the
			 * bytecode is inserted.
			 */
			if (stack > ca.getMaxStack())
				ca.setMaxStack(stack);
