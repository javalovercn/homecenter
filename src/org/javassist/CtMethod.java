 ...
 *
 * <p>
 * See the super class <code>CtBehavior</code> since a number of useful methods are in
 * <code>CtBehavior</code>. A number of useful factory methods are in <code>CtNewMethod</code>.
 *
...
public final class CtMethod extends CtBehavior {
	...
	/**
	 * Creates a public abstract method. The created method must be added to a class with
	 * <code>CtClass.addMethod()</code>.
	 *
	 * @param declaring
	...
	/**
	 * Creates a copy of a <code>CtMethod</code> object. The created method must be added to a class
	 * with <code>CtClass.addMethod()</code>.
	 *
	 * <p>
	 * All occurrences of class names in the created method are replaced with names specified by
	 ...
	 *
	 * <p>
	 * (<code>X</code> is a class name.) If <code>map</code> substitutes <code>String</code> for
	 * <code>X</code>, then the created method is:
	 *
	 * <pre>
	 ...
	 *
	 * <p>
	 * By default, all the occurrences of the names of the class declaring <code>at()</code> and the
	 * superclass are replaced with the name of the class and the superclass that the created method
	 * is added to. This is done whichever <code>map</code> is null or not. To prevent this
	 * replacement, call <code>ClassMap.fix()</code> or <code>put()</code> to explicitly specify
	 * replacement.
	 *
	 * <p>
	...
	/**
	 * Compiles the given source code and creates a method. This method simply delegates to
	 * <code>make()</code> in <code>CtNewMethod</code>. See it for more details.
	 ...
	 *             if the the <code>MethodInfo</code> object and the declaring class have different
	...
	/**
	 * Returns a hash code value for the method. If two methods have the same name and signature,
	 * then the hash codes for the two methods are equal.
	 */
	public int hashCode() {
	 ...
	 * <code>org.javassist.CtMethod.setBody(String)</code>.
	 *
	 * @since 3.5
	 */
	...
	/**
	 * Returns true if the method body is empty, that is, <code>{}</code>. It also returns true if
	...
	/**
	 * Copies a method body from another method. If this method is abstract, the abstract modifier
	 * is removed after the method body is copied.
	 *
	 * <p>
	 * All occurrences of the class names in the copied method body are replaced with the names
	 * specified by <code>map</code> if <code>map</code> is not <code>null</code>.
	 *
	 * @param src
	 ...
	 *            the wrapped method
	 * @param constParam
	 *            the constant parameter given to the wrapped method (maybe <code>null</code>).
	 *
	...
	/**
	 * Instances of this class represent a constant parameter. They are used to specify the
	 * parameter given to the methods created by <code>CtNewMethod.wrapped()</code>.
	 *
