...
import org.javassist.bytecode.*;
import org.javassist.convert.*;

/**
 * Simple translator of method bodies (also see the <code>org.javassist.expr</code> package).
 *
 * <p>
 * Instances of this class specifies how to instrument of the bytecodes representing a method body.
 * They are passed to <code>CtClass.instrument()</code> or <code>CtMethod.instrument()</code> as a
 * parameter.
 *
 * <p>
 * Example:
 ...
 *
 * <p>
 * This program substitutes "<code>Singleton.makePoint()</code>" for all occurrences of
 * "<code>new Point()</code>" appearing in methods declared in a <code>Client</code> class.
 *
 * @see org.javassist.CtClass#instrument(CodeConverter)
 * @see org.javassist.CtMethod#instrument(CodeConverter)
 * @see org.javassist.expr.ExprEditor
 */
public class CodeConverter {
	protected Transformer transformers = null;

	/**
	 * Modify a method body so that instantiation of the specified class is replaced with a call to
	 * the specified static method. For example,
	 * <code>replaceNew(ctPoint, ctSingleton, "createPoint")</code> (where <code>ctPoint</code> and
	 * <code>ctSingleton</code> are compile-time classes for class <code>Point</code> and class
	 * <code>Singleton</code>, respectively) replaces all occurrences of:
	 *
	 * <pre>
	 * new Point(x, y)
	 * </pre>
	 ...
	 *
	 * <p>
	 * This enables to intercept instantiation of <code>Point</code> and change the samentics. For
	 * example, the following <code>createPoint()</code> implements the singleton pattern:
	 *
	 * <pre>
	 ...
	 *
	 * <p>
	 * The static method call substituted for the original <code>new</code> expression must be able
	 * to receive the same set of parameters as the original constructor. If there are multiple
	 * constructors with different parameter types, then there must be multiple static methods with
	 * the same name but different parameter types.
	 *
	 * <p>
	 * The return type of the substituted static method must be the exactly same as the type of the
	 * instantiated class specified by <code>newClass</code>.
	 *
	 * @param newClass
	...
	/**
	 * Modify a method body so that instantiation of the class specified by <code>oldClass</code> is
	 * replaced with instantiation of another class <code>newClass</code>. For example,
	 * <code>replaceNew(ctPoint, ctPoint2)</code> (where <code>ctPoint</code> and
	 * <code>ctPoint2</code> are compile-time classes for class <code>Point</code> and class
	 * <code>Point2</code>, respectively) replaces all occurrences of:
	 *
	 * <pre>
	 * new Point(x, y)
	 * </pre>
	 ...
	 *
	 * <p>
	 * Note that <code>Point2</code> must be type-compatible with <code>Point</code>. It must have
	 * the same set of methods, fields, and constructors as the replaced class.
	 */
	...
	/**
	 * Modify a method body so that field read/write expressions access a different field from the
	 * original one.
	 *
	 * <p>
	 * Note that this method changes only the filed name and the class declaring the field; the type
	 * of the target object does not change. Therefore, the substituted field must be declared in
	 * the same class or a superclass of the original class.
	 *
	 * <p>
	 * Also, <code>clazz</code> and <code>newClass</code> must specify the class directly declaring
	 * the field. They must not specify a subclass of that class.
	 *
	 * @param field
	...
	/**
	 * Modify a method body so that an expression reading the specified field is replaced with a
	 * call to the specified <i>static</i> method. This static method receives the target object of
	 * the original read expression as a parameter. It must return a value of the same type as the
	 * field.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * The type of the parameter of <code>readX()</code> must be <code>java.lang.Object</code>
	 * independently of the actual type of <code>target</code>. The return type must be the same as
	 * the field type.
	 *
	 * @param field
	 *            the field.
	...
	/**
	 * Modify a method body so that an expression writing the specified field is replaced with a
	 * call to the specified static method. This static method receives two parameters: the target
	 * object of the original write expression and the assigned value. The return type of the static
	 ...
	 * <code>java.lang.Object</code> independently of the actual type of <code>target</code>. The
	 * type of the second parameter is the same as the field type.
	 *
	 * @param field
	 *            the field.
	 * @param calledClass
	...
	/**
	 * Modify a method body, so that ALL accesses to an array are replaced with calls to static
	 * methods within another class. In the case of reading an element from the array, this is
	 * replaced with a call to a static method with the array and the index as arguments, the return
	 * value is the value read from the array. If writing to an array, this is replaced with a call
	 * to a static method with the array, index and new value as parameters, the return value of the
	 ...
	 * require a different method than if writing to an <code>int[]</code>, and writing to a
	 * <code>long[]</code> will require a different method than if writing to a <code>byte[]</code>.
	 * If the implementation of <code>ArrayAccessReplacementMethodNames</code> does not contain the
	 * name for access for a type of array, that access is not replaced.
	 * 
	 * <p>
	 * A default implementation of <code>ArrayAccessReplacementMethodNames</code> called
	 * <code>DefaultArrayAccessReplacementMethodNames</code> has been provided and is what is used
	 * in the following example. This also assumes that <code>'foo.ArrayAdvisor'</code> is the name
	 * of the <code>CtClass</code> passed in.
	 * 
	 * <p>
	 ...
	 * @param names
	 *            contains the names of the methods to replace the different kinds of array access
	 *            with.
	 */
	 ...
	 *
	 * <p>
	 * Note that the target object, the parameters, or the type of invocation (static method call,
	 * interface call, or private method call) are not modified. Only the method name is changed.
	 * The substituted method must have the same signature that the original one has. If the
	 * original method is a static method, the substituted method must be static.
	 *
	 * @param origMethod
	 *            original method
	 * @param substMethod
	...
	/**
	 * Correct invocations to a method that has been renamed. If a method is renamed, calls to that
	 * method must be also modified so that the method with the new name will be called.
	 *
	 * <p>
	 ...
	 * @see org.javassist.CtMethod#setName(String)
	 */
	...
	/**
	 * Insert a call to another method before an existing method call. That "before" method must be
	 * static. The return type must be <code>void</code>. As parameters, the before method receives
	 * the target object and all the parameters to the originally invoked method. For example, if
	 * the originally invoked method is <code>move()</code>:
	 *
	 * <pre>
	 * class Point {
	...
	/**
	 * Inserts a call to another method after an existing method call. That "after" method must be
	 * static. The return type must be <code>void</code>. As parameters, the after method receives
	 * the target object and all the parameters to the originally invoked method. For example, if
	 * the originally invoked method is <code>move()</code>:
	 *
	 * <pre>
	...
	public interface ArrayAccessReplacementMethodNames {
		...
		/**
		 * Returns the name of a static method with the signature
		 * <code>(Ljava/lang/Object;I)Ljava/lang/Object;</code> to replace reading from a Object[]
		...
		/**
		 * Returns the name of a static method with the signature
		 * <code>(Ljava/lang/Object;ILjava/lang/Object;)V</code> to replace writing to a Object[]
		 * (or any subclass of object).
		 */
		String objectWrite();
	...
	/**
	 * Default implementation of the <code>ArrayAccessReplacementMethodNames</code> interface giving
	 * default values for method names to be used for replacing accesses to array elements.
	 *
	 * @author <a href="kabir.khan@jboss.com">Kabir Khan</a>
	 * @version $Revision: 1.2 $
	 */
	public static class DefaultArrayAccessReplacementMethodNames implements ArrayAccessReplacementMethodNames {
		 ...
		 * (Ljava/lang/Object;I)Ljava/lang/Object; to replace reading from a Object[] (or any
		 * subclass of object).
		 */
		 ...
		 * (Ljava/lang/Object;ILjava/lang/Object;)V to replace writing to a Object[] (or any
		 * subclass of object).
		 */
