...
import org.javassist.bytecode.AnnotationsAttribute;
import org.javassist.bytecode.AttributeInfo;
...
import org.javassist.bytecode.ConstantAttribute;
import org.javassist.bytecode.Descriptor;
import org.javassist.bytecode.EnclosingMethodAttribute;
import org.javassist.bytecode.FieldInfo;
import org.javassist.bytecode.InnerClassesAttribute;
import org.javassist.bytecode.MethodInfo;
import org.javassist.bytecode.ParameterAnnotationsAttribute;
import org.javassist.bytecode.SignatureAttribute;
import org.javassist.bytecode.annotation.Annotation;
import org.javassist.compiler.AccessorMaker;
...
class CtClassType extends CtClass {
		...
		try {
			...
			if (!cf.getName().equals(qualifiedName))
				throw new RuntimeException(
						"cannot find " + qualifiedName + ": " + cf.getName() + " found in " + qualifiedName.replace('.', '/') + ".class");
	 ...
	 *
	 * @see org.javassist.CtClass#incGetCounter()
	 * 
	...
	/**
	 * Invoked from ClassPool#compress(). It releases the class files that have not been recently
	 * used if they are unmodified.
	 */
	void compress() {
		...
		try {
			...
			try {
				...
				try {
					...
					return org.javassist.bytecode.annotation.AnnotationImpl.make(clazz.getClassLoader(), clazz, cp, anno);
				} catch (Throwable e3) {
	...
	/**
	 * @see org.javassist.CtClass#prune()
	 * @see org.javassist.CtClass#stopPruning(boolean)
	 */
	public void prune() {
		if (wasPruned)
			return;
