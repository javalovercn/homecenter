...
import org.javassist.compiler.JvstCodeGen;
...
class CtNewWrappedMethod {
	...
	static Bytecode makeBody(CtClass clazz, ClassFile classfile, CtMethod wrappedBody, CtClass[] parameters, CtClass returnType,
	...
	/*
	 * The generated method body does not need a stack map table because it does not contain a
	 * branch instruction.
	 */
	protected static int makeBody0(CtClass clazz, ClassFile classfile, CtMethod wrappedBody, boolean isStatic, CtClass[] parameters,
			CtClass returnType, ConstParameter cparam, Bytecode code) throws CannotCompileException {
		...
		try {
			...
			/*
			 * if an exception is thrown below, the method added above should be removed. (future
			 * work :<)
			 */
		} catch (BadBytecode e) {
			throw new CannotCompileException(e);
	...
	private static void checkSignature(CtMethod wrappedBody, String descriptor) throws CannotCompileException {
		if (!descriptor.equals(wrappedBody.getMethodInfo2().getDescriptor()))
			throw new CannotCompileException(
					"wrapped method with a bad signature: " + wrappedBody.getDeclaringClass().getName() + '.' + wrappedBody.getName());
	}
