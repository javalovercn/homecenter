...
import org.javassist.CodeConverter.ArrayAccessReplacementMethodNames;
...
import org.javassist.bytecode.analysis.Analyzer;
import org.javassist.bytecode.analysis.Frame;
...
public final class TransformAccessArrayField extends Transformer {
	...
	public void initialize(ConstPool cp, CtClass clazz, MethodInfo minfo) throws CannotCompileException {
		/*
		 * This transformer must be isolated from other transformers, since some of them affect the
		 * local variable and stack maximums without updating the code attribute to reflect the
		 * changes. This screws up the data-flow analyzer, since it relies on consistent code state.
		 * Even if the attribute values were updated correctly, we would have to detect it, and redo
		 * analysis, which is not cheap. Instead, we are better off doing all changes in
		 * initialize() before everyone else has a chance to muck things up.
		 */
