...
import org.javassist.bytecode.AccessFlag;
...
import org.javassist.bytecode.ExceptionTable;
import org.javassist.bytecode.ExceptionsAttribute;
...
public abstract class Expr implements Opcode {
	...
	public CtBehavior where() {
		 ...
		 * cached in a CtClass object, it might be recreated for some reason. Thus, the member name
		 * and the signature must be also checked.
		 */
		for (int i = cb.length - 1; i >= 0; --i) {
	...
	/**
	 * Returns the list of exceptions that the expression may throw. This list includes both the
	 * exceptions that the try-catch statements including the expression can catch and the
	 * exceptions that the throws declaration allows the method to throw.
	 */
	...
	/**
	 * Returns the index of the bytecode corresponding to the expression. It is the index into the
	 * byte array containing the Java bytecode that implements the method.
	 */
	...
	/*
	 * If isStaticCall is true, null is assigned to $0. So $0 must be declared by calling
	 * Javac.recordParams().
	 * 
	 * After executing this method, the current stack depth might be less than 0.
	 */
	 ...
	 *            a Java statement except try-catch.
	 * @param recursive
	 *            if not null, the substituted bytecode is recursively processed by the given
	 *            <code>ExprEditor</code>.
	 * @since 3.1
	 */
