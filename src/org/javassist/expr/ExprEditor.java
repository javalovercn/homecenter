 ...
 *
 * <p>
 * The users can define a subclass of this class to customize how to modify a method body. The
 * overall architecture is similar to the strategy pattern.
 *
 * <p>
 * If <code>instrument()</code> is called in <code>CtMethod</code>, the method body is scanned from
 * the beginning to the end. Whenever an expression, such as a method call and a <tt>new</tt>
 * expression (object creation), is found, <code>edit()</code> is called in <code>ExprEdit</code>.
 * <code>edit()</code> can inspect and modify the given expression. The modification is reflected on
 * the original method body. If <code>edit()</code> does nothing, the original method body is not
 * changed.
 *
 * <p>
 ...
 *
 * <p>
 * This code inspects all method calls appearing in the method represented by <code>cm</code> and it
 * prints the names and the line numbers of the methods declared in class <code>Point</code>. This
 * code does not modify the body of the method represented by <code>cm</code>. If the method body
 * must be modified, call <code>replace()</code> in <code>MethodCall</code>.
 *
 * @see org.javassist.CtClass#instrument(ExprEditor)
 * @see org.javassist.CtMethod#instrument(ExprEditor)
 * @see org.javassist.CtConstructor#instrument(ExprEditor)
 * @see MethodCall
 * @see NewExpr
 * @see FieldAccess
 *
...
public class ExprEditor {
	...
	/**
	 * Edits a <tt>new</tt> expression (overridable). The default implementation performs nothing.
	 *
	 * @param e
	...
	/**
	 * Edits an expression for array creation (overridable). The default implementation performs
	 * nothing.
	 *
	 * @param a
	...
	/**
	 * Edits a constructor call (overridable). The constructor call is either <code>super()</code>
	 * or <code>this()</code> included in a constructor body.
	 *
	 * The default implementation performs nothing.
	 *
	 * @see #edit(NewExpr)
	...
	/**
	 * Edits a field-access expression (overridable). Field access means both read and write. The
	 * default implementation performs nothing.
	 */
	...
	/**
	 * Edits an expression for explicit type casting (overridable). The default implementation
	 * performs nothing.
	 */
