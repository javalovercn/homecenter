...
/* Code generator methods depending on org.javassist.* classes.
 */
public class MemberCodeGen extends CodeGen {
	...
	private void atMethodCallCore2(CtClass targetClass, String mname, boolean isStatic, boolean isSpecial, int aload0pos,
			MemberResolver.Method found) throws CompileError {
		...
		if ((acc & AccessFlag.STATIC) != 0) {
			if (!isStatic) {
				/*
				 * this method is static but the target object is on stack. It must be popped out.
				 * If aload0pos >= 0, then the target object was pushed by aload_0. It is
				 * overwritten by NOP.
				 */
				isStatic = true;
				if (aload0pos >= 0)
	...
	/*
	 * Finds (or adds if necessary) a hidden accessor if the method is in an enclosing class.
	 *
	...
	/*
	 * Finds (or adds if necessary) a hidden constructor if the given constructor is in an enclosing
	 * class.
	 *
	...
	/**
	 * Generates bytecode for reading a field value. It returns a fieldref_info index or zero if the
	 * field is a private one declared in an enclosing class.
	 */
	...
	/**
	 * Returns null if the field is accessible. Otherwise, it throws an exception or it returns
	 * AccessorMaker if the field is a private one declared in an enclosing class.
	 */
	...
	protected CtField fieldAccess(ASTree expr, boolean acceptLength) throws CompileError {
		if (expr instanceof Member) {
			...
			if (op == MEMBER) {
				/*
				 * static member by # (extension by Javassist) For example, if int.class is parsed,
				 * the resulting tree is (# "java.lang.Integer" "TYPE").
				 */
				...
				try {
					...
					/*
					 * Don't call lookupFieldByJvmName2(). The left operand of . is not a class name
					 * but a normal expression.
					 */
