...
public abstract class CodeGen extends Visitor implements Opcode, TokenId {
	...
	/**
	 * @param isCons
	 *            true if super() must be called. false if the method is a class initializer.
	 */
	...
	static final int[] binOp = { '+', DADD, FADD, LADD, IADD, '-', DSUB, FSUB, LSUB, ISUB, '*', DMUL, FMUL, LMUL, IMUL, '/', DDIV, FDIV,
			LDIV, IDIV, '%', DREM, FREM, LREM, IREM, '|', NOP, NOP, LOR, IOR, '^', NOP, NOP, LXOR, IXOR, '&', NOP, NOP, LAND, IAND, LSHIFT,
			NOP, NOP, LSHL, ISHL, RSHIFT, NOP, NOP, LSHR, ISHR, ARSHIFT, NOP, NOP, LUSHR, IUSHR };
	...
	/*
	 * Produces the opcode to branch if the condition is true. The oprand (branch offset) is not
	 * produced.
	 *
	 * @return true if the compiled code is GOTO (always branch). GOTO is not produced.
	 */
	...
	private static final int ifOp[] = { EQ, IF_ICMPEQ, IF_ICMPNE, NEQ, IF_ICMPNE, IF_ICMPEQ, LE, IF_ICMPLE, IF_ICMPGT, GE, IF_ICMPGE,
			IF_ICMPLT, '<', IF_ICMPLT, IF_ICMPGE, '>', IF_ICMPGT, IF_ICMPLE };

	private static final int ifOp2[] = { EQ, IFEQ, IFNE, NEQ, IFNE, IFEQ, LE, IFLE, IFGT, GE, IFGE, IFLT, '<', IFLT, IFGE, '>', IFGT,
			IFLE };

	/*
	 * Produces the opcode to branch if the condition is true. The oprands are not produced.
	 *
	...
	private static final int[] castOp = {
			/* D F L I */
			/* double */ NOP, D2F, D2L, D2I, /* float */ F2D, NOP, F2L, F2I, /* long */ L2D, L2F, NOP, L2I,
			/* other */ I2D, I2F, I2L, NOP };

	/*
	 * do implicit type conversion. arrayDim values of the two oprands must be zero.
	 */
	...
	public void atExpr(Expr expr) throws CompileError {
		...
		if (token == '.') {
			...
			/*
			 * MEMBER ('#') is an extension by Javassist. The compiler internally uses # for
			 * compiling .class expressions such as "int.class".
			 */
			atFieldRead(expr);
		} else if (token == ARRAY)
		...
		if (cname.startsWith("[")) {
			...
			if (i >= 0) {
				...
				if (!name.equals(name2)) {
					/*
					 * For example, to obtain String[].class, "[Ljava.lang.String;" (not
					 * "[Ljava/lang/String"!) must be passed to Class.forName().
					 */
	...
	protected void atClassObject2(String cname) throws CompileError {
		 ...
		 * 
		 * bytecode.addNew("java.lang.NoClassDefFoundError"); bytecode.addOpcode(DUP);
		 * bytecode.addAload(var); bytecode.addInvokevirtual("java.lang.ClassNotFoundException",
		...
		bytecode.addInvokestatic("org.javassist.runtime.DotClass", "fail",
