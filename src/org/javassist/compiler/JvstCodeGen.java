...
public class JvstCodeGen extends MemberCodeGen {
	...
	/*
	 * If the type of the expression compiled last is void, add ACONST_NULL and change exprType,
	 * arrayDim, className.
	 */
	...
	public void atMember(Member mem) throws CompileError {
		...
		if (name.equals(paramArrayName)) {
			...
			bytecode.addInvokestatic("org.javassist/runtime/Desc", "getParams", "(Ljava/lang/String;)[Ljava/lang/Class;");
			exprType = CLASS;
			arrayDim = 1;
	...
	private void callGetType(String method) {
		bytecode.addInvokestatic("org.javassist/runtime/Desc", method, "(Ljava/lang/String;)Ljava/lang/Class;");
		exprType = CLASS;
		arrayDim = 0;
		className = "java/lang/Class";
	}
	...
	/*
	 * Delegates to a ProcHandler object if the method call is $proceed(). It may process $cflow().
	 */
	...
	protected void atCflow(ASTList cname) throws CompileError {
		...
		bytecode.addInvokevirtual("org.javassist.runtime.Cflow", "value", "()I");
		exprType = INT;
		arrayDim = 0;
		className = null;
	}
	 ...
	 * Syntax:
	 *
	 * <cflow> : $cflow '(' <cflow name> ')' <cflow name> : <identifier> ('.' <identifier>)*
	 */
	...
	/*
	 * To support $$. ($$) is equivalent to ($1, ..., $n). It can be used only as a parameter list
	 * of method call.
	 */
	...
	/*
	 * public int getMethodArgsLength(ASTList args) { if (!isParamListName(args)) return
	 * super.getMethodArgsLength(args);
	 * 
	...
	/*
	 * public void atMethodArgs(ASTList args, int[] types, int[] dims, String[] cnames) throws
	 * CompileError { if (!isParamListName(args)) { super.atMethodArgs(args, types, dims, cnames);
	 * return; }
	 * 
	 * CtClass[] params = paramTypeList; if (params == null) return;
	 * 
	 * int n = params.length; int regno = indexOfParam1(); for (int i = 0; i < n; ++i) { CtClass p =
	 * params[i]; regno += bytecode.addLoad(regno, p); setType(p); types[i] = exprType; dims[i] =
	 * arrayDim; cnames[i] = className; } }
	 */
	...
	/**
	 * Makes method parameters $0, $1, ..., $args, $$, and $class available. $0 is equivalent to
	 * THIS if the method is not static. Otherwise, if the method is static, then $0 is not
	 * available.
	 */
	...
	/**
	 * Makes method parameters $0, $1, ..., $args, $$, and $class available. $0 is available only if
	 * use0 is true. It might not be equivalent to THIS.
	 *
	 * @param params
	 ...
	 * @param target
	 *            the class of $0. If use0 is false, target can be null. The value of "target" is
	 *            also used as the name of the type represented by $class.
	 * @param isStatic
	 *            true if the method in which the compiled bytecode is embedded is static.
	 */
	public int recordParams(CtClass[] params, boolean isStatic, String prefix, String paramVarName, String paramsName, boolean use0,
			int paramBase, String target, SymbolTable tbl) throws CompileError {
		int varNo;
	 ...
	 *
	 * @param regno the index of the local variable in which the first argument is received. (0:
	 * static method, 1: regular method.)
	 */
	...
	/*
	 * Sets exprType, arrayDim, and className; If type is void, then this method does nothing.
	 */
