...
import org.javassist.CtBehavior;
import org.javassist.CtClass;
import org.javassist.CtConstructor;
import org.javassist.CtField;
import org.javassist.CtMember;
import org.javassist.CtMethod;
import org.javassist.CtPrimitiveType;
import org.javassist.Modifier;
import org.javassist.NotFoundException;
import org.javassist.bytecode.BadBytecode;
import org.javassist.bytecode.Bytecode;
...
import org.javassist.compiler.ast.*;
...
public class Javac {
	...
	/**
	 * Constructs a compiler. The produced bytecode is stored in the <code>Bytecode</code> object
	...
	/**
	 * Compiles a method, constructor, or field declaration to a class. A field declaration can
	 * declare only one field.
	 *
	 * <p>
	 ...
	 *
	 * @param src
	 *            a single statement or a block. If null, this method produces a body returning zero
	 *            or null.
	 */
	 ...
	 * LocalVariableAttribute is not available, this method does not record any local variable. It
	 * only returns false.
	 *
	 * @param pc
	 *            program counter (&gt;= 0)
	 * @return false if the CodeAttribute does not include a LocalVariableAttribute.
	 */
	...
	/**
	 * Records parameter names if the LocalVariableAttribute is available. It returns false unless
	 * the LocalVariableAttribute is available.
	 *
	...
	/**
	 * Makes variables $0 (this), $1, $2, ..., and $args represent method parameters. $args
	 * represents an array of all the parameters. It also makes $$ available as a parameter list of
	 * method call.
	 *
	 * <p>
	 * This must be called before calling <code>compileStmnt()</code> and
	 * <code>compileExpr()</code>. The correct value of <code>isStatic</code> must be recorded
	 * before compilation. <code>maxLocals</code> is updated to include $0,...
	 */
	...
	/**
	 * Makes variables $0, $1, $2, ..., and $args represent method parameters. $args represents an
	 * array of all the parameters. It also makes $$ available as a parameter list of method call.
	 * $0 can represent a local variable other than THIS (variable 0). $class is also made
	 * available.
	 *
	 * <p>
	 * This must be called before calling <code>compileStmnt()</code> and
	 ...
	 * @param target
	 *            the type of $0 (it can be null if use0 is false). It is used as the name of the
	 *            type represented by $class.
	 * @param isStatic
	 *            true if the method in which the compiled bytecode is embedded is static.
	 */
	...
	/**
	 * Sets <code>maxLocals</code> to <code>max</code>. This method tells the compiler the local
	 * variables that have been allocated for the rest of the code. When the compiler needs new
	 * local variables, the local variables at the index <code>max</code>, <code>max + 1</code>, ...
	 * are assigned.
	 *
	 * <p>
	...
	/**
	 * Prepares to use cast $r, $w, $_, and $type. $type is made to represent the specified return
	 * type. It also enables to write a return statement with a return value for void method.
	 *
	 * <p>
	...
	/**
	 * Prepares to use $proceed(). If the return type of $proceed() is void, null is pushed on the
	 * stack.
	 *
	 * @param target
	 *            an expression specifying the target object. if null, "this" is the target.
	 * @param method
	 *            the method name.
	 */
	...
	/**
	 * Prepares to use $proceed() representing a static method. If the return type of $proceed() is
	 * void, null is pushed on the stack.
	 *
	 * @param targetClass
	...
	/**
	 * Prepares to use $proceed() representing a private/super's method. If the return type of
	 * $proceed() is void, null is pushed on the stack. This method is for methods invoked by
	 * INVOKESPECIAL.
	 *
	 * @param target
	 *            an expression specifying the target object. if null, "this" is the target.
	 ...
	 */
	public void recordSpecialProceed(String target, final String classname, final String methodname, final String descriptor,
	 ...
	 *
	 * <p>
	 * Local variables that are not declared in the compiled source text might not be accessible
	 * within that source text. Fields and method parameters ($0, $1, ..) are available.
	 */
	...
	/**
	 * Compiles an exression. <code>recordParams()</code> must be called before invoking this
	 * method.
	 *
	 * <p>
	 * Local variables are not accessible within the compiled source text. Fields and method
	 * parameters ($0, $1, ..) are available if <code>recordParams()</code> have been invoked.
	 */
