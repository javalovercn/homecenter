...
class CodeAnalyzer implements Opcode {
	...
	public int computeMaxStack() throws BadBytecode {
		/*
		 * d = stack[i] d == 0: not visited d > 0: the depth is d - 1 after executing the bytecode
		 * at i. d < 0: not visited. the initial depth (before execution) is 1 - d.
		 */
	...
	private boolean processBranch(int opcode, CodeIterator ci, int index, int codeLength, int[] stack, int stackDepth, int[] jsrDepth)
		...
		if ((IFEQ <= opcode && opcode <= IF_ACMPNE) || opcode == IFNULL || opcode == IFNONNULL) {
			...
			case JSR_W:
				...
				checkTarget(index, target, codeLength, stack, stackDepth);
				/*
				 * It is unknown which RET comes back to this JSR. So we assume that if the stack
				 * depth at one JSR instruction is N, then it is also N at other JSRs and N - 1 at
				 * all RET instructions. Note that STACK_GROW[JSR] is 1 since it pushes a return
