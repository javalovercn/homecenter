...
/**
 * MultiType represents an unresolved type. Whenever two {@code Type} instances are merged, if they
 * share more than one super type (either an interface or a superclass), then a {@code MultiType} is
 * used to represent the possible super types. The goal of a {@code MultiType} is to reduce the set
 * of possible types down to a single resolved type. This is done by eliminating non-assignable
 * types from the typeset when the {@code MultiType} is passed as an argument to
 * {@link Type#isAssignableFrom(Type)}, as well as removing non-intersecting types during a merge.
 *
 * Note: Currently the {@code MultiType} instance is reused as much as possible so that updates are
 * visible from all frames. In addition, all {@code MultiType} merge paths are also updated. This is
...
/*
 * TODO - A better, but more involved, approach would be to track the instruction offset that
 * resulted in the creation of this type, and whenever the typeset changes, to force a merge on that
 * position. This would require creating a new MultiType instance every time the typeset changes,
 * and somehow communicating assignment changes to the Analyzer
 */
public class MultiType extends Type {
	...
	/**
	 * Gets the class that corresponds with this type. If this information is not yet known,
	 * java.lang.Object will be returned.
	 */
