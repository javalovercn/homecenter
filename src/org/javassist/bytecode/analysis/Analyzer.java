...
/**
 * A data-flow analyzer that determines the type state of the stack and local variable table at
 * every reachable instruction in a method. During analysis, bytecode verification is performed in a
 * similar manner to that described in the JVM specification.
 *
 * <p>
 * Example:
 * </p>
...
public class Analyzer implements Opcode {
	...
	/**
	 * Performs data-flow analysis on a method and returns an array, indexed by instruction
	 * position, containing the starting frame state of all reachable instructions. Non-reachable
	 * code, and illegal code offsets are represented as a null in the frame state array. This can
	 * be used to detect dead code.
	 *
	 * If the method does not contain code (it is either native or abstract), null is returned.
	 *
	 * @param clazz
	 *            the declaring class of the method
	 * @param method
	 *            the method to analyze
	 * @return an array, indexed by instruction position, of the starting frame state, or null if
	 *         this method doesn't have code
	 * @throws BadBytecode
