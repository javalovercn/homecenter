 ...
 * License.
 */
package org.javassist.bytecode.analysis;
...
/**
 * Represents a JVM type in data-flow analysis. This abstraction is necessary since a JVM type not
 * only includes all normal Java types, but also a few special types that are used by the JVM
 * internally. See the static field types on this class for more info on these special types.
 *
 * All primitive and special types reuse the same instance, so identity comparison can be used when
 * examining them. Normal java types must use {@link #equals(Object)} to compare type instances.
 *
 * In most cases, applications which consume this API, only need to call {@link #getCtClass()} to
 * obtain the needed type information.
 *
...
public class Type {
	...
	/**
	 * Represents an unknown, or null type. This occurs when aconst_null is used. It is important
	 * not to treat this type as java.lang.Object, since a null can be assigned to any reference
	 * type. The analyzer will replace these with an actual known type if it can be determined by a
	 * merged path with known type information. If this type is encountered on a frame then it is
	 * guaranteed to be null, and the type information is simply not available. Any attempts to
	...
	/**
	 * Represents an internal JVM return address, which is used by the RET instruction to return to
	 * a JSR that invoked the subroutine.
	 */
	...
	/**
	 * Represents a non-accessible value. Code cannot access the value this type represents. It
	 * occurs when bytecode reuses a local variable table position with non-mergable types. An
	 * example would be compiled code which uses the same position for a primitive type in one
	 * branch, and a reference type in another branch.
	 */
	...
	/**
	 * Obtain the Type for a given class. If the class is a primitive, the the unique type instance
	 * for the primitive will be returned. Otherwise a new Type instance representing the class is
	 * returned.
	 *
	 * @param clazz
	 *            The java class
	...
	/**
	 * Gets the word size of this type. Double-word types, such as long and double will occupy two
	 * positions on the local variable table or stack.
	 *
	...
	/**
	 * Returns whether or not this type is a normal java reference, i.e. it is or extends
	 * java.lang.Object.
	 *
	...
	/**
	 * Returns whether or not the type is special. A special type is one that is either used for
	 * internal tracking, or is only used internally by the JVM.
	 *
	...
	/**
	 * Returns the array component if this type is an array. If the type is not an array null is
	 * returned.
	 *
	...
	/**
	 * Determines whether this type is assignable, to the passed type. A type is assignable to
	 * another if it is either the same type, or a sub-type.
	 *
	 * @param type
	...
	/**
	 * Finds the common base type, or interface which both this and the specified type can be
	 * assigned. If there is more than one possible answer, then a {@link MultiType}, or a
	 * {@link MultiArrayType} is returned. Multi-types have special rules, and successive merges and
	 * assignment tests on them will alter their internal state, as well as other multi-types they
	 * have been merged with. This method is used by the data-flow analyzer to merge the type state
