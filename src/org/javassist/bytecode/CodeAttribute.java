 ...
 *
 * <p>
 * To browse the <code>code</code> field of a <code>Code_attribute</code> structure, use
...
public class CodeAttribute extends AttributeInfo implements Opcode {
	...
	/**
	 * Constructs a copy of <code>Code_attribute</code>. Specified class names are replaced during
	 * the copy.
	 *
	 * @param cp
	 *            constant pool table.
	 ...
	 *                if a <code>BadBytecode</code> exception is thrown, it is converted into
	...
	/**
	 * Returns the length of this <code>attribute_info</code> structure. The returned value is
	 * <code>attribute_length + 6</code>.
	 */
	public int length() {
	...
	/**
	 * Returns <code>attributes[]</code>. It returns a list of <code>AttributeInfo</code>. A new
	 * element can be added to the returned list and an existing element can be removed from the
	 * list.
	 *
	 * @see AttributeInfo
	 */
	...
	/**
	 * Adds a stack map table. If another copy of stack map table is already contained, the old one
	 * is removed.
	 *
	 * @param smt
	 *            the stack map table added to this code attribute. If it is null, a new stack map
	 *            is not added. Only the old stack map is removed.
	 */
	...
	private static LdcEntry copyCode(byte[] code, int beginPos, int endPos, ConstPool srcCp, byte[] newcode, ConstPool destCp,
	...
	static class LdcEntry {
		...
		static byte[] doit(byte[] code, LdcEntry ldc, ExceptionTable etable, CodeAttribute ca) throws BadBytecode {
			 ...
			 * 
			 * while (ldc != null) { int where = ldc.where; code = CodeIterator.insertGapCore0(code,
	 ...
	 * not update <code>LocalVariableAttribute</code>, <code>LocalVariableTypeAttribute</code>,
	 * <code>StackMapTable</code>, or <code>StackMap</code>. These attributes must be explicitly
	 * updated.
	 *
	 * @param where
	...
	/**
	 * @param lessThan
	 *            If the index of the local variable is less than this value, it does not change.
	 *            Otherwise, the index is increased.
	 * @param delta
	 *            the indexes of the local variables are increased by this value.
	 */
