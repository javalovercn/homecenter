...
public abstract class Tracer implements TypeTag {
	...
	protected void visitJSR(int pos, byte[] code) throws BadBytecode {
		/*
		 * Since JSR pushes a return address onto the operand stack, the stack map at the entry
		 * point of a subroutine is stackTypes resulting after executing the following code:
		 *
	...
	/**
	 * Invoked when the visited instruction is ret or wide ret. Java6 or later does not allow using
	 * RET.
	 */
	...
	/*
	 * This is a constructor call on an uninitialized object. Sets flags of other references to that
	 * object.
	 *
	...
	private int doInvokeDynamic(int pos, byte[] code) throws BadBytecode {
		...
		/*
		 * TypeData target = stackTypes[--stackTop]; if (target instanceof TypeData.UninitTypeVar &&
		 * target.isUninit()) constructorCalled((TypeData.UninitTypeVar)target);
		 */
