...
import org.javassist.bytecode.StackMapTable;
...
public abstract class TypeData {
	...
	/**
	 * Sets the type name of this object type. If the given type name is a subclass of the current
	 * type name, then the given name becomes the name of this object type.
	 *
	 * @param className
	...
	/**
	 * Returns this if it is a TypeVar or a TypeVar that this type depends on. Otherwise, this
	 * method returns null. It is used by dfs().
	 *
	 * @param dim
	 *            dimension
	 */
	...
	public static class TypeVar extends AbsTypeVar {
		...
		public int getTypeTag() {
			/*
			 * If fixedType is null after calling dfs(), then this type is NULL, Uninit, or a basic
			 * type. So call getTypeTag() on the first element of lowers.
			 */
			if (fixedType == null)
		...
		/*
		 * When fixTypes() is called, getName() will return the correct (i.e. fixed) type name.
		 */
		public TypeData getArrayType(int dim) throws NotFoundException {
			if (dim == 0)
				return this;
				...
				for (int j = 0; j < size2; j++) {
					...
					if (kind == null) {
						if (bt == null) {
							...
							kind = d;
							/*
							 * If scc has only an UninitData, fixedType is kept null. So lowerSet
							 * must be empty. If scc has not only an UninitData but also another
							 * TypeData, an error must be thrown but this error detection has not
							 * been implemented.
							 */
							if (d.isUninit())
								break;
	 ...
	 * org.javassist.bytecode.analysis.Type.
	 */
	...
	/*
	 * A type variable representing an array type. It is a decorator of another type variable.
	 */
	...
	/*
	 * A type variable representing an array-element type. It is a decorator of another type
	 * variable.
	 */
	...
	/**
	 * Type data for NULL or OBJECT. The types represented by the instances of this class are
