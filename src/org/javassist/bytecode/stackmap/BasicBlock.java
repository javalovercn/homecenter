...
/**
 * A basic block is a sequence of bytecode that does not contain jump/branch instructions except at
 * the last bytecode. Since Java7 or later does not allow JSR, this class throws an exception when
 * it finds JSR.
 */
public class BasicBlock {
	...
	public static class Maker {
		/*
		 * Override these two methods if a subclass of BasicBlock must be instantiated.
		 */
		...
		/*
		 * We could ignore JSR since Java 7 or later does not allow it. See The JVM Spec. Sec.
		 * 4.10.2.5.
		 */
		protected void makeJsr(HashMap marks, int pos, int target, int size) throws BadBytecode {
			/*
			 * Mark to = makeMark(marks, target); Mark next = makeMark(marks, pos + size);
			 * BasicBlock[] jumps = makeArray(to.block, next.block); makeMark(marks, pos, jumps,
			 * size, false);
			 */
