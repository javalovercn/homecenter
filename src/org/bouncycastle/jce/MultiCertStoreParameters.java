...
public class MultiCertStoreParameters implements CertStoreParameters {
	...
	/**
	 * Create a parameters object which can be to used to make a multi store made up of the passed
	 * in CertStores. If the searchAllStores parameter is false, any search on the multi-store will
	 * terminate as soon as a search query produces a result.
	 * 
	 * @param certStores
	 *            CertStores making up the multi CertStore
	 * @param searchAllStores
	 *            true if all CertStores should be searched on request, false if a result should be
	 *            returned on the first successful CertStore query.
	 */
