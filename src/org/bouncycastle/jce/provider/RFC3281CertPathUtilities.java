...
class RFC3281CertPathUtilities {
	...
	protected static void processAttrCert7(X509AttributeCertificate attrCert, CertPath certPath, CertPath holderCertPath,
	 ...
	 *             occurs.
	 */
	protected static void checkCRLs(X509AttributeCertificate attrCert, PKIXExtendedParameters paramsPKIX, X509Certificate issuerCert,
		...
		if (paramsPKIX.isRevocationEnabled()) {
			...
			if (attrCert.getExtensionValue(NO_REV_AVAIL) == null) {
				...
				/*
				 * If the revocation status has not been determined, repeat the process above with
				 * any available CRLs not specified in a distribution point but issued by the
				 * certificate issuer.
				 */
				...
				if (certStatus.getCertStatus() == CertStatus.UNREVOKED && !reasonsMask.isAllReasons()) {
					try {
						...
						try {
							...
							issuer = new ASN1InputStream(((X500Principal) attrCert.getIssuer().getPrincipals()[0]).getEncoded())
									.readObject();
						} catch (Exception e) {
							throw new AnnotatedException("Issuer from certificate for CRL could not be reencoded.", e);
						}
						DistributionPoint dp = new DistributionPoint(
								new DistributionPointName(0, new GeneralNames(new GeneralName(GeneralName.directoryName, issuer))), null,
								null);
						PKIXExtendedParameters paramsPKIXClone = (PKIXExtendedParameters) paramsPKIX.clone();
	 ...
	 *             if
	 *             <ul>
	 *             <li>no public key certificate can be found although holder information is given
	 *             by an entity name or a base certificate ID
		...
		if (attrCert.getHolder().getIssuer() != null) {
			...
			for (int i = 0; i < principals.length; i++) {
				try {
					...
					holderPKCs.addAll(CertPathValidatorUtilities.findCertificates(new PKIXCertStoreSelector.Builder(selector).build(),
	 ...
	 *             occurs.
	 */
	private static void checkCRL(DistributionPoint dp, X509AttributeCertificate attrCert, PKIXExtendedParameters paramsPKIX, Date validDate,
			X509Certificate issuerCert, CertStatus certStatus, ReasonsMask reasonMask, List certPathCerts, JcaJceHelper helper)
			throws AnnotatedException {

		/*
		 * 4.3.6 No Revocation Available
		 * 
		 * The noRevAvail extension, defined in [X.509-2000], allows an AC issuer to indicate that
		 * no revocation information will be made available for this AC.
		 */
		 ...
		 * PKIXExtendedParameters and are in getAdditionalStore()
		 */
		...
		while (crl_iter.hasNext() && certStatus.getCertStatus() == CertStatus.UNREVOKED && !reasonMask.isAllReasons()) {
			try {
				...
				if (paramsPKIX.isUseDeltasEnabled()) {
					// get delta CRLs
					Set deltaCRLs = CertPathValidatorUtilities.getDeltaCRLs(currentDate, crl, paramsPKIX.getCertStores(),
