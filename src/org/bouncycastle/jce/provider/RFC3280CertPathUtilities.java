...
class RFC3280CertPathUtilities {
	 ...
	 * distribution field is omitted from the DP, then verify that one of the names in the IDP
	 * matches one of the names in the cRLIssuer field of the DP.
	 * </p>
	 * <p>
	 * (ii) If the onlyContainsUserCerts boolean is asserted in the IDP CRL extension, verify that
	 * the certificate does not include the basic constraints extension with the cA boolean
	 * asserted.
	 * </p>
	 * <p>
	 * (iii) If the onlyContainsCACerts boolean is asserted in the IDP CRL extension, verify that
	 * the certificate includes the basic constraints extension with the cA boolean asserted.
	 * </p>
	 * <p>
	...
	protected static void processCRLB2(DistributionPoint dp, Object cert, X509CRL crl) throws AnnotatedException {
		...
		try {
			idp = IssuingDistributionPoint
					.getInstance(CertPathValidatorUtilities.getExtensionValue(crl, RFC3280CertPathUtilities.ISSUING_DISTRIBUTION_POINT));
		} catch (Exception e) {
		...
		if (idp != null) {
			if (idp.getDistributionPoint() != null) {
						...
						if (dp.getCRLIssuer() != null) {
							...
							try {
								genNames[0] = new GeneralName(
										X500Name.getInstance(PrincipalUtils.getEncodedIssuerPrincipal(cert).getEncoded()));
							} catch (Exception e) {
				...
				// the DP
					...
					if (dp.getCRLIssuer() == null) {
						throw new AnnotatedException(
								"Either the cRLIssuer or the distributionPoint field must " + "be contained in DistributionPoint.");
					}
	 ...
	 * cRLIssuer in the DP and that the complete CRL contains an issuing distribution point
	 * extension with the indirectCRL boolean asserted. Otherwise, verify that the CRL issuer
	 ...
	 *
	 * @param crl
	 *            CRL which contains revocation information for the certificate <code>cert</code>.
	 * @param cert
	 *            The attribute certificate or certificate to check if it is revoked.
	 ...
	 * @throws AnnotatedException
	 *             if the CRL is not valid or the status cannot be checked or some error occurs.
	 */
	protected static Set processCRLF(X509CRL crl, Object cert, X509Certificate defaultCRLSignCert, PublicKey defaultCRLSignKey,
			PKIXExtendedParameters paramsPKIX, List certPathCerts, JcaJceHelper helper) throws AnnotatedException {
		// (f)
		...
		try {
			...
			throw new AnnotatedException("Subject criteria for certificate selector to find issuer certificate for CRL could not be set.",
					e);
		}
		...
		while (cert_it.hasNext()) {
			...
			/*
			 * CA of the certificate, for which this CRL is checked, has also signed CRL, so skip
			 * the path validation, because is already done
			 */
			...
			try {
				...
				/*
				 * if signingCert is placed not higher on the cert path a dependency loop results.
				 * CRL for cert is checked, but signingCert is needed for checking the CRL which is
				 * dependent on checking cert because it is higher in the cert path and so signing
				 * signingCert transitively. so, revocation is disabled, forgery attacks of the CRL
				 * are detected in this outer loop for all other it must be enabled to prevent
				 * forgery attacks
				 */
	...
	protected static Set[] processCRLA1ii(Date currentDate, PKIXExtendedParameters paramsPKIX, X509Certificate cert, X509CRL crl)
		...
		if (paramsPKIX.isUseDeltasEnabled()) {
			...
			try {
				deltaSet.addAll(
						CertPathValidatorUtilities.getDeltaCRLs(validityDate, crl, paramsPKIX.getCertStores(), paramsPKIX.getCRLStores()));
			} catch (AnnotatedException e) {
				throw new AnnotatedException("Exception obtaining delta CRLs.", e);
			}
		}
	...
	protected static void processCRLC(X509CRL deltaCRL, X509CRL completeCRL, PKIXExtendedParameters pkixParams) throws AnnotatedException {
		...
		try {
			completeidp = IssuingDistributionPoint.getInstance(
					CertPathValidatorUtilities.getExtensionValue(completeCRL, RFC3280CertPathUtilities.ISSUING_DISTRIBUTION_POINT));
		} catch (Exception e) {
			throw new AnnotatedException("Issuing distribution point extension could not be decoded.", e);
		}
		...
		if (pkixParams.isUseDeltasEnabled()) {
			...
			try {
				deltaidp = IssuingDistributionPoint
						.getInstance(CertPathValidatorUtilities.getExtensionValue(deltaCRL, ISSUING_DISTRIBUTION_POINT));
			} catch (Exception e) {
	...
	protected static PKIXPolicyNode prepareCertB(CertPath certPath, int index, List[] policyNodes, PKIXPolicyNode validPolicyTree,
						...
						while (nodes_i.hasNext()) {
							...
							if (RFC3280CertPathUtilities.ANY_POLICY.equals(node.getValidPolicy())) {
								...
								try {
									...
									throw new ExtCertPathValidatorException("Certificate policies extension could not be decoded.", e,
											certPath, index);
								}
								...
								while (e.hasMoreElements()) {
									...
									if (RFC3280CertPathUtilities.ANY_POLICY.equals(pinfo.getPolicyIdentifier().getId())) {
										try {
											...
											throw new ExtCertPathValidatorException("Policy qualifier info set could not be decoded.", ex,
													certPath, index);
										}
										break;
									}
								...
								if (RFC3280CertPathUtilities.ANY_POLICY.equals(p_node.getValidPolicy())) {
									PKIXPolicyNode c_node = new PKIXPolicyNode(new ArrayList(), i, (Set) m_idp.get(id_p), p_node, pq, id_p,
											ci);
	...
	protected static PKIXPolicyNode processCertE(CertPath certPath, int index, PKIXPolicyNode validPolicyTree)
		...
		try {
			certPolicies = DERSequence
					.getInstance(CertPathValidatorUtilities.getExtensionValue(cert, RFC3280CertPathUtilities.CERTIFICATE_POLICIES));
		} catch (AnnotatedException e) {
		...
		if (!(CertPathValidatorUtilities.isSelfIssued(cert) && (i < n))) {
			...
			try {
				altName = GeneralNames
						.getInstance(CertPathValidatorUtilities.getExtensionValue(cert, RFC3280CertPathUtilities.SUBJECT_ALTERNATIVE_NAME));
			} catch (Exception e) {
			...
			for (int eI = 0; eI != emails.length; eI++) {
				...
				try {
					...
					throw new CertPathValidatorException("Subtree check for certificate subject alternative email failed.", ex, certPath,
							index);
				}
			}
			if (altName != null) {
				...
				for (int j = 0; j < genNames.length; j++) {
					...
					try {
						...
						throw new CertPathValidatorException("Subtree check for certificate subject alternative name failed.", e, certPath,
								index);
					}
				}
			}
	...
	protected static PKIXPolicyNode processCertD(CertPath certPath, int index, Set acceptablePolicies, PKIXPolicyNode validPolicyTree,
			List[] policyNodes, int inhibitAnyPolicy) throws CertPathValidatorException {
		List certs = certPath.getCertificates();
		X509Certificate cert = (X509Certificate) certs.get(index);
		int n = certs.size();
		// i as defined in the algorithm description
						...
						for (int k = 0; k < _nodes.size(); k++) {
							...
							while (_policySetIter.hasNext()) {
								...
								if (!_found) {
									...
									PKIXPolicyNode _newChild = new PKIXPolicyNode(new ArrayList(), i, _newChildExpectedPolicies, _node,
											_apq, _policy, false);
	...
	protected static void processCertA(CertPath certPath, PKIXExtendedParameters paramsPKIX, int index, PublicKey workingPublicKey,
			boolean verificationAlreadyPerformed, X500Name workingIssuerName, X509Certificate sign, JcaJceHelper helper)
		...
		if (paramsPKIX.isRevocationEnabled()) {
			try {
				checkCRLs(paramsPKIX, cert, CertPathValidatorUtilities.getValidCertDateFromValidityModel(paramsPKIX, certPath, index), sign,
		...
		if (!PrincipalUtils.getEncodedIssuerPrincipal(cert).equals(workingIssuerName)) {
			throw new ExtCertPathValidatorException("IssuerName(" + PrincipalUtils.getEncodedIssuerPrincipal(cert)
					+ ") does not match SubjectName(" + workingIssuerName + ") of signing certificate.", null, certPath, index);
		}
	}
	...
	protected static void prepareNextCertG(CertPath certPath, int index, PKIXNameConstraintValidator nameConstraintValidator)
		...
		try {
			ASN1Sequence ncSeq = DERSequence
					.getInstance(CertPathValidatorUtilities.getExtensionValue(cert, RFC3280CertPathUtilities.NAME_CONSTRAINTS));
			if (ncSeq != null) {
		...
		if (nc != null) {
			...
			if (permitted != null) {
				try {
					...
					throw new ExtCertPathValidatorException("Permitted subtrees cannot be build from name constraints extension.", ex,
							certPath, index);
				}
			}
			...
			if (excluded != null) {
				for (int i = 0; i != excluded.length; i++)
					try {
						...
						throw new ExtCertPathValidatorException("Excluded subtrees cannot be build from name constraints extension.", ex,
								certPath, index);
					}
			}
		}
	 ...
	 *             occurs.
	 */
	private static void checkCRL(DistributionPoint dp, PKIXExtendedParameters paramsPKIX, X509Certificate cert, Date validDate,
			X509Certificate defaultCRLSignCert, PublicKey defaultCRLSignKey, CertStatus certStatus, ReasonsMask reasonMask,
			List certPathCerts, JcaJceHelper helper) throws AnnotatedException {
		...
		// (a)
		/*
		 * We always get timely valid CRLs, so there is no step (a) (1). "locally cached" CRLs are
		 * assumed to be in getStore(), additional CRLs must be enabled in the
		 * ExtendedPKIXParameters and are in getAdditionalStore()
		 */
		...
		while (crl_iter.hasNext() && certStatus.getCertStatus() == CertStatus.UNREVOKED && !reasonMask.isAllReasons()) {
			try {
				...
				// (e)
				/*
				 * The reasons mask is updated at the end, so only valid CRLs can update it. If this
				 * CRL does not contain new reasons it must be ignored.
				 */
				...
				if (paramsPKIX.isUseDeltasEnabled()) {
					// get delta CRLs
					Set deltaCRLs = CertPathValidatorUtilities.getDeltaCRLs(validityDate, crl, paramsPKIX.getCertStores(),
				 ...
				 * certificate is revoked with reason keyCompromise, cACompromise, it can be used
				 * for forgery, also for the past. This reason may not be contained in older CRLs.
				 */
	 ...
	 *             occurs.
	 */
	protected static void checkCRLs(PKIXExtendedParameters paramsPKIX, X509Certificate cert, Date validDate, X509Certificate sign,
			PublicKey workingPublicKey, List certPathCerts, JcaJceHelper helper) throws AnnotatedException {
		AnnotatedException lastException = null;
		CRLDistPoint crldp = null;
		try {
			crldp = CRLDistPoint
					.getInstance(CertPathValidatorUtilities.getExtensionValue(cert, RFC3280CertPathUtilities.CRL_DISTRIBUTION_POINTS));
		} catch (Exception e) {
		...
		/*
		 * If the revocation status has not been determined, repeat the process above with any
		 * available CRLs not specified in a distribution point but issued by the certificate
		 * issuer.
		 */
		...
		if (certStatus.getCertStatus() == CertStatus.UNREVOKED && !reasonsMask.isAllReasons()) {
			try {
				...
				DistributionPoint dp = new DistributionPoint(
						new DistributionPointName(0, new GeneralNames(new GeneralName(GeneralName.directoryName, issuer))), null, null);
	...
	protected static void prepareNextCertK(CertPath certPath, int index) throws CertPathValidatorException {
		...
		try {
			bc = BasicConstraints
					.getInstance(CertPathValidatorUtilities.getExtensionValue(cert, RFC3280CertPathUtilities.BASIC_CONSTRAINTS));
		} catch (Exception e) {
	...
	protected static void prepareNextCertN(CertPath certPath, int index) throws CertPathValidatorException {
		...
		if ((_usage != null) && !_usage[RFC3280CertPathUtilities.KEY_CERT_SIGN]) {
			throw new ExtCertPathValidatorException("Issuer certificate keyusage extension is critical and does not permit key signing.",
					null, certPath, index);
		}
	}
	...
	protected static final String[] crlReasons = new String[] { "unspecified", "keyCompromise", "cACompromise", "affiliationChanged",
			"superseded", "cessationOfOperation", "certificateHold", "unknown", "removeFromCRL", "privilegeWithdrawn", "aACompromise" };
			...
			while (policyConstraints.hasMoreElements()) {
				...
				case 0:
					try {
						...
						throw new ExtCertPathValidatorException("Policy constraints requireExplicitPolicy field could not be decoded.", e,
								certPath, index);
					}
					if (tmpInt == 0) {
						return 0;
	...
	protected static PKIXPolicyNode wrapupCertG(CertPath certPath, PKIXExtendedParameters paramsPKIX, Set userInitialPolicySet, int index,
			List[] policyNodes, PKIXPolicyNode validPolicyTree, Set acceptablePolicies) throws CertPathValidatorException {
