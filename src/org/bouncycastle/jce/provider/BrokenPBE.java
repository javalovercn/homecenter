...
class OldPKCS12ParametersGenerator extends PBEParametersGenerator {
	...
	/**
	 * Generate a key parameter derived from the password, salt, and iteration count we are
	...
	static class Util {
		...
		static private void setOddParity(byte[] bytes) {
			for (int i = 0; i < bytes.length; i++) {
				int b = bytes[i];
				bytes[i] = (byte) ((b & 0xfe) | (((b >> 1) ^ (b >> 2) ^ (b >> 3) ^ (b >> 4) ^ (b >> 5) ^ (b >> 6) ^ (b >> 7)) ^ 0x01));
			}
		}
		 ...
		 */
		static CipherParameters makePBEParameters(BCPBEKey pbeKey, AlgorithmParameterSpec spec, int type, int hash, String targetAlgorithm,
		...
		/**
		 * generate a PBE based key suitable for a MAC algorithm, the key size is chosen according
		 * the MAC size, or the hashing algorithm, whichever is greater.
		 */
