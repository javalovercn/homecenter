	...
	public AlgorithmIdentifier getAlgorithmIdentifier(ASN1ObjectIdentifier algorithm, AlgorithmParameterSpec algorithmSpec)
		...
		if (algorithmSpec instanceof OAEPParameterSpec) {
			if (algorithmSpec.equals(OAEPParameterSpec.DEFAULT)) {
				return new AlgorithmIdentifier(algorithm, new RSAESOAEPparams(RSAESOAEPparams.DEFAULT_HASH_ALGORITHM,
				...
				AlgorithmIdentifier mgf1HashAlgorithm = new DefaultDigestAlgorithmIdentifierFinder()
						.find((((MGF1ParameterSpec) oaepSpec.getMGFParameters()).getDigestAlgorithm()));
