...
public class TimeStampTokenGenerator {
	...
	/**
	 * Basic Constructor - set up a calculator based on signerInfoGen with a ESSCertID calculated
	 * from the signer's associated certificate using the sha1DigestCalculator. If alternate values
	 * are required for id-aa-signingCertificate they should be added to the signerInfoGen object
	 * before it is passed in, otherwise a standard digest based value will be added.
	 *
	 * @param signerInfoGen
	 *            the generator for the signer we are using.
	 * @param digestCalculator
	 *            calculator for to use for digest of certificate.
	 * @param tsaPolicy
	 *            tasPolicy to send.
	 * @throws IllegalArgumentException
	 *             if calculator is not SHA-1 or there is no associated certificate for the signer,
	 * @throws TSPException
	 *             if the signer certificate cannot be processed.
	 */
	public TimeStampTokenGenerator(final SignerInfoGenerator signerInfoGen, DigestCalculator digestCalculator,
			ASN1ObjectIdentifier tsaPolicy) throws IllegalArgumentException, TSPException {
	 ...
	 * @param tsaPolicy
	 *            tasPolicy to send.
	 * @param isIssuerSerialIncluded
	 *            should issuerSerial be included in the ESSCertIDs, true if yes, by default false.
	 * @throws IllegalArgumentException
	 *             if calculator is not SHA-1 or there is no associated certificate for the signer,
	 * @throws TSPException
	 *             if the signer certificate cannot be processed.
	 */
	public TimeStampTokenGenerator(final SignerInfoGenerator signerInfoGen, DigestCalculator digestCalculator,
			ASN1ObjectIdentifier tsaPolicy, boolean isIssuerSerialIncluded) throws IllegalArgumentException, TSPException {
			...
			if (digestCalculator.getAlgorithmIdentifier().getAlgorithm().equals(OIWObjectIdentifiers.idSHA1)) {
				final ESSCertID essCertid = new ESSCertID(digestCalculator.getDigest(),
						isIssuerSerialIncluded
								? new IssuerSerial(new GeneralNames(new GeneralName(assocCert.getIssuer())), assocCert.getSerialNumber())
								: null);
				...
				final ESSCertIDv2 essCertid = new ESSCertIDv2(digAlgID, digestCalculator.getDigest(),
						isIssuerSerialIncluded
								? new IssuerSerial(new GeneralNames(new GeneralName(assocCert.getIssuer())),
	 ...
	 *
	 * @param crlStore
	 *            a Store containing X509CRLHolder objects.
	 */
	 ...
	 * TimeStampToken.
	 *
	 * @param otherRevocationInfoFormat
	 *            the OID specifying the format of the otherRevocationInfo data.
	 * @param otherRevocationInfos
	...
	/**
	 * Generate a TimeStampToken for the passed in request and serialNumber marking it with the
	 * passed in genTime.
	 *
	 * @param request
	...
	public TimeStampToken generate(TimeStampRequest request, BigInteger serialNumber, Date genTime) throws TSPException {
		...
		TSTInfo tstInfo = new TSTInfo(tsaPolicy, messageImprint, new ASN1Integer(serialNumber), new ASN1GeneralizedTime(genTime), accuracy,
				derOrdering, nonce, tsa, request.getExtensions());
		...
		try {
			...
			CMSSignedData signedData = signedDataGenerator
					.generate(new CMSProcessableByteArray(PKCSObjectIdentifiers.id_ct_TSTInfo, derEncodedTSTInfo), true);
