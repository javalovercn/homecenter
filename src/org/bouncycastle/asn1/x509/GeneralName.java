...
public class GeneralName extends ASN1Object implements ASN1Choice {
	 ...
	 * included as an rfc822Name. The format of an rfc822Name is an "addr-spec" as defined in RFC
	 * 822 [RFC 822].
	 *
	 * When the subjectAltName extension contains a domain name service label, the domain name MUST
	 * be stored in the dNSName (an IA5String). The name MUST be in the "preferred name syntax," as
	 * specified by RFC 1034 [RFC 1034].
	 *
	 * When the subjectAltName extension contains a URI, the name MUST be stored in the
	 * uniformResourceIdentifier (an IA5String). The name MUST be a non-relative URL, and MUST
	 * follow the URL syntax and encoding rules specified in [RFC 1738]. The name must include both
	 * a scheme (e.g., "http" or "ftp") and a scheme-specific-part. The scheme- specific-part must
	 * include a fully qualified domain name or IP address as the host.
	 *
	 * When the subjectAltName extension contains a iPAddress, the address MUST be stored in the
	 * octet string in "network byte order," as specified in RFC 791 [RFC 791]. The least
	 * significant bit (LSB) of each octet is the LSB of the corresponding byte in the network
	 * address. For IP Version 4, as specified in RFC 791, the octet string MUST contain exactly
	 * four octets. For IP Version 6, as specified in RFC 1883, the octet string MUST contain
	 * exactly sixteen octets [RFC 1883].
	 */
	 ...
	 * <p>
	 * Note: A directory name can be encoded in different ways into a byte representation. Be aware
	 * of this if the byte representation is used for comparing results.
	 *
	 * @param tag
	 *            tag number
	 * @param name
