...
public class X509Name extends ASN1Object {
	...
	/**
	 * default look up table translating OID values into their common symbols following the
	 * convention in RFC 2253 with a few extras
	 */
	public static final Hashtable DefaultSymbols = new Hashtable();

	/**
	 * look up table translating OID values into their common symbols following the convention in
	 * RFC 2253
	 * 
	 */
	 ...
	 * <p>
	 * <b>Note:</b> if the name you are trying to generate should be following a specific ordering,
	 * you should use the constructor with the ordering specified below.
	 * 
	 ...
	 * <p>
	 * it's is assumed the table contains OID/String pairs, and the contents of the table are copied
	 * into an internal table as part of the construction process. The ordering vector should
	...
	/**
	 * Takes an X509 dir name as a string of the format "C=AU, ST=Victoria", or some such,
	 * converting it into an ordered set of name attributes.
	 * 
	 * @deprecated use X500Name, X500NameBuilder
	 */
	...
	/**
	 * Takes an X509 dir name as a string of the format "C=AU, ST=Victoria", or some such,
	 * converting it into an ordered set of name attributes with each string value being converted
	 * to its associated ASN.1 type using the passed in converter.
	 * 
	 * @deprecated use X500Name, X500NameBuilder
	 */
	...
	/**
	 * Takes an X509 dir name as a string of the format "C=AU, ST=Victoria", or some such,
	 * converting it into an ordered set of name attributes. If reverse is true, create the encoded
	 * version of the sequence starting from the last element in the string.
	 * 
	 * @deprecated use X500Name, X500NameBuilder
	 */
	 ...
	 * Takes an X509 dir name as a string of the format "C=AU, ST=Victoria", or some such,
	 * converting it into an ordered set of name attributes with each string value being converted
	 * to its associated ASN.1 type using the passed in converter. If reverse is true the ASN.1
	 * sequence representing the DN will be built by starting at the end of the string, rather than
	 * the start.
	 * 
	 * @deprecated use X500Name, X500NameBuilder
	 */
	...
	/**
	 * Takes an X509 dir name as a string of the format "C=AU, ST=Victoria", or some such,
	 * converting it into an ordered set of name attributes. lookUp should provide a table of
	 * lookups, indexed by lowercase only strings and yielding a ASN1ObjectIdentifier, other than
	 * that OID. and numeric oids will be processed automatically. <br>
	 ...
	 * converting it into an ordered set of name attributes. lookUp should provide a table of
	 * lookups, indexed by lowercase only strings and yielding a ASN1ObjectIdentifier, other than
	 * that OID. and numeric oids will be processed automatically. The passed in converter is used
	 * to convert the string values to the right of each equals sign to their ASN.1 counterparts.
	 * <br>
	 * 
	 * @param reverse
	...
	/**
	 * @param inOrder
	 *            if true the order of both X509 names must be the same, as well as the values
	 ...
	 * starting with the last element in the sequence (ala RFC 2253), otherwise the string will
	 * begin with the first element of the structure. If no string definition for the oid is found
	 * in oidSymbols the string value of the oid is added. Two standard symbol tables are provided
