 ...
 * <ul>
 * <li>a) the responder supports the extension and is allowed to publish the certificate:
 * RequestedCertificate returned including the requested certificate
 * <li>b) the responder supports the extension but is NOT allowed to publish the certificate:
 * RequestedCertificate returned including an empty OCTET STRING
 * <li>c) the responder does not support the extension: RequestedCertificate is not included in the
 * response
 * </ul>
 * Clients requesting RetrieveIfAllowed MUST be able to handle these cases. If any of the OCTET
 * STRING options is used, it MUST contain the DER encoding of the requested certificate.
 * 
 * <pre>
