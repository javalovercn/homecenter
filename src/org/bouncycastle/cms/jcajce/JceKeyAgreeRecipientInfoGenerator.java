...
public class JceKeyAgreeRecipientInfoGenerator extends KeyAgreeRecipientInfoGenerator {
	...
	public JceKeyAgreeRecipientInfoGenerator(ASN1ObjectIdentifier keyAgreementOID, PrivateKey senderPrivateKey, PublicKey senderPublicKey,
	...
	/**
	 * Add a recipient identified by the passed in subjectKeyID and the for the passed in public
	 * key.
	 *
	 * @param subjectKeyID
	 *            identifier actual recipient will use to match the private key.
	 * @param publicKey
	 *            the public key for encrypting the secret key.
	 * @return the current instance.
	 * @throws CertificateEncodingException
	 */
	...
	public ASN1Sequence generateRecipientEncryptedKeys(AlgorithmIdentifier keyAgreeAlgorithm, AlgorithmIdentifier keyEncryptionAlgorithm,
			GenericKey contentEncryptionKey) throws CMSException {
			...
			try {
				...
				if (CMSUtils.isMQV(keyAgreementOID)) {
					...
					byte[] ukmKeyingMaterial = ecc_cms_Generator.generateKDFMaterial(keyEncryptionAlgorithm,
							keySizeProvider.getKeySize(keyEncryptionAlgorithm.getAlgorithm()), userKeyingMaterial);
