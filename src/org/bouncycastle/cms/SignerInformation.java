...
public class SignerInformation {
	...
	/**
	 * Protected constructor. In some cases clients have their own idea about how to encode the
	 * signed attributes and calculate the signature. This constructor is to allow developers to
	 * deal with that by extending off the class and overridng methods like getSignedAttributes().
	 *
	 * @param baseInfo
	...
	public SignerInformationStore getCounterSignatures() {
		...
		/*
		 * The countersignature attribute MUST be an unsigned attribute; it MUST NOT be a signed
		 * attribute, an authenticated attribute, an unauthenticated attribute, or an unprotected
		 * attribute.
		 */
		...
		/*
		 * The UnsignedAttributes syntax is defined as a SET OF Attributes. The UnsignedAttributes
		 * in a signerInfo may include multiple instances of the countersignature attribute.
		 */
		...
		for (int i = 0; i < allCSAttrs.size(); ++i) {
			...
			/*
			 * A countersignature attribute can have multiple attribute values. The syntax is
			 * defined as a SET OF AttributeValue, and there MUST be one or more instances of
			...
			for (Enumeration en = values.getObjects(); en.hasMoreElements();) {
				/*
				 * Countersignature values have the same meaning as SignerInfo values for ordinary
				 * signatures, except that:
				 * 
				 * 1. The signedAttributes field MUST NOT contain a content-type attribute; there is
				 * no content type for countersignatures.
				 * 
				 * 2. The signedAttributes field MUST contain a message-digest attribute if it
				 * contains any other attributes.
				 * 
				 * 3. The input to the message-digesting process is the contents octets of the DER
				 * encoding of the signatureValue field of the SignerInfo value with which the
			...
			if (signedAttributeSet == null && resultDigest != null) {
				if (contentVerifier instanceof RawContentVerifier) {
					...
					if (encName.equals("RSA")) {
						DigestInfo digInfo = new DigestInfo(new AlgorithmIdentifier(digestAlgorithm.getAlgorithm(), DERNull.INSTANCE),
								resultDigest);
	...
	/**
	 * Verify that the given verifier can successfully verify the signature on this
	 * SignerInformation object.
	 *
	 * @param verifier
	 *            a suitably configured SignerInformationVerifier.
	 * @return true if the signer information is verified, false otherwise.
	 * @throws org.bouncycastle.cms.CMSVerifierCertificateNotValidException
	 *             if the provider has an associated certificate and the certificate is not valid at
	 *             the time given as the SignerInfo's signing time.
	 ...
	 *            the signerInfo to be used as the basis.
	...
	public static SignerInformation replaceUnsignedAttributes(SignerInformation signerInformation, AttributeTable unsignedAttributes) {
		...
		return new SignerInformation(
				new SignerInfo(sInfo.getSID(), sInfo.getDigestAlgorithm(), sInfo.getAuthenticatedAttributes(),
						sInfo.getDigestEncryptionAlgorithm(), sInfo.getEncryptedDigest(), unsignedAttr),
	...
	public static SignerInformation addCounterSigners(SignerInformation signerInformation, SignerInformationStore counterSigners) {
		...
		return new SignerInformation(
				new SignerInfo(sInfo.getSID(), sInfo.getDigestAlgorithm(), sInfo.getAuthenticatedAttributes(),
						sInfo.getDigestEncryptionAlgorithm(), sInfo.getEncryptedDigest(), new DERSet(v)),
				signerInformation.contentType, signerInformation.content, null);
	}
}
