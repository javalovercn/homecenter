...
public abstract class RecipientInformation {
	...
	RecipientInformation(AlgorithmIdentifier keyEncAlg, AlgorithmIdentifier messageAlgorithm, CMSSecureReadable secureReadable,
	...
	/**
	 * Return the content digest calculated during the read of the content if one has been
	 * generated. This will only happen if we are dealing with authenticated data and authenticated
		...
		if (resultMac == null) {
			if (operator.isMacBased()) {
				if (additionalData != null) {
					try {
						Streams.drain(operator
								.getInputStream(new ByteArrayInputStream(additionalData.getAuthAttributes().getEncoded(ASN1Encoding.DER))));
					} catch (IOException e) {
