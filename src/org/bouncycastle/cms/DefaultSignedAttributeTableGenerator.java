...
public class DefaultSignedAttributeTableGenerator implements CMSAttributeTableGenerator {
	 ...
	 * contentType, signingTime, and messageDigest. If the constructor using an AttributeTable was
	 * used, entries in it for contentType, signingTime, and messageDigest will override the
	 * generated ones.
	 *
	 * @param parameters
		...
		if (!std.contains(CMSAttributes.cmsAlgorithmProtect)) {
			Attribute attr = new Attribute(CMSAttributes.cmsAlgorithmProtect,
					new DERSet(new CMSAlgorithmProtection(
							...
							(AlgorithmIdentifier) parameters.get(CMSAttributeTableGenerator.SIGNATURE_ALGORITHM_IDENTIFIER))));
			std.put(attr.getAttrType(), attr);
		}
