...
public class BCECPublicKey implements ECPublicKey, org.bouncycastle.jce.interfaces.ECPublicKey, ECPointEncoder {
	...
	public BCECPublicKey(String algorithm, org.bouncycastle.jce.spec.ECPublicKeySpec spec, ProviderConfiguration configuration) {
		this.algorithm = algorithm;
		this.q = spec.getQ();
		...
		if (spec.getParams() != null) // can be null if implictlyCa
		{
			...
			this.q = EC5Util.convertCurve(ellipticCurve).createPoint(spec.getQ().getAffineXCoord().toBigInteger(),
	...
	public BCECPublicKey(String algorithm, ECPublicKeyParameters params, org.bouncycastle.jce.spec.ECParameterSpec spec,
		...
		this.q = EC5Util.convertCurve(ecSpec.getCurve()).createPoint(params.getQ().getAffineXCoord().toBigInteger(),
				params.getQ().getAffineYCoord().toBigInteger());
	...
	public byte[] getEncoded() {
		...
		if (ecSpec == null) {
			p = (ASN1OctetString) new X9ECPoint(
					curve.createPoint(this.getQ().getXCoord().toBigInteger(), this.getQ().getYCoord().toBigInteger(), withCompression))
							.toASN1Primitive();
		} else {
			p = (ASN1OctetString) new X9ECPoint(curve.createPoint(this.getQ().getAffineXCoord().toBigInteger(),
