	...
	protected Key engineDoPhase(Key key, boolean lastPhase) throws InvalidKeyException, IllegalStateException {
		...
		if (agreement instanceof ECMQVBasicAgreement) {
			if (!(key instanceof MQVPublicKey)) {
				...
				ECPublicKeyParameters ephemKey = (ECPublicKeyParameters) ECUtil
		...
		if (agreement instanceof ECMQVBasicAgreement) {
			...
			if (!(key instanceof MQVPrivateKey) && !(parameterSpec instanceof MQVParameterSpec)) {
				throw new InvalidKeyException(
						kaAlgorithm + " key agreement requires " + getSimpleName(MQVParameterSpec.class) + " for initialisation");
			}
			...
			if (!(key instanceof PrivateKey)) {
				throw new InvalidKeyException(
						kaAlgorithm + " key agreement requires " + getSimpleName(ECPrivateKey.class) + " for initialisation");
			}
