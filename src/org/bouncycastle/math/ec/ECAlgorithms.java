...
public class ECAlgorithms {
	...
	/*
	 * "Shamir's Trick", originally due to E. G. Straus (Addition chains of vectors. American
	 * Mathematical Monthly, 71(7):806-808, Aug./Sept. 1964) <pre> Input: The points P, Q, scalar k
	 * = (km?, ... , k1, k0) and scalar l = (lm?, ... , l1, l0). Output: R = k * P + l * Q. 1: Z <-
	 * P + Q 2: R <- O 3: for i from m-1 down to 0 do 4: R <- R + R {point doubling} 5: if (ki = 1)
	 * and (li = 0) then R <- R + P end if 6: if (ki = 0) and (li = 1) then R <- R + Q end if 7: if
	...
	static ECPoint implShamirsTrickJsf(ECPoint P, BigInteger k, ECPoint Q, BigInteger l) {
		...
		ECPoint[] table = new ECPoint[] { points[3].negate(), points[2].negate(), points[1].negate(), points[0].negate(), infinity,
	...
	private static ECPoint implShamirsTrickWNaf(ECPoint[] preCompP, ECPoint[] preCompNegP, byte[] wnafP, ECPoint[] preCompQ,
