...
public class DiscoverEndomorphisms {
	...
	private static void printGLVTypeBParameters(X9ECParameters x9, BigInteger lambda, ECFieldElement[] betas) {
		/*
		 * Check the basic premise of the endomorphism: that multiplying a point by lambda preserves
		 * the y-coordinate
		 */
		...
		/*
		 * Determine which of the beta values corresponds with this choice of lambda, by checking
		 * that it scales the x-coordinate the same way a point-multiplication by lambda does.
		 */
		...
		/*
		 * These parameters are used to avoid division when decomposing the scalar in a GLV point
		 * multiplication. The precision is determined by 'bits', even 2 bits is enough, but we try
		 * to get more whilst keeping it 8-bit aligned and limiting the possible growth of product
		 * sizes on a 32-bit machine.
		 */
	...
	private static BigInteger[] extEuclidBezout(BigInteger[] ab) {
		...
		if (r1.signum() <= 0) {
			/*
			 * NOTE: This case occurred while testing on curves over tiny fields; probably due to a
			 * 0 input.
			 */
			return null;
		}
