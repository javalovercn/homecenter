...
public abstract class WNafUtil {
	 ...
	 * 
	 * @param width
	 *            The width <code>w</code> of the Window NAF. The width is defined as the minimal
	 *            number <code>w</code>, such that for any <code>w</code> consecutive digits in the
	 *            resulting representation, at most one is non-zero.
	 * @param k
	 ...
	 *            a monotonically increasing list of bit sizes at which to increment the window
	 *            width
	 * @return the window size to use
	 */
			...
			if (reqPreCompLen == 1) {
				...
				if (reqPreCompLen == 2) {
					...
					if (twiceP == null) {
						 ...
						 * (quasi-)isomorphism where 'twiceP' is "affine", so that the subsequent
						 * additions are cheaper. This also requires scaling the initial point's X,
						 * Y coordinates, and reversing the isomorphism as part of the subsequent
						 * normalization.
						 * 
						 * NOTE: The correctness of this optimization depends on: 1) additions do
						 * not use the curve's A, B coefficients. 2) no special cases (i.e. Q +/- Q)
						 * when calculating 1P, 3P, 5P, ...
						 */
