...
public class GMSSPrivateKeyParameters extends GMSSKeyParameters {
	 ...
	 *
	 * @param currentSeed
	 *            seed for the generation of private OTS keys for the current subtrees
	 * @param nextNextSeed
	...
	public GMSSPrivateKeyParameters(byte[][] currentSeed, byte[][] nextNextSeed, byte[][][] currentAuthPath, byte[][][] nextAuthPath,
			Treehash[][] currentTreehash, Treehash[][] nextTreehash, Vector[] currentStack, Vector[] nextStack, Vector[][] currentRetain,
			Vector[][] nextRetain, byte[][] nextRoot, byte[][] currentRootSig, GMSSParameters gmssParameterset,
		...
		this(null, currentSeed, nextNextSeed, currentAuthPath, nextAuthPath, null, currentTreehash, nextTreehash, currentStack, nextStack,
				currentRetain, nextRetain, null, null, null, null, nextRoot, null, currentRootSig, null, gmssParameterset, digestProvider);
	}
	 ...
	 *            the GMSS Parameterset
	 */
	public GMSSPrivateKeyParameters(int[] index, byte[][] currentSeeds, byte[][] nextNextSeeds, byte[][][] currentAuthPaths,
	...
	/**
	 * This method computes the authpath (AUTH) for the current tree, Additionally the root
	 * signature for the next tree (SIG+), the authpath (AUTH++) and root (ROOT++) for the tree
	 * after next in layer <code>layer</code>, and the LEAF++^1 for the next next tree in the layer
	 * above are updated This method is used by nextKey()
	 *
	 * @param layer
	 */
			...
			for (int tmp = 1; tmp <= (H - K) / 2; tmp++) {
				...
				if (minTreehash >= 0) {
					try {
						...
						System.arraycopy(this.currentTreehash[layer][minTreehash].getSeedActive(), 0, seed, 0, mdLength);
	 ...
	 * @param Phi
	 *            the leaf index
	 * @return The largest <code>h</code> with <code>2^h | Phi</code> if <code>Phi!=0</code> else
	 *         return <code>-1</code>
	 */
