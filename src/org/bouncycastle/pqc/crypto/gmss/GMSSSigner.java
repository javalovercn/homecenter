...
public class GMSSSigner implements MessageSigner {
	...
	/**
	 * Array of strings containing the name of the hash function used by the OTS and the
	...
	public byte[] generateSignature(byte[] message) {
		...
		for (int i = numLayer - 1 - 1; i >= 0; i--) {
			...
			System.arraycopy(authPathBytes, 0, gmssSigNextPart, (helpGmssSig.length + indexBytes.length + subtreeRootSig[i].length),
	...
	/**
	 * This function verifies the signature of the message that has been updated, with the aid of
	 * the public key.
	 *
	 * @param message
	 *            the message
