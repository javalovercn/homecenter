...
public class RainbowPrivateKey extends ASN1Object {
	...
	private RainbowPrivateKey(ASN1Sequence seq) {
		...
		for (int i = 0; i < numOfLayers; i++) {
			Layer l = new Layer(vi[i], vi[i + 1], RainbowUtil.convertArray(alphas[i]), RainbowUtil.convertArray(betas[i]),
					RainbowUtil.convertArray(gammas[i]), RainbowUtil.convertArray(etas[i]));
			this.layers[i] = l;
