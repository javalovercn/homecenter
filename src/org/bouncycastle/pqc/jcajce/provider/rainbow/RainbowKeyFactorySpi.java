...
public class RainbowKeyFactorySpi extends KeyFactorySpi implements AsymmetricKeyInfoConverter {
	 ...
	 * following key specifications are supported: {@link RainbowPrivateKeySpec},
	...
	public final KeySpec engineGetKeySpec(Key key, Class keySpec) throws InvalidKeySpecException {
		if (key instanceof BCRainbowPrivateKey) {
			if (PKCS8EncodedKeySpec.class.isAssignableFrom(keySpec)) {
				...
				return new RainbowPrivateKeySpec(privKey.getInvA1(), privKey.getB1(), privKey.getInvA2(), privKey.getB2(), privKey.getVi(),
						privKey.getLayers());
			}
			...
			if (X509EncodedKeySpec.class.isAssignableFrom(keySpec)) {
				...
				return new RainbowPublicKeySpec(pubKey.getDocLength(), pubKey.getCoeffQuadratic(), pubKey.getCoeffSingular(),
						pubKey.getCoeffScalar());
			}
		} else {
	...
	/**
	 * Translates a key into a form known by the FlexiProvider. Currently the following key types
	 * are supported: RainbowPrivateKey, RainbowPublicKey.
	 *
	 * @param key
	 *            the key
