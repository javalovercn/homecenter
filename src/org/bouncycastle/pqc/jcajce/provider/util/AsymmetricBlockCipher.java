...
public abstract class AsymmetricBlockCipher extends CipherSpiExt {
	...
	/**
	 * Return the block size (in bytes). Note: although the ciphers extending this class are not
	 * block ciphers, the method was adopted to return the maximal plaintext and ciphertext sizes
	 * for non hybrid ciphers. If the cipher is hybrid, it returns 0.
	 *
	 * @return if the cipher is not a hybrid one the max plain/cipher text size is returned,
	 ...
	 * of the next update or doFinal operation, given the input length <tt>inLen</tt> (in bytes).
	 ...
	 * <p>
	 * If the input length plus the length of the buffered data exceeds the maximum length,
	 * <tt>0</tt> is returned.
	 * </p>
	 * 
	 * @param inLen
	 ...
	 * the underlying cipher implementation is supposed to generate the required parameters itself
	 * (using provider-specific default or random values) if it is being initialized for encryption,
	 * and raise an InvalidKeyException if it is being initialized for decryption. The generated
	 ...
	 *             if the given algortihm parameters are inappropriate for this cipher, or if this
	...
	/**
	 * Continue a multiple-part encryption or decryption operation. This method just writes the
	 ...
	 * method uses the specific values {@link #maxPlainTextSize} and {@link #cipherTextSize} which
	 * are set by the implementations. If the input length plus the length of the internal buffer is
	...
	protected void checkLength(int inLen) throws IllegalBlockSizeException {
		...
		if (opMode == ENCRYPT_MODE) {
			if (inLength > maxPlainTextSize) {
				throw new IllegalBlockSizeException("The length of the plaintext (" + inLength + " bytes) is not supported by "
						+ "the cipher (max. " + maxPlainTextSize + " bytes).");
			}
		} else if (opMode == DECRYPT_MODE) {
			if (inLength != cipherTextSize) {
				throw new IllegalBlockSizeException(
						"Illegal ciphertext length (expected " + cipherTextSize + " bytes, was " + inLength + " bytes).");
			}
		}
	 ...
	 *
	 * @param input
	 *            the message to be encrypted (usually the message length is less than or equal to
	 *            maxPlainTextSize)
	 ...
	 *
	 * @param input
	 *            the ciphertext to be decrypted (the ciphertext length is less than or equal to
	 *            maxCipherTextSize)
