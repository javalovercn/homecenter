...
public class McElieceKeyFactorySpi extends KeyFactorySpi {
	...
	/**
	 * Converts, if possible, a key specification into a {@link BCMcEliecePublicKey}. Currently, the
	 * following key specifications are supported: {@link McEliecePublicKeySpec},
	...
	/**
	 * Converts, if possible, a key specification into a {@link BCMcEliecePrivateKey}. Currently,
	 * the following key specifications are supported: {@link McEliecePrivateKeySpec},
		...
		if (keySpec instanceof McEliecePrivateKeySpec) {
			...
			try {
				...
				return new BCMcEliecePrivateKey(
						new McEliecePrivateKeySpec(OID, n, k, encFieldPoly, encGoppaPoly, encSInv, encP1, encP2, encH, encQInv));
	 ...
	 * <ul>
	 * <li>for McEliecePublicKey: {@link X509EncodedKeySpec}, {@link McEliecePublicKeySpec}</li>
	 * <li>for McEliecePrivateKey: {@link PKCS8EncodedKeySpec}, {@link McEliecePrivateKeySpec}</li>.
	 * </ul>
	 *
	 * @param key
	 *            the key
	...
	public KeySpec getKeySpec(Key key, Class keySpec) throws InvalidKeySpecException {
		if (key instanceof BCMcEliecePrivateKey) {
			if (PKCS8EncodedKeySpec.class.isAssignableFrom(keySpec)) {
				...
				return new McEliecePrivateKeySpec(OID, privKey.getN(), privKey.getK(), privKey.getField(), privKey.getGoppaPoly(),
						privKey.getSInv(), privKey.getP1(), privKey.getP2(), privKey.getH(), privKey.getQInv());
			}
	...
	/**
	 * Translates a key into a form known by the FlexiProvider. Currently, only the following
	 * "source" keys are supported: {@link BCMcEliecePrivateKey}, {@link BCMcEliecePublicKey}.
	 *
	 * @param key
	 *            the key
	...
	public PrivateKey generatePrivate(PrivateKeyInfo pki) throws InvalidKeySpecException {
		...
		try {
			...
			return new BCMcEliecePrivateKey(key.getOID().getId(), key.getN(), key.getK(), key.getField(), key.getGoppaPoly(), key.getSInv(),
					key.getP1(), key.getP2(), key.getH(), key.getQInv());
		} catch (IOException cce) {
			throw new InvalidKeySpecException("Unable to decode PKCS8EncodedKeySpec");
		}
	}
