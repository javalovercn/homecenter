...
public class McElieceCCA2KeyFactorySpi extends KeyFactorySpi {
	 ...
	 * the following key specifications are supported: {@link McElieceCCA2PublicKeySpec},
	...
	/**
	 * Converts, if possible, a key specification into a {@link BCMcElieceCCA2PrivateKey}.
	 * Currently, the following key specifications are supported:
	 * {@link McElieceCCA2PrivateKeySpec}, {@link PKCS8EncodedKeySpec}.
	 *
	 * @param keySpec
	 *            the key specification
		...
		if (keySpec instanceof McElieceCCA2PrivateKeySpec) {
			...
			try {
				...
				return new BCMcElieceCCA2PrivateKey(
						new McElieceCCA2PrivateKeySpec(OID, n, k, encFieldPoly, encGoppaPoly, encP, encH, encQInv));
	...
	public KeySpec getKeySpec(Key key, Class keySpec) throws InvalidKeySpecException {
		if (key instanceof BCMcElieceCCA2PrivateKey) {
			if (PKCS8EncodedKeySpec.class.isAssignableFrom(keySpec)) {
				...
				return new McElieceCCA2PrivateKeySpec(OID, privKey.getN(), privKey.getK(), privKey.getField(), privKey.getGoppaPoly(),
	 ...
	 * "source" keys are supported: {@link BCMcElieceCCA2PrivateKey},
	...
	public PrivateKey generatePrivate(PrivateKeyInfo pki) throws InvalidKeySpecException {
		...
		try {
			...
			return new BCMcElieceCCA2PrivateKey(key.getOID().getId(), key.getN(), key.getK(), key.getField(), key.getGoppaPoly(),
					key.getP(), key.getH(), key.getQInv());
		} catch (IOException cce) {
			throw new InvalidKeySpecException("Unable to decode PKCS8EncodedKeySpec");
		}
	}
