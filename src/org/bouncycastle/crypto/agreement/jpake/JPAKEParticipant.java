 ...
 * <a href="http://grouper.ieee.org/groups/1363/Research/contributions/hao-ryan-2008.pdf"> "Password
 * Authenticated Key Exchange by Juggling, 2008."</a>
 * <p>
 ...
 * <li>{@link #validateRound1PayloadReceived(JPAKERound1Payload)} - use the payload received from
 * the other participant</li>
 * <li>{@link #createRound2PayloadToSend()} - and send the payload to the other participant</li>
 * <li>{@link #validateRound2PayloadReceived(JPAKERound2Payload)} - use the payload received from
 * the other participant</li>
 * <li>{@link #calculateKeyingMaterial()}</li>
 * <li>{@link #createRound3PayloadToSend(BigInteger)} - and send the payload to the other
 * participant</li>
 * <li>{@link #validateRound3PayloadReceived(JPAKERound3Payload, BigInteger)} - use the payload
 * received from the other participant</li>
 * </ol>
 * <p>
 * Each side should derive a session key from the keying material returned by
 * {@link #calculateKeyingMaterial()}. The caller is responsible for deriving the session key using
 * a secure key derivation function (KDF).
 * <p>
 * Round 3 is an optional key confirmation process. If you do not execute round 3, then there is no
 * assurance that both participants are using the same key. (i.e. if the participants used different
 * passwords, then their session keys will differ.)
 * <p>
 * If the round 3 validation succeeds, then the keys are guaranteed to be the same on both sides.
 * <p>
 * The symmetric design can easily support the asymmetric cases when one party initiates the
 * communication. e.g. Sometimes the round1 payload and round2 payload may be sent in one pass.
 * Also, in some cases, the key confirmation payload can be sent together with the round2 payload.
 * These are the trivial techniques to optimize the communication.
 * <p>
 * The key confirmation process is implemented as specified in <a href=
 * "http://csrc.nist.gov/publications/nistpubs/800-56A/SP800-56A_Revision1_Mar08-2007.pdf">NIST SP
 * 800-56A Revision 1</a>, Section 8.2 Unilateral Key Confirmation for Key Agreement Schemes.
 * <p>
 * This class is stateful and NOT threadsafe. Each instance should only be used for ONE complete
 * J-PAKE exchange (i.e. a new {@link JPAKEParticipant} should be constructed for each new J-PAKE
 * exchange).
 * <p>
 */
public class JPAKEParticipant {
	...
	/**
	 * Unique identifier of this participant. The two participants in the exchange must NOT share
	 * the same id.
	 */
	private final String participantId;

	/**
	 * Shared secret. This only contains the secret between construction and the call to
	 ...
	 * {@link JPAKEPrimeOrderGroups#NIST_3072} prime order group, a SHA-256 digest, and a default
	 * {@link SecureRandom} implementation.
	 * <p>
	 * After construction, the {@link #getState() state} will be {@link #STATE_INITIALIZED}.
	 *
	 * @param participantId
	 *            unique identifier of this participant. The two participants in the exchange must
	 *            NOT share the same id.
	 * @param password
	 *            shared secret. A defensive copy of this array is made (and cleared once
	 *            {@link #calculateKeyingMaterial()} is called). Caller should clear the input
	 ...
	 *            password as soon as possible.
	 * @param group
	 *            prime order group. See {@link JPAKEPrimeOrderGroups} for standard groups
	 * @throws NullPointerException
	 *             if any argument is null
	 * @throws IllegalArgumentException
	 *             if password is empty
	 ...
	 *            prime order group. See {@link JPAKEPrimeOrderGroups} for standard groups
	 * @param digest
	 *            digest to use during zero knowledge proofs and key confirmation (SHA-256 or
	 *            stronger preferred)
	 * @param random
	...
	public JPAKEParticipant(String participantId, char[] password, JPAKEPrimeOrderGroup group, Digest digest, SecureRandom random) {
		 ...
		 * 
		 * This array will contain the password for the lifetime of this participant BEFORE {@link
		 * #calculateKeyingMaterial()} is called.
		 * 
		 * i.e. When {@link #calculateKeyingMaterial()} is called, the array will be cleared in
		 * order to remove the password from memory.
		 * 
		 * The caller is responsible for clearing the original password array given as input to this
		 * constructor.
		 */
	 ...
	 *
	 * @throws IllegalStateException
	 *             if called prior to {@link #validateRound1PayloadReceived(JPAKERound1Payload)}, or
	 *             multiple times
	 */
	 ...
	 * password is through derivation of different keys (which can be detected explicitly by
	 * executing round 3 and round 4)
	 * <p>
	 ...
	 * derived from the keying material, then you must handle detection of incorrect keys. If you
	 * want to handle this detection explicitly, you can optionally perform rounds 3 and 4. See
	 * {@link JPAKEParticipant} for details on how to execute rounds 3 and 4.
	 * <p>
	 ...
	 *
	 * @throws IllegalStateException
	 *             if called prior to {@link #validateRound2PayloadReceived(JPAKERound2Payload)}, or
	 *             if called multiple times.
	 */
	public BigInteger calculateKeyingMaterial() {
		 ...
		 * 
		 * Also set the field to null as a flag to indicate that the key has already been
		 * calculated.
		 */
		...
		/*
		 * Clear the ephemeral private key fields as well. Note that we're relying on the garbage
		 * collector to do its job to clean these up. The old objects will hang around in memory
		 * until the garbage collector destroys them.
		 * 
		 * If the ephemeral private keys x1 and x2 are leaked, the attacker might be able to
	 ...
	 *            The keying material as returned from {@link #calculateKeyingMaterial()}.
	 * @throws IllegalStateException
	 *             if called prior to {@link #calculateKeyingMaterial()}, or multiple times
	 */
	public JPAKERound3Payload createRound3PayloadToSend(BigInteger keyingMaterial) {
		...
		BigInteger macTag = JPAKEUtil.calculateMacTag(this.participantId, this.partnerParticipantId, this.gx1, this.gx2, this.gx3, this.gx4,
				keyingMaterial, this.digest);
	...
	public void validateRound3PayloadReceived(JPAKERound3Payload round3PayloadReceived, BigInteger keyingMaterial) throws CryptoException {
		...
		if (this.state < STATE_KEY_CALCULATED) {
			throw new IllegalStateException(
					"Keying material must be calculated validated prior to validating Round3 payload for " + this.participantId);
		}
		...
		JPAKEUtil.validateMacTag(this.participantId, this.partnerParticipantId, this.gx1, this.gx2, this.gx3, this.gx4, keyingMaterial,
				this.digest, round3PayloadReceived.getMacTag());
