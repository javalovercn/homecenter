...
public class JPAKEUtil {
	 ...
	 */
	public static BigInteger[] calculateZeroKnowledgeProof(BigInteger p, BigInteger q, BigInteger g, BigInteger gx, BigInteger x,
			String participantId, Digest digest, SecureRandom random) {
	...
	private static BigInteger calculateHashForZeroKnowledgeProof(BigInteger g, BigInteger gr, BigInteger gx, String participantId,
			Digest digest) {
		digest.reset();
	 ...
	 * <p>
	 * <blockquote> Alice could simply check ga != 1 to ensure it is a generator. In fact, as we
	 * will explain in Section 3, (x1 + x3 + x4 ) is random over Zq even in the face of active
	 * attacks. Hence, the probability for ga = 1 is extremely small - on the order of 2^160 for
	 * 160-bit q. </blockquote>
	 *
	 * @throws CryptoException
	 *             if ga is 1
	 */
	 ...
	 */
	public static void validateZeroKnowledgeProof(BigInteger p, BigInteger q, BigInteger g, BigInteger gx, BigInteger[] zeroKnowledgeProof,
			String participantId, Digest digest) throws CryptoException {
		...
		if (!(gx.compareTo(ZERO) == 1 && // g^x > 0
				...
				/*
				 * Below, I took an straightforward way to compute g^r * g^x^h, which needs 2 exp.
				 * Using a simultaneous computation technique would only need 1 exp.
				 */
	 ...
	 * </pre>
	 */
	public static BigInteger calculateKeyingMaterial(BigInteger p, BigInteger q, BigInteger gx4, BigInteger x2, BigInteger s,
			BigInteger B) {
	...
	public static void validateParticipantIdsDiffer(String participantId1, String participantId2) throws CryptoException {
		if (participantId1.equals(participantId2)) {
			throw new CryptoException("Both participants are using the same participantId (" + participantId1 + "). This is not allowed. "
	...
	/**
	 * Calculates the MacTag (to be used for key confirmation), as defined by <a href=
	 ...
	 * </pre>
	 */
	public static BigInteger calculateMacTag(String participantId, String partnerParticipantId, BigInteger gx1, BigInteger gx2,
			BigInteger gx3, BigInteger gx4, BigInteger keyingMaterial, Digest digest) {
		...
		/*
		 * MacData = "KC_1_U" || participantId_Alice || participantId_Bob || gx1 || gx2 || gx3 ||
		 * gx4.
		 */
	 ...
	 *             if the participantId strings are equal.
	 */
	public static void validateMacTag(String participantId, String partnerParticipantId, BigInteger gx1, BigInteger gx2, BigInteger gx3,
			BigInteger gx4, BigInteger keyingMaterial, Digest digest, BigInteger partnerMacTag) throws CryptoException {
		/*
		 * Calculate the expected MacTag using the parameters as the partner would have used when
		 * the partner called calculateMacTag.
		 * 
		 * i.e. basically all the parameters are reversed. participantId <-> partnerParticipantId x1
		 * <-> x3 x2 <-> x4
		 */
