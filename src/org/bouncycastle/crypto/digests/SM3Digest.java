...
public class SM3Digest extends GeneralDigest {
	 ...
	 * 
	 * 
	 * FFj(X;Y;Z) = X XOR Y XOR Z when 0 < = j < = 15 = (X AND Y) OR (X AND Z) OR (Y AND Z) when 16
	 * < = j < = 63
	 * 
	 * GGj(X;Y;Z) = X XOR Y XOR Z when 0 < = j < = 15 = (X AND Y) OR (NOT X AND Z) when 16 < = j < =
	 * 63
	 * 
	 ...
	 * 
	 * 
	 * P0(X) = X XOR (X <<< 9) XOR (X <<< 17) ## ROLL, not SHIFT P1(X) = X XOR (X <<< 15) XOR (X <<<
	 * 23) ## ROLL, not SHIFT
	 * 
	 ...
	 * 
	 * ROLL 9 : ((x << 9) | (x >>> (32-9)))) ROLL 17 : ((x << 17) | (x >>> (32-17))) ROLL 15 : ((x
	 * << 15) | (x >>> (32-15))) ROLL 23 : ((x << 23) | (x >>> (32-23)))
	 * 
	 */
