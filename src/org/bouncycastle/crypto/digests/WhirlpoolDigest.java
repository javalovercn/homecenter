...
public final class WhirlpoolDigest implements ExtendedDigest, Memoable {
	...
	private static final int[] SBOX = { ,
	...
	public WhirlpoolDigest() {
		...
		for (int r = 1; r <= ROUNDS; r++) {
			int i = 8 * (r - 1);
			_rc[r] = (C0[i] & 0xff00000000000000L) ^ (C1[i + 1] & 0x00ff000000000000L) ^ (C2[i + 2] & 0x0000ff0000000000L)
					^ (C3[i + 3] & 0x000000ff00000000L) ^ (C4[i + 4] & 0x00000000ff000000L) ^ (C5[i + 5] & 0x0000000000ff0000L)
	...
	private long packIntoLong(int b7, int b6, int b5, int b4, int b3, int b2, int b1, int b0) {
		return ((long) b7 << 56) ^ ((long) b6 << 48) ^ ((long) b5 << 40) ^ ((long) b4 << 32) ^ ((long) b3 << 24) ^ ((long) b2 << 16)
				^ ((long) b1 << 8) ^ b0;
	}

	/*
	 * int's are used to prevent sign extension. The values that are really being used are actually
	 * just 0..255
	 */
	...
	private long bytesToLongFromBuffer(byte[] buffer, int startPos) {
		long rv = (((buffer[startPos + 0] & 0xffL) << 56) | ((buffer[startPos + 1] & 0xffL) << 48) | ((buffer[startPos + 2] & 0xffL) << 40)
				| ((buffer[startPos + 3] & 0xffL) << 32) | ((buffer[startPos + 4] & 0xffL) << 24) | ((buffer[startPos + 5] & 0xffL) << 16)
	...
	/*
	 * increment() can be implemented in this way using 2 arrays or by having some temporary
	 * variables that are used to set the value provided by EIGHT[i] and carry within the loop.
	 * 
	 * not having done any timing, this seems likely to be faster at the slight expense of
	 * 32*(sizeof short) bytes
	 */
	...
	private void finish() {
		/*
		 * this makes a copy of the current bit length. at the expense of an object creation of 32
		 * bytes rather than providing a _stopCounting boolean which was the alternative I could
		 * think of.
		 */
