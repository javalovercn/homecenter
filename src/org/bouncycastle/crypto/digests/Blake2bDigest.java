...
public class Blake2bDigest implements ExtendedDigest {
	...
	/*
	 * private int fanout = 1; // 0-255 private int depth = 1; // 1 - 255 private int leafLength= 0;
	 * private long nodeOffset = 0L; private int nodeDepth = 0; private int innerHashLength = 0;
	 */
	...
	/**
	 * Blake2b with key, required digest length, salt and personalization. After calling the
	 * doFinal() method, the key, the salt and the personal string will remain and might be used for
	 * further computations with this instance. The key can be overwritten using the clearKey()
	...
	private final byte[] long2bytes(long longValue) {
		return new byte[] { (byte) longValue, (byte) (longValue >> 8), (byte) (longValue >> 16), (byte) (longValue >> 24),
