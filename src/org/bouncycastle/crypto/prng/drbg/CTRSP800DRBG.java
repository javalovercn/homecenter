...
public class CTRSP800DRBG implements SP80090DRBG {
	...
	/*
	 * 1. chaining_value = 0^outlen . Comment: Set the first chaining value to outlen zeros. 2. n =
	 * len (data)/outlen. 3. Starting with the leftmost bits of data, split the data into n blocks
	 * of outlen bits each, forming block(1) to block(n). 4. For i = 1 to n do 4.1 input_block =
	 * chaining_value ^ block(i) . 4.2 chaining_value = Block_Encrypt (Key, input_block). 5.
	 * output_block = chaining_value. 6. Return output_block.
	 */
	...
	private void padKey(byte[] keyMaster, int keyOff, byte[] tmp, int tmpOff) {
		...
		for (int i = tmpOff; i <= tmpOff + 7; i++) {
			int b = tmp[i];
			tmp[i] = (byte) ((b & 0xfe) | ((((b >> 1) ^ (b >> 2) ^ (b >> 3) ^ (b >> 4) ^ (b >> 5) ^ (b >> 6) ^ (b >> 7)) ^ 0x01) & 0x01));
		}
	}
}
