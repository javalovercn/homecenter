...
public class SP800SecureRandomBuilder {
	 ...
	 *            the number of bits of entropy to be requested from the entropy source on each
	 *            seed/reseed.
	 * @return the current builder.
	 */
	 ...
	 * @param predictionResistant
	 *            specify whether the underlying DRBG in the resulting SecureRandom should reseed on
	 *            each request for bytes.
	...
	public SP800SecureRandom buildCTR(BlockCipher cipher, int keySizeInBits, byte[] nonce, boolean predictionResistant) {
		return new SP800SecureRandom(random, entropySourceProvider.get(entropyBitsRequired),
				new CTRDRBGProvider(cipher, keySizeInBits, nonce, personalizationString, securityStrength), predictionResistant);
	}
	...
	public SP800SecureRandom buildDualEC(Digest digest, byte[] nonce, boolean predictionResistant) {
		return new SP800SecureRandom(random, entropySourceProvider.get(entropyBitsRequired),
				new DualECDRBGProvider(digest, nonce, personalizationString, securityStrength), predictionResistant);
	}
	 ...
	 *
	 * @param pointSet
	 *            an array of DualECPoints to use for DRB generation.
	 * @param digest
	 *            digest algorithm to use in the DRBG underneath the SecureRandom.
	 * @param nonce
	 *            nonce value to use in DRBG construction.
	 ...
	 *            each request for bytes.
	 * @return a SecureRandom supported by a Dual EC DRBG.
	 * @deprecated Don't use this for anything other than research purposes. It appears this
	 *             construct is simply not safe.
	 */
	public SP800SecureRandom buildDualEC(DualECPoints[] pointSet, Digest digest, byte[] nonce, boolean predictionResistant) {
		return new SP800SecureRandom(random, entropySourceProvider.get(entropyBitsRequired),
				new ConfigurableDualECDRBGProvider(pointSet, digest, nonce, personalizationString, securityStrength), predictionResistant);
	}
	...
	private static class ConfigurableDualECDRBGProvider implements DRBGProvider {
		...
		public ConfigurableDualECDRBGProvider(DualECPoints[] pointSet, Digest digest, byte[] nonce, byte[] personalizationString,
