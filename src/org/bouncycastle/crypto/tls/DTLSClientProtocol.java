...
public class DTLSClientProtocol extends DTLSProtocol {
	...
	protected DTLSTransport clientHandshake(ClientHandshakeState state, DTLSRecordLayer recordLayer) throws IOException {
		...
		while (serverMessage.getType() == HandshakeType.hello_verify_request) {
			 ...
			 * of the version of TLS that is expected to be negotiated. DTLS 1.2 and 1.0 clients
			 * MUST use the version solely to indicate packet formatting (which is the same in both
			 * DTLS 1.2 and 1.0) and not as part of version negotiation.
			 */
		...
		if (state.clientCredentials != null && state.clientCredentials instanceof TlsSignerCredentials) {
			 ...
			 */
			SignatureAndHashAlgorithm signatureAndHashAlgorithm = TlsUtils.getSignatureAndHashAlgorithm(state.clientContext,
					signerCredentials);
		...
		if (state.tlsSession != null) {
			state.sessionParameters = new SessionParameters.Builder().setCipherSuite(securityParameters.getCipherSuite())
					...
					.setPSKIdentity(securityParameters.getPSKIdentity()).setSRPIdentity(securityParameters.getSRPIdentity())
	...
	protected void processCertificateStatus(ClientHandshakeState state, byte[] body) throws IOException {
		if (!state.allowCertificateStatus) {
			/*
			 * RFC 3546 3.6. If a server returns a "CertificateStatus" message, then the server MUST
	...
	protected void processServerHello(ClientHandshakeState state, byte[] body) throws IOException {
		...
		if (!Arrays.contains(state.offeredCipherSuites, selectedCipherSuite) || selectedCipherSuite == CipherSuite.TLS_NULL_WITH_NULL_NULL
		 ...
		 */
		{
			...
			if (renegExtData != null) {
				 ...
				 * client MUST then verify that the length of the "renegotiated_connection" field is
				 * zero, and if it is not, MUST abort the handshake (by sending a fatal
		...
		if (sessionServerExtensions != null) {
			{
				 ...
				 * client and then selects a stream or Authenticated Encryption with Associated Data
				 * (AEAD) ciphersuite, it MUST NOT send an encrypt-then-MAC response extension back
				 * to the client.
				 */
			...
			securityParameters.maxFragmentLength = evaluateMaxFragmentLengthExtension(state.resumedSession, sessionClientExtensions,
					sessionServerExtensions, AlertDescription.illegal_parameter);

			securityParameters.truncatedHMac = TlsExtensionsUtils.hasTruncatedHMacExtension(sessionServerExtensions);

			/*
			 * TODO It's surprising that there's no provision to allow a 'fresh' CertificateStatus
			 * to be sent in a session resumption handshake.
			 */
			state.allowCertificateStatus = !state.resumedSession && TlsUtils.hasExpectedEmptyExtensionData(sessionServerExtensions,
					TlsExtensionsUtils.EXT_status_request, AlertDescription.illegal_parameter);

			state.expectSessionTicket = !state.resumedSession && TlsUtils.hasExpectedEmptyExtensionData(sessionServerExtensions,
		 ...
		 * TODO[session-hash]
		 * 
		 * draft-ietf-tls-session-hash-04 4. Clients and servers SHOULD NOT accept handshakes that
		 * do not use the extended master secret [..]. (and see 5.2, 5.3)
		 */
