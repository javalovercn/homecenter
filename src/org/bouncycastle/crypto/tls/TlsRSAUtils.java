...
public class TlsRSAUtils {
	...
	public static byte[] safeDecryptPreMasterSecret(TlsContext context, RSAKeyParameters rsaServerPrivateKey,
		...
		/*
		 * Generate 48 random bytes we can use as a Pre-Master-Secret, if the PKCS1 padding check
		 * should fail.
		 */
		...
		try {
			...
			/*
			 * This should never happen since the decryption should never throw an exception and
			 * return a random value instead.
			 *
			 * In any case, a TLS server MUST NOT generate an alert if processing an RSA-encrypted
			 * premaster secret message fails, or the version number is not as expected. Instead, it
			 * MUST continue the handshake with a randomly generated premaster secret.
			 */
		}

		/*
		 * If ClientHello.client_version is TLS 1.1 or higher, server implementations MUST check the
		 * version number [..].
		 */
