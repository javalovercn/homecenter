...
public class TlsStreamCipher implements TlsCipher {
	...
	public TlsStreamCipher(TlsContext context, StreamCipher clientWriteCipher, StreamCipher serverWriteCipher, Digest clientWriteDigest,
			Digest serverWriteDigest, int cipherKeySize, boolean usesNonce) throws IOException {
	...
	public byte[] encodePlaintext(long seqNo, short type, byte[] plaintext, int offset, int len) {
		/*
		 * draft-josefsson-salsa20-tls-04 2.1 Note that Salsa20 requires a 64-bit nonce. That nonce
		 * is updated on the encryption of every TLS record, and is set to be the 64-bit TLS record
		 * sequence number. In case of DTLS the 64-bit nonce is formed as the concatenation of the
