...
public class TlsClientProtocol extends TlsProtocol {
	 ...
	 * <br>
	 * When data is received, use {@link #offerInput(java.nio.ByteBuffer)} to provide the received
	 * ciphertext, then use {@link #readInput(byte[], int, int)} to read the corresponding
	 * cleartext.<br>
	 * <br>
	 ...
	 * <br>
	 * In blocking mode, this will not return until the handshake is complete. In non-blocking mode,
	 * use {@link TlsPeer#notifyHandshakeComplete()} to receive a callback when the handshake is
	 * complete.
	 *
	 * @param tlsClient
		...
		case HandshakeType.server_hello_done: {
			...
			case CS_CERTIFICATE_REQUEST: {
				...
				if (clientCreds != null && clientCreds instanceof TlsSignerCredentials) {
					 ...
					 * TLS 1.2
					 */
					SignatureAndHashAlgorithm signatureAndHashAlgorithm = TlsUtils.getSignatureAndHashAlgorithm(getContext(),
							signerCredentials);
	...
	protected void receiveServerHelloMessage(ByteArrayInputStream buf) throws IOException {
		...
		if (!Arrays.contains(this.offeredCipherSuites, selectedCipherSuite) || selectedCipherSuite == CipherSuite.TLS_NULL_WITH_NULL_NULL
		...
		if (sessionServerExtensions != null) {
			...
			this.securityParameters.maxFragmentLength = processMaxFragmentLengthExtension(sessionClientExtensions, sessionServerExtensions,
			 ...
			 */
			this.allowCertificateStatus = !this.resumedSession && TlsUtils.hasExpectedEmptyExtensionData(sessionServerExtensions,
					TlsExtensionsUtils.EXT_status_request, AlertDescription.illegal_parameter);

			this.expectSessionTicket = !this.resumedSession && TlsUtils.hasExpectedEmptyExtensionData(sessionServerExtensions,
