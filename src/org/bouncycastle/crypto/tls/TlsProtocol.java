...
public abstract class TlsProtocol {
	...
	protected void completeHandshake() throws IOException {
		try {
			...
			/*
			 * If this was an initial handshake, we are now ready to send and receive application
			 * data.
			 */
			if (!appDataReady) {
			...
			if (this.tlsSession != null) {
				if (this.sessionParameters == null) {
					this.sessionParameters = new SessionParameters.Builder().setCipherSuite(this.securityParameters.getCipherSuite())
							.setCompressionAlgorithm(this.securityParameters.getCompressionAlgorithm())
							.setMasterSecret(this.securityParameters.getMasterSecret()).setPeerCertificate(this.peerCertificate)
	 ...
	 * <br>
	 * After this method returns, the input buffer is "owned" by this object. Other code must not
	 * attempt to do anything with it.<br>
	 * <br>
	 * This method will decrypt and process all records that are fully available. If only part of a
	 * record is available, the buffer will be retained until the remainder of the record is
	 * offered.<br>
	 * <br>
	 * If any records containing application data were processed, the decrypted data can be obtained
	 * using {@link #readInput(byte[], int, int)}. If any records containing protocol data were
	 * processed, a response may have been generated. You should always check to see if there is any
	 * available output after calling this method by calling {@link #getAvailableOutputBytes()}.
	 * 
	 * @param input
	...
	/**
	 * Retrieves received application data. Use {@link #getAvailableInputBytes()} to check how much
	 * application data is currently available. This method functions similarly to
	 * {@link InputStream#read(byte[], int, int)}, except that it never blocks. If no data is
	 * available, nothing will be copied and zero will be returned.<br>
	 * <br>
	 * Only allowed in non-blocking mode.
	 * 
	 * @param buffer
	 ...
	 * @param length
	 *            The maximum number of bytes to read
	 * @return The total number of bytes copied to the buffer. May be less than the length specified
	 *         if the length was greater than the amount of available data.
	 */
	 ...
	 * <br>
	 * After this method returns, the specified section of the buffer will have been processed. Use
	 * {@link #readOutput(byte[], int, int)} to get the bytes to transmit to the other peer.<br>
	 * <br>
	 * This method must not be called until after the handshake is complete! Attempting to call it
	 * before the handshake is complete will result in an exception.
	 * 
	 * @param buffer
	 ...
	 * {@link #readOutput(byte[], int, int)} is guaranteed to be able to return at least this much
	 * data.<br>
	 * <br>
	 * Only allowed in non-blocking mode.
	 * 
	...
	/**
	 * Retrieves encrypted data to be sent. Use {@link #getAvailableOutputBytes()} to check how much
	 * encrypted data is currently available. This method functions similarly to
	 * {@link InputStream#read(byte[], int, int)}, except that it never blocks. If no data is
	 * available, nothing will be copied and zero will be returned.<br>
	 * <br>
	 * Only allowed in non-blocking mode.
	...
	protected short processMaxFragmentLengthExtension(Hashtable clientExtensions, Hashtable serverExtensions, short alertDescription)
		...
		if (maxFragmentLength >= 0) {
			if (!MaxFragmentLength.isValid(maxFragmentLength)
					|| (!this.resumedSession && maxFragmentLength != TlsExtensionsUtils.getMaxFragmentLengthExtension(clientExtensions))) {
	...
	protected void refuseRenegotiation() throws IOException {
		/*
		 * RFC 5746 4.5 SSLv3 clients that refuse renegotiation SHOULD use a fatal handshake_failure
		 * alert.
		 */
