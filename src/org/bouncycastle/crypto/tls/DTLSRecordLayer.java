...
class DTLSRecordLayer implements DatagramTransport {
	...
	public int receive(byte[] buf, int off, int len, int waitMillis) throws IOException {
		...
		for (;;) {
			...
			try {
				...
				byte[] plaintext = recordEpoch.getCipher().decodeCiphertext(getMacSequenceNumber(recordEpoch.getEpoch(), seq), type, record,
	...
	private void sendRecord(short contentType, byte[] buf, int off, int len) throws IOException {
		...
		byte[] ciphertext = writeEpoch.getCipher().encodePlaintext(getMacSequenceNumber(recordEpoch, recordSequenceNumber), contentType,
				buf, off, len);
