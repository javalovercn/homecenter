...
public class NaccacheSternEngine implements AsymmetricBlockCipher {
	 ...
	 * 
	 * @see org.bouncycastle.crypto.AsymmetricBlockCipher#processBlock(byte[], int, int)
	 */
		...
		if (forEncryption) {
			...
			for (int i = 0; i < primes.size(); i++) {
				...
				if (lookup[i].size() != ((BigInteger) primes.elementAt(i)).intValue()) {
					...
					throw new InvalidCipherTextException("Error in lookup Array for " + ((BigInteger) primes.elementAt(i)).intValue()
							+ ": Size mismatch. Expected ArrayList with length " + ((BigInteger) primes.elementAt(i)).intValue()
							+ " but found ArrayList of length " + lookup[i].size());
				}
	...
	/**
	 * Computes the integer x that is expressed through the given primes and the congruences with
