...
public class HC256Engine implements StreamCipher {
	...
	private void init() {
		...
		for (int i = 16; i < 2560; i++) {
			int x = w[i - 2];
			int y = w[i - 15];
			w[i] = (rotateRight(x, 17) ^ rotateRight(x, 19) ^ (x >>> 10)) + w[i - 7] + (rotateRight(y, 7) ^ rotateRight(y, 18) ^ (y >>> 3))
					+ w[i - 16] + i;
		}
	 ...
	 *
	 * @param forEncryption
	 *            whether or not we are for encryption. Irrelevant, as encryption and decryption are
	 *            the same.
	 * @param params
	 *            the parameters required to set up the cipher.
	 * @throws IllegalArgumentException
	 *             if the params argument is inappropriate (ie. the key is not 256 bit long).
	 */
