...
public class Grain128Engine implements StreamCipher {
	...
	private void setKey(byte[] keyBytes, byte[] ivBytes) {
		...
		for (int i = 0; i < nfsr.length; i++) {
			nfsr[i] = ((workingKey[j + 3]) << 24) | ((workingKey[j + 2]) << 16) & 0x00FF0000 | ((workingKey[j + 1]) << 8) & 0x0000FF00
					| ((workingKey[j]) & 0x000000FF);

			lfsr[i] = ((workingIV[j + 3]) << 24) | ((workingIV[j + 2]) << 16) & 0x00FF0000 | ((workingIV[j + 1]) << 8) & 0x0000FF00
