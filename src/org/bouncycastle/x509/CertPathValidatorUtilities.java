...
class CertPathValidatorUtilities {
	 ...
	 */
	protected static ASN1Primitive getExtensionValue(java.security.cert.X509Extension ext, String oid) throws AnnotatedException {
			...
			try {
				if (location.startsWith("ldap://")) {
					...
					pkixParams.addAdditionalStore(
							X509Store.getInstance("ATTRIBUTECERTIFICATE/LDAP", params, BouncyCastleProvider.PROVIDER_NAME));
					pkixParams
							.addAdditionalStore(X509Store.getInstance("CERTIFICATEPAIR/LDAP", params, BouncyCastleProvider.PROVIDER_NAME));
				}
			} catch (Exception e) {
				// cannot happen
	 ...
	 * @param certStores
	 *            a List containing only {@link X509Store} objects. These are used to search for
	 *            certificates.
	 * @return a Collection of all found {@link X509Certificate} or
	 *         {@link org.bouncycastle.x509.X509AttributeCertificate} objects. May be empty but
	 ...
	 * <p>
	 * The <code>issuerPrincipals</code> are a collection with a single <code>X500Principal</code>
	 * for <code>X509Certificate</code>s. For {@link X509AttributeCertificate}s the issuer may
	 * contain more than one <code>X500Principal</code>.
	 * </p>
	 * 
	 * @param dp
	...
	protected static void getCertStatus(Date validDate, X509CRL crl, Object cert, CertStatus certStatus) throws AnnotatedException {
		...
		if (crl_entry.hasExtensions()) {
			try {
				reasonCode = ASN1Enumerated
						.getInstance(CertPathValidatorUtilities.getExtensionValue(crl_entry, X509Extension.reasonCode.getId()));
			} catch (Exception e) {
		...
		// unspecified
		if (!(validDate.getTime() < crl_entry.getRevocationDate().getTime()) || reasonCode == null || reasonCode.getValue().intValue() == 0
				|| reasonCode.getValue().intValue() == 1 || reasonCode.getValue().intValue() == 2
	 ...
	 *            {@link org.bouncycastle.x509.X509AttributeCertificate} for which the CRL should be
	 *            searched.
	 * @param currentDate
	 *            The date for which the delta CRLs must be valid.
	 * @param paramsPKIX
