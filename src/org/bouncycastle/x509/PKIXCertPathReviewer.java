...
public class PKIXCertPathReviewer extends CertPathValidatorUtilities {
	...
	/**
	 * Creates a PKIXCertPathReviewer and initializes it with the given {@link CertPath} and
	 * {@link PKIXParameters} params
	 * 
	 * @param certPath
	 *            the {@link CertPath} to validate
	 ...
	 * messages for each certificate in the path. The global error List is at index 0. The error
	 * lists for each certificate at index 1 to n. The error messages are of type.
	 * 
	 ...
	 * botification messages for each certificate in the path. The global notificatio List is at
	 * index 0. The notification lists for each certificate at index 1 to n. The error messages are
	 * of type.
	 * 
	...
	/*
	 * checks: - signatures - name chaining - validity of certificates - todo: if certificate
	 * revoked (if specified in the parameters)
	 */
		...
		try {
			...
			if (trustColl.size() > 1) {
				...
				ErrorBundle msg = new ErrorBundle(RESOURCE_NAME, "CertPathReviewer.conflictingTrustAnchors",
						new Object[] { Integers.valueOf(trustColl.size()), new UntrustedInput(cert.getIssuerX500Principal()) });
				addError(msg);
								...
								for (int k = 0; k < _nodes.size(); k++) {
									...
									while (_policySetIter.hasNext()) {
										...
										if (!_found) {
											...
											PKIXPolicyNode _newChild = new PKIXPolicyNode(new ArrayList(), i, _newChildExpectedPolicies,
				...
				if (QCStatement.id_etsi_qcs_QcCompliance.equals(stmt.getStatementId())) {
					...
					if (limit.getCurrency().isAlphabetic()) {
						msg = new ErrorBundle(RESOURCE_NAME, "CertPathReviewer.QcLimitValueAlpha",
								new Object[] { limit.getCurrency().getAlphabetic(), new TrustedInput(new Double(value)), limit });
					} else {
						msg = new ErrorBundle(RESOURCE_NAME, "CertPathReviewer.QcLimitValueNum", new Object[] {
	...
	protected void checkRevocation(PKIXParameters paramsPKIX, X509Certificate cert, Date validDate, X509Certificate sign,
			PublicKey workingPublicKey, Vector crlDistPointUrls, Vector ocspUrls, int index) throws CertPathReviewerException {
		...
		try {
			...
			if (crl_coll.isEmpty()) {
				...
				ErrorBundle msg = new ErrorBundle(RESOURCE_NAME, "CertPathReviewer.noCrlInCertstore",
						new Object[] { new UntrustedInput(crlselect.getIssuerNames()), new UntrustedInput(nonMatchingCrlNames),
					...
					if (onlineCRL != null) {
						...
						if (!cert.getIssuerX500Principal().equals(onlineCRL.getIssuerX500Principal())) {
							ErrorBundle msg = new ErrorBundle(RESOURCE_NAME, "CertPathReviewer.onlineCRLWrongCA",
									new Object[] { new UntrustedInput(onlineCRL.getIssuerX500Principal().getName()),
											new UntrustedInput(cert.getIssuerX500Principal().getName()), new UntrustedUrlInput(location) });
							addNotification(msg, index);
							continue;
						}
						...
						if (onlineCRL.getNextUpdate() == null || pkixParams.getDate().before(onlineCRL.getNextUpdate())) {
							...
							ErrorBundle msg = new ErrorBundle(RESOURCE_NAME, "CertPathReviewer.onlineValidCRL",
									new Object[] { new TrustedInput(onlineCRL.getThisUpdate()), new TrustedInput(onlineCRL.getNextUpdate()),
			...
			if (dci != null) {
				...
				try {
					baseSelect.setMaxCRLNumber(
							((ASN1Integer) getExtensionValue(crl, CRL_NUMBER)).getPositiveValue().subtract(BigInteger.valueOf(1)));
				} catch (AnnotatedException ae) {
