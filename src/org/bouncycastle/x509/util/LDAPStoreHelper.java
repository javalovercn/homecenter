 ...
 * {@link org.bouncycastle.jce.X509LDAPCertStoreParameters} with the given information of the
 * subject (for all kind of certificates) or issuer (for CRLs), respectively, if a
 * {@link org.bouncycastle.x509.X509CertStoreSelector} or
 * {@link org.bouncycastle.x509.X509AttributeCertificate} is given with that details.
 * </p>
 * <p>
 * For the used schemes see:
 * <ul>
 * <li><a href="http://www.ietf.org/rfc/rfc2587.txt">RFC 2587</a>
 * <li><a href="http://www3.ietf.org/proceedings/01mar/I-D/pkix-ldap-schema-01.txt">Internet X.509
 * Public Key Infrastructure Additional LDAP Schema for PKIs and PMIs</a>
 * </ul>
 * </p>
 */
public class LDAPStoreHelper {
	...
	/**
	 * Can use the subject and serial and the subject and serialNumber of the certificate of the
	 * given of the X509CertStoreSelector. If a certificate for checking is given this has higher
	 * precedence.
	 *
	 * @param xselector
	 ...
	 *            Subject attribute names (like "CN", "O", "OU") to use to search in the LDAP
	 *            directory
	 ...
	 * the forward {@link org.bouncycastle.x509.X509CertStoreSelector} of the given selector.
	 *
	 * @param xselector
	 *            The selector with the search criteria.
	 * @param attrs
	 *            Attributes which contain the attribute certificates in the LDAP directory.
	 * @param attrNames
	 ...
	 *             if an error occurs while searching.
	 */
	private List crossCertificatePairSubjectSearch(X509CertPairStoreSelector xselector, String[] attrs, String attrNames[],
			String subjectAttributeNames[]) throws StoreException {
		List list = new ArrayList();
	 ...
	 *             if an error occurs while searching.
	 */
	private List attrCertSubjectSerialSearch(X509AttributeCertStoreSelector xselector, String[] attrs, String attrNames[],
			String subjectAttributeNames[]) throws StoreException {
		List list = new ArrayList();
	 ...
	 *            Issuer attribute names (like "CN", "O", "OU") to use to search in the LDAP
	 *            directory
	 ...
	 * @param attrs
	 *            The attributes in the LDAP which hold the certificate, attribute certificate,
		...
		while (i < list.size()) {
			...
			try {
				...
				try {
					...
					pair = new X509CertificatePair(new CertificatePair(Certificate.getInstance(new ASN1InputStream(forward).readObject()),
	...
	/**
	 * Returns the CRLs for issued certificates for other CAs matching the given selector. <br>
	 * The authorityRevocationList attribute includes revocation information regarding certificates
	 * issued to other CAs.
	 *
	 * @param selector
