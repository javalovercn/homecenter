...
/**
 * Non-public class used by {@link JarFile} and {@link JarInputStream} to manage the verification of
 * signed JARs. {@code JarFile} and {@code JarInputStream} objects are expected to have a
 * {@code JarVerifier} instance member which can be used to carry out the tasks associated with
 * verifying a signed JAR. These tasks would typically include:
 * <ul>
 * <li>verification of all signed signature files
 * <li>confirmation that all signed data was signed only by the party or parties specified in the
 * signature block data
 * <li>verification that the contents of all signature files (i.e. {@code .SF} files) agree with the
 * JAR entries information found in the JAR manifest.
 * </ul>
 */
public class JarVerifier {
	/**
	 * List of accepted digest algorithms. This list is in order from most preferred to least
	 * preferred.
	 */
	...
	private final Hashtable<String, HashMap<String, Attributes>> signatures = new Hashtable<String, HashMap<String, Attributes>>(5);

	private final Hashtable<String, Certificate[]> certificates = new Hashtable<String, Certificate[]>(5);
	...
	/**
	 * Stores and a hash and a message digest and verifies that massage digest matches the hash.
	 */
	static class VerifierEntry extends OutputStream {
		...
		VerifierEntry(final String name, final MessageDigest digest, final byte[] hash, final Certificate[][] certChains,
				final Hashtable<String, Certificate[][]> verifedEntries) {
			this.name = name;
			this.digest = digest;
			this.hash = hash;
		...
		/**
		 * Verifies that the digests stored in the manifest match the decrypted digests from the .SF
		 * file. This indicates the validity of the signing, not the integrity of the file, as its
		 * digest must be calculated and verified when its contents are read.
		 *
		 * @throws SecurityException
		 *             if the digest value stored in the manifest does <i>not</i> agree with the
		 *             decrypted digest as recovered from the <code>.SF</code> file.
		 */
		void verify() {
	...
	private static SecurityException invalidDigest(final String signatureFile, final String name, final String jarName) {
	...
	/**
	 * Invoked for each new JAR entry read operation from the input stream. This method constructs
	 * and returns a new {@link VerifierEntry} which contains the certificates used to sign the
	 * entry and its hash value as specified in the JAR MANIFEST format.
	 *
	 * @param name
	 *            the name of an entry in a JAR file which is <b>not</b> in the {@code META-INF}
	 *            directory.
	 * @return a new instance of {@link VerifierEntry} which can be used by callers as an
	 *         {@link OutputStream}.
	 */
	VerifierEntry initEntry(final String name) {
		...
		for (int i = 0; i < DIGEST_ALGORITHMS.length; i++) {
			...
			try {
				return new VerifierEntry(name, MessageDigest.getInstance(algorithm, BCProvider.getBCProvider()), hashBytes, certChainsArray,
						verifiedEntries);
	...
	/**
	 * Add a new meta entry to the internal collection of data held on each JAR entry in the
	 * {@code META-INF} directory including the manifest file itself. Files associated with the
	 * signing of a JAR would also be added to this collection.
	 *
	 * @param name
	 *            the name of the file located in the {@code META-INF} directory.
	 * @param buf
	 ...
	 *         validity of the signature(s). {@code false} if the associated JAR file has no entries
	 *         at all in its {@code
	 *         META-INF} directory. This situation is indicative of an invalid JAR file.
	 *         <p>
	 *         Will also return {@code true} if the JAR file is <i>not</i> signed.
	 * @throws SecurityException
	 *             if the JAR file is signed and it is determined that a signature block file
	 *             contains an invalid signature for the corresponding signature file.
	 */
	...
	private static X509Certificate[] createChain(final X509Certificate signer, final X509Certificate[] candidates) {
	...
	public static Certificate[] verifySignature(final byte[] sfBytes, final byte[] signatureBlock) throws Exception {
		...
		while (it.hasNext()) {
			...
			final X509Certificate certFromSignedData = new JcaX509CertificateConverter().setProvider(BCProvider.getBCProvider())
		...
		for (i = 0; i < certs.length; i++) {
			if (issuer.toString().equals(certs[i].getIssuerDN().toString()) && snum.equals(certs[i].getSerialNumber())) {
				issuerSertIndex = i;
				break;
			}
		}
	...
	static HashMap<String, byte[]> readMetaEntries(final ZipFile jarFile, final boolean verificationRequired) throws Throwable {
		...
		for (final ZipEntry entry : metaEntries) {
			...
			if (entryName.equalsIgnoreCase(JarFile.MANIFEST_NAME) && !metaEntriesMap.containsKey(JarFile.MANIFEST_NAME)) {

				metaEntriesMap.put(JarFile.MANIFEST_NAME, readFully(jarFile.getInputStream(entry)));
				...
				if (endsWithIgnoreCase(entryName, ".SF") || endsWithIgnoreCase(entryName, ".DSA") || endsWithIgnoreCase(entryName, ".RSA")
	...
	/**
	 * Returns a <code>boolean</code> indication of whether or not the associated jar file is
	 * signed.
	 *
	...
	private boolean verify(final Attributes attributes, final String entry, final byte[] data, final int start, final int end,
			final boolean ignoreSecondEndline, final boolean ignorable) {
		for (int i = 0; i < DIGEST_ALGORITHMS.length; i++) {
			final String algorithm = DIGEST_ALGORITHMS[i];
	...
	/**
	 * Returns all of the {@link java.security.cert.Certificate} chains that were used to verify the
	 * signature on the JAR entry called {@code name}. Callers must not modify the returned arrays.
	 *
	 * @param name
	...
	/**
	 * Remove all entries from the internal collection of data held about each JAR entry in the
