 ...
 */
package third.apache.lucene.analysis.util;
...
public abstract class StopwordAnalyzerBase extends Analyzer {
  ...
  protected static CharArraySet loadStopwordSet(final boolean ignoreCase,
    ...
    Reader reader = null;
    try {
      reader = IOUtils.getDecodingReader(aClass.getResourceAsStream(resource), Charset.forName("UTF-8"));
  ...
  protected static CharArraySet loadStopwordSet(Path stopwords) throws IOException {
    Reader reader = null;
    try {
      reader = Files.newBufferedReader(stopwords, Charset.forName("UTF-8"));
