...
public class QuartzSchedulerThread extends Thread {
    ...
    QuartzSchedulerThread(QuartzScheduler qs, QuartzSchedulerResources qsRsrcs, boolean setDaemon, int threadPrio) {
        ...
        if(qsRsrcs.isThreadsInheritInitializersClassLoadContext()) {
            L.V = L.WShop ? false : ThirdLogManager.info("QuartzSchedulerThread Inheriting ContextClassLoader of thread: " + Thread.currentThread().getName());
            ...
            try {
                ...
                if(availThreadCount > 0) { // will always be true, due to semantics of blockForAvailableThreads...
                    ...
                    try {
                        ...
                        acquiresFailed = 0;
                        L.V = L.WShop ? false : ThirdLogManager.debug("batch acquisition of " + (triggers == null ? 0 : triggers.size()) + " triggers");
                        ...
                        if (acquiresFailed == 0) {
                            ThirdLogManager.error("quartzSchedulerThreadLoop: RuntimeException "
                                    +e.getMessage(), e);
                        }
                        if (acquiresFailed < Integer.MAX_VALUE)
                            acquiresFailed++;
                    ...
                    if (triggers != null && !triggers.isEmpty()) {
                        ...
                        for (int i = 0; i < bndles.size(); i++) {
                            ...
                            if (exception instanceof RuntimeException) {
                                ThirdLogManager.error("RuntimeException while firing trigger " + triggers.get(i), exception);
                                qsRsrcs.getJobStore().releaseAcquiredTrigger(triggers.get(i));
                                continue;
                            }
                            ...
                            if (qsRsrcs.getThreadPool().runInThread(shell) == false) {
                                ...
                                // say not to do...
                                ThirdLogManager.error("ThreadPool.runInThread() return false!");
                                qsRsrcs.getJobStore().triggeredJobComplete(triggers.get(i), bndle.getJobDetail(), CompletedExecutionInstruction.SET_ALL_JOB_TRIGGERS_ERROR);
                            }
                ...
                ThirdLogManager.error("Runtime error occurred in main trigger firing loop.", re);
            }
        } // while (!halted)
