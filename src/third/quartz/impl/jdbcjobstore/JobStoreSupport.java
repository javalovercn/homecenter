...
import third.quartz.JobPersistenceException;
import third.quartz.ObjectAlreadyExistsException;
import third.quartz.Scheduler;
import third.quartz.SchedulerConfigException;
import third.quartz.SchedulerException;
import third.quartz.SimpleTrigger;
...
import third.quartz.impl.DefaultThreadExecutor;
import third.quartz.impl.matchers.GroupMatcher;
import third.quartz.impl.matchers.StringMatcher;
import third.quartz.impl.matchers.StringMatcher.StringOperatorName;
import third.quartz.impl.triggers.SimpleTriggerImpl;
import third.quartz.spi.ClassLoadHelper;
...
public abstract class JobStoreSupport implements JobStore, Constants {
    ...
    protected HashMap<String, JobCalendar> calendarCache = new HashMap<String, JobCalendar>();
    ...
    public void initialize(ClassLoadHelper loadHelper,
        ...
        if(isThreadsInheritInitializersClassLoadContext()) {
        	L.V = L.WShop ? false : ThirdLogManager.info("JDBCJobStore threads will inherit ContextClassLoader of thread: " + Thread.currentThread().getName());
            ...
            if (getUseDBLocks()) {
                if(getDriverDelegateClass() != null) {
                    if(getSelectWithLockSQL() == null) {
                        ...
                        L.V = L.WShop ? false : ThirdLogManager.info("Detected usage of MSSQLDelegate class - defaulting 'selectWithLockSQL' to '" + msSqlDflt + "'.");
                        setSelectWithLockSQL(msSqlDflt);
                    }
                }
                L.V = L.WShop ? false : ThirdLogManager.info("Using db table-based data access locking (synchronization).");
    ...
    /**
     * @see third.quartz.spi.JobStore#schedulerStarted()
     */
    public void schedulerStarted() throws SchedulerException {
        ...
        L.V = L.WShop ? false : ThirdLogManager.debug("JobStore background threads started (as scheduler was started).");
    }
    ...
    public void shutdown() {
        ...
        try {
            ...
            ThirdLogManager.warn("Database connection shutdown unsuccessful.", sqle);
        }        
        
        L.V = L.WShop ? false : ThirdLogManager.debug("JobStore background threads shutdown.");
    }
...
//        conn = getAttributeRestoringConnection(conn);
        ...
        try {
            ...
            ThirdLogManager.warn("Failed to override connection auto commit/transaction isolation.", sqle);
        } catch (Throwable e) {
    ...
    protected void releaseLock(String lockName, boolean doIt) {
        if (doIt) {
            try {
                ...
                ThirdLogManager.error("Error returning lock: " + le.getMessage(), le);
            }
        }
    }
    ...
    protected void recoverJobs(Connection conn) throws JobPersistenceException {
        try {
            ...
            L.V = L.WShop ? false : ThirdLogManager.info("Recovery complete.");
            ...
            L.V = L.WShop ? false : ThirdLogManager.info("Removed " + n + " stale fired job entries.");
    ...
    /**
     * <p>
     * Store the given <code>{@link third.quartz.JobDetail}</code> and <code>{@link third.quartz.Trigger}</code>.
     * </p>
     * 
     * @param newJob
    ...
    /**
     * <p>
     * Store the given <code>{@link third.quartz.JobDetail}</code>.
     * </p>
     * 
     * @param newJob
     *          The <code>JobDetail</code> to be stored.
    ...
    /**
     * <p>
     * Store the given <code>{@link third.quartz.Trigger}</code>.
     * </p>
     * 
     * @param newTrigger
     *          The <code>Trigger</code> to be stored.
    ...
    /**
     * <p>
     * Remove (delete) the <code>{@link third.quartz.Job}</code> with the given
     * name, and any <code>{@link third.quartz.Trigger}</code> s that reference
     * it.
     * </p>
     * 
     * <p>
     ...
     * 
     * @see #removeJob(java.sql.Connection, third.quartz.JobKey)
    ...
    /**
     * <p>
     * Retrieve the <code>{@link third.quartz.JobDetail}</code> for the given
     * <code>{@link third.quartz.Job}</code>.
     * </p>
     * 
    ...
    /**
     * <p>
     * Remove (delete) the <code>{@link third.quartz.Trigger}</code> with the
     * given name.
     * </p>
     * 
     * <p>
    ...
    /** 
     * @see third.quartz.spi.JobStore#replaceTrigger(TriggerKey, OperableTrigger)
     */
    ...
    /**
     * <p>
     * Retrieve the given <code>{@link third.quartz.Trigger}</code>.
     * </p>
     * 
    ...
    void resetTriggerFromErrorState(Connection conn, final TriggerKey triggerKey)
        ...
        try {
            ...
            L.V = L.WShop ? false : ThirdLogManager.info("Trigger " + triggerKey + " reset from ERROR state to: " + newState);
        } catch (SQLException e) {
            throw new JobPersistenceException(
    ...
    public void storeCalendar(final String calName,
        final JobCalendar calendar, final boolean replaceExisting, final boolean updateTriggers)
        throws JobPersistenceException {
        executeInLock(
    ...
    protected void storeCalendar(Connection conn, 
            String calName, JobCalendar calendar, boolean replaceExisting, boolean updateTriggers)
        throws JobPersistenceException {
        try {
     ...
     *         match.
     */
    public JobCalendar retrieveCalendar(final String calName)
        throws JobPersistenceException {
        return (JobCalendar)executeWithoutLock( // no locks necessary for read...
            new TransactionCallback() {
                public Object execute(Connection conn) throws JobPersistenceException {
    ...
    protected JobCalendar retrieveCalendar(Connection conn,
            String calName)
        throws JobPersistenceException {
        // all calendars are persistent, but we can lazy-cache them during run
        // time as long as we aren't running clustered.
    	JobCalendar cal = (isClustered) ? null : calendarCache.get(calName);
        if (cal != null) {
            return cal;
        }
    ...
    /**
     * <p>
     * Get the number of <code>{@link third.quartz.Job}</code> s that are
    ...
    /**
     * <p>
     * Get the number of <code>{@link third.quartz.Trigger}</code> s that are
    ...
    /**
     * <p>
     * Get the names of all of the <code>{@link third.quartz.Job}</code> s that
    ...
    /**
     * <p>
     * Get the names of all of the <code>{@link third.quartz.Trigger}</code> s
    ...
    /**
     * <p>
     * Get the names of all of the <code>{@link third.quartz.Job}</code>
     * groups.
     * </p>
     * 
     * <p>
    ...
    /**
     * <p>
     * Get the names of all of the <code>{@link third.quartz.Trigger}</code>
     * groups.
     * </p>
     * 
     * <p>
    ...
    /**
     * <p>
     * Pause the <code>{@link third.quartz.Trigger}</code> with the given name.
     * </p>
     * 
    ...
    /**
     * <p>
     * Pause the <code>{@link third.quartz.Job}</code> with the given name - by
    ...
    /**
     * <p>
     * Pause all of the <code>{@link third.quartz.Job}s</code> matching the given
     * groupMatcher - by pausing all of their <code>Trigger</code>s.
     * </p>
     * 
     * @see #resumeJobs(third.quartz.impl.matchers.GroupMatcher)
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * <p>
     * Resume (un-pause) the <code>{@link third.quartz.Trigger}</code> with the
     * given name.
     * </p>
     * 
     * <p>
    ...
    /**
     * <p>
     * Resume (un-pause) the <code>{@link third.quartz.Job}</code> with the
     * given name.
     * </p>
     * 
     * <p>
    ...
    /**
     * <p>
     * Resume (un-pause) all of the <code>{@link third.quartz.Job}s</code> in
     * the given group.
     * </p>
     * 
     * <p>
     ...
     * </p>
     * 
     * @see #pauseJobs(third.quartz.impl.matchers.GroupMatcher)
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * <p>
     * Pause all of the <code>{@link third.quartz.Trigger}s</code> matching the
     * given groupMatcher.
     * </p>
     * 
     * @see #resumeTriggerGroup(java.sql.Connection, third.quartz.impl.matchers.GroupMatcher)
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * <p>
     * Pause all of the <code>{@link third.quartz.Trigger}s</code> in the
     * given group.
     * </p>
     * 
     * @see #resumeTriggers(third.quartz.impl.matchers.GroupMatcher)
     */
    ...
    /**
     * <p>
     * Resume (un-pause) all of the <code>{@link third.quartz.Trigger}s</code>
     ...
     * </p>
     * 
     * @see #pauseTriggers(third.quartz.impl.matchers.GroupMatcher)
     */
    @SuppressWarnings("unchecked")
     ...
     * 
     * @see #resumeAll()
     * @see #pauseTriggerGroup(java.sql.Connection, third.quartz.impl.matchers.GroupMatcher)
     */
            ...
            try {
                ...
                for(TriggerKey triggerKey: keys) {
                    ...
                    try {
                        ...
                        try {
                            ThirdLogManager.error("Error retrieving job, setting trigger state to ERROR.", jpe);
                            getDelegate().updateTriggerState(conn, triggerKey, STATE_ERROR);
                        } catch (SQLException sqle) {
                            ThirdLogManager.error("Unable to set trigger state to ERROR.", sqle);
                        }
                        continue;
                    }
     ...
     * associated <code>Job</code>), and that the <code>{@link third.quartz.JobDataMap}</code>
    ...
    protected void triggeredJobComplete(Connection conn,
        ...
        try {
            if (triggerInstCode == CompletedExecutionInstruction.DELETE_TRIGGER) {
                ...
                L.V = L.WShop ? false : ThirdLogManager.info("Trigger " + trigger.getKey() + " set to ERROR state.");
                getDelegate().updateTriggerState(conn, trigger.getKey(),
                        STATE_ERROR);
                signalSchedulingChangeOnTxCompletion(0L);
                ...
                L.V = L.WShop ? false : ThirdLogManager.info("All triggers of Job " + 
    ...
    protected void logWarnIfNonZero(int val, String warning) {
        if (val > 0) {
            L.V = L.WShop ? false : ThirdLogManager.info(warning);
        } else {
            L.V = L.WShop ? false : ThirdLogManager.debug(warning);
        }
    }
    ...
    protected void closeConnection(Connection conn) {
        if (conn != null) {
            try {
                ...
                ThirdLogManager.error("Failed to close Connection", e);
            } catch (Throwable e) {
    ...
    protected <T> T retryExecuteInNonManagedTXLock(String lockName, TransactionCallback<T> txCallback) {
        for (int retry = 1; !shutdown; retry++) {
            try {
                ...
                ThirdLogManager.error("retryExecuteInNonManagedTXLock: RuntimeException " + e.getMessage(), e);
            }
            try {
    ...
    class ClusterManager extends Thread {
        ...
        private boolean manage() {
            ...
            try {
                ...
                numFails = 0;
                L.V = L.WShop ? false : ThirdLogManager.debug("ClusterManager: Check-in complete.");
            } catch (Exception e) {
                if(numFails % 4 == 0) {
                    ThirdLogManager.error(
    ...
    class MisfireHandler extends Thread {
        ...
        private RecoverMisfiredJobsResult manage() {
            try {
                L.V = L.WShop ? false : ThirdLogManager.debug("MisfireHandler: scanning for misfires...");
