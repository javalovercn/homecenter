...
public class UpdateLockRowSemaphore extends DBSemaphore {
    ...
    protected void executeSQL(Connection conn, final String lockName, final String expandedSQL, final String expandedInsertSQL) throws LockException {
        ...
        for (int i = 0; i < RETRY_COUNT; i++) {
            try {
                ...
                lastFailure = e;
                if ((i + 1) == RETRY_COUNT) {
                    L.V = L.WShop ? false : ThirdLogManager.debug("Lock '{}' was not obtained by: {}", lockName, Thread.currentThread().getName());
                } else {
                    L.V = L.WShop ? false : ThirdLogManager.debug("Lock '{}' was not obtained by: {} - will try again.", lockName, Thread.currentThread().getName());
                }
                try {
                    Thread.sleep(1000L);
    ...
    private boolean lockViaUpdate(Connection conn, String lockName, String sql) throws SQLException {
        ...
        try {
            ...
            L.V = L.WShop ? false : ThirdLogManager.debug("Lock '" + lockName + "' is being obtained: " + Thread.currentThread().getName());
    ...
    private void lockViaInsert(Connection conn, String lockName, String sql) throws SQLException {
        L.V = L.WShop ? false : ThirdLogManager.debug("Inserting new lock row for lock: '" + lockName + "' being obtained by thread: " + Thread.currentThread().getName());
        PreparedStatement ps = conn.prepareStatement(sql);
        try {
            ps.setString(1, lockName);
