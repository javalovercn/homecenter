...
import third.quartz.CalendarIntervalScheduleBuilder;
import third.quartz.CalendarIntervalTrigger;
...
import third.quartz.TriggerUtils;
import third.quartz.DateBuilder.IntervalUnit;


/**
 * <p>A concrete <code>{@link Trigger}</code> that is used to fire a <code>{@link third.quartz.JobDetail}</code>
 ...
 * (see {@link #setRepeatIntervalUnit(third.quartz.DateBuilder.IntervalUnit)}) as specified in the trigger's definition.
...
public class CalendarIntervalTriggerImpl extends AbstractTrigger<CalendarIntervalTrigger> implements CalendarIntervalTrigger, CoreTrigger {
    ...
    /* (non-Javadoc)
     * @see third.quartz.DateIntervalTriggerI#getRepeatIntervalUnit()
     */
    ...
    /* (non-Javadoc)
     * @see third.quartz.DateIntervalTriggerI#getRepeatInterval()
     */
    ...
    /* (non-Javadoc)
     * @see third.quartz.CalendarIntervalTriggerI#getTimeZone()
     */
    ...
    /* (non-Javadoc)
     * @see third.quartz.DateIntervalTriggerI#getTimesTriggered()
     */
    ...
    protected Date getFireTimeAfter(Date afterTime, boolean ignoreEndTime) {
        ...
        if(getRepeatIntervalUnit().equals(IntervalUnit.SECOND)) {
            ...
            int initialHourOfDay = sTime.get(java.util.Calendar.HOUR_OF_DAY);
    ...
    private boolean daylightSavingHourShiftOccurredAndAdvanceNeeded(java.util.Calendar newTime, int initialHourOfDay, Date afterTime) {
        if(isPreserveHourOfDayAcrossDaylightSavings() && newTime.get(java.util.Calendar.HOUR_OF_DAY) != initialHourOfDay) {
            newTime.set(java.util.Calendar.HOUR_OF_DAY, initialHourOfDay);
            if (newTime.get(java.util.Calendar.HOUR_OF_DAY) != initialHourOfDay) {
