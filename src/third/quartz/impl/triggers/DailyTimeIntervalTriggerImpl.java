...
import third.quartz.DailyTimeIntervalScheduleBuilder;
import third.quartz.DailyTimeIntervalTrigger;
import third.quartz.DateBuilder;
import third.quartz.JobExecutionContext;
import third.quartz.JobExecutionException;
import third.quartz.ScheduleBuilder;
import third.quartz.Scheduler;
import third.quartz.SchedulerException;
import third.quartz.TimeOfDay;
import third.quartz.Trigger;
import third.quartz.DateBuilder.IntervalUnit;

/**
 * A concrete implementation of DailyTimeIntervalTrigger that is used to fire a <code>{@link third.quartz.JobDetail}</code>
 ...
 * (see {@link #setRepeatIntervalUnit(third.quartz.DateBuilder.IntervalUnit)}) during a given time window on specified days of the week.</p>
 * 
...
public class DailyTimeIntervalTriggerImpl extends AbstractTrigger<DailyTimeIntervalTrigger> implements DailyTimeIntervalTrigger, CoreTrigger {
    ...
    /* (non-Javadoc)
     * @see third.quartz.DailyTimeIntervalTriggerI#getRepeatIntervalUnit()
     */
    ...
    /* (non-Javadoc)
     * @see third.quartz.DailyTimeIntervalTriggerI#getRepeatInterval()
     */
    ...
    /* (non-Javadoc)
     * @see third.quartz.DailyTimeIntervalTriggerI#getTimesTriggered()
     */
    ...
    private boolean isSameDay(Date d1, Date d2) {
    	...
    	java.util.Calendar c1 = createCalendarTime(d1);
    	java.util.Calendar c2 = createCalendarTime(d2);
    ...
    private Date advanceToNextDayOfWeekIfNecessary(Date fireTime, boolean forceToAdvanceNextDay) {
        ...
        java.util.Calendar fireTimeStartDateCal = createCalendarTime(fireTimeStartDate);          
        int dayOfWeekOfFireTime = fireTimeStartDateCal.get(java.util.Calendar.DAY_OF_WEEK);
        ...
        if (forceToAdvanceNextDay || !daysOfWeekToFire.contains(dayOfWeekOfFireTime)) {
          ...
          for(int i=1; i <= 7; i++) {
            fireTimeStartDateCal.add(java.util.Calendar.DATE, 1);
            dayOfWeekOfFireTime = fireTimeStartDateCal.get(java.util.Calendar.DAY_OF_WEEK);
