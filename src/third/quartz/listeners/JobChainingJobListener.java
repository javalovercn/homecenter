...
public class JobChainingJobListener extends JobListenerSupport {
    ...
    public void jobWasExecuted(JobExecutionContext context, JobExecutionException jobException) {
        ...
        L.V = L.WShop ? false : ThirdLogManager.info("Job '" + context.getJobDetail().getKey() + "' will now chain to Job '" + sj + "'");

        try {
             context.getScheduler().triggerJob(sj);
        } catch(SchedulerException se) {
            ThirdLogManager.error("Error encountered during chaining to Job '" + sj + "'", se);
        }
    }
}
