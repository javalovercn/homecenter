 ...
 * <code>{@link third.quartz.spi.ThreadPool}</code> interface.
 * </p>
 * 
 * <p>
...
public class SimpleThreadPool implements ThreadPool {
    ...
    public void shutdown(boolean waitForJobsToComplete) {
        ...
        synchronized (nextRunnableLock) {
            L.V = L.WShop ? false : ThirdLogManager.debug("Shutting down threadpool...");
            ...
            if (waitForJobsToComplete == true) {
                ...
                L.V = L.WShop ? false : ThirdLogManager.debug("No executing jobs remaining, all threads stopped.");
            }
            L.V = L.WShop ? false : ThirdLogManager.debug("Shutdown of threadpool complete.");
        }
    }
        ...
        public void run() {
            ...
            while (run.get()) {
                try {
                    ...
                    try {
                        ThirdLogManager.error("Worker thread was interrupt()'ed.", unblock);
                    } catch(Exception e) {
                        // ignore to help with a tomcat glitch
                    }
                } catch (Throwable exceptionInRunnable) {
                    try {
                        ThirdLogManager.error("Error while executing the Runnable: ",
                            exceptionInRunnable);
                    } catch(Exception e) {
                        // ignore to help with a tomcat glitch
                    }
            ...
            //if (L.isInWorkshop)
            try {
                L.V = L.WShop ? false : ThirdLogManager.debug("WorkerThread is shut down.");
            } catch(Exception e) {
                // ignore to help with a tomcat glitch
            }
        }
